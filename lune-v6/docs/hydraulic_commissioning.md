...
For every port, use `POST /api/v1/settings/{text|number}` with its stable
...
`GET /api/v1/zones/{zone}` returns the complete commissioning record.
...
`GET /api/v1/diagnostics` publishes six hydraulic alarm records. Every
