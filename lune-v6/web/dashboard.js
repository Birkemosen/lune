...
`,vn=5,xn=35,bo=.5;function ht(t){return t==null||Number.isNaN(Number(t))?"":(Math.round(Number(t)*10)/10).toFixed(1)}function ls(t){if(t==null)return null;let e=String(t).trim().replace(",",".").replace(/[^\d.+-]/g,"");if(!e)return null;let o=Number(e);if(!Number.isFinite(o))return null;let a=Math.round(o/bo)*bo;return Math.min(xn,Math.max(vn,Number(a.toFixed(1))))}function cs(t){let e=Number(C(h.setpoint(t)));return Number.isFinite(e)?e:null}function go(t){let e=Number(C(h.coordinatorOffset(t)));return Number.isFinite(e)?e:0}function Ke(t){let e=Number(C(h.effectiveSetpoint(t)));if(Number.isFinite(e))return e;let o=cs(t);return o==null?null:Number((o+go(t)).toFixed(1))}function Et(t){return Math.min(xn,Math.max(vn,Number(Number(t).toFixed(1))))}function ds(t,e){if(!e)return d("common.disabled");let o=String(t||"IDLE").toUpperCase();return o==="HEATING"?d("state.heating"):o==="IDLE"?d("state.idle"):o==="OFF"?d("state.off"):o==="FAULT"?d("common.fault"):o==="MANUAL"?d("state.manual"):o==="OVERHEATED"?d("state.overheated"):o==="CALIBRATING"?d("state.calibrating"):o}var dc=O({tag:"zone-detail",state:t=>({zone:t.zone,temp:"---",setpoint:"---",valve:"---",state:"---"}),render:is,methods:{update(t,e){var p;let o=D("selectedZone"),a=String(L(h.state(o))||"").toUpperCase(),r=ce(h.enabled(o));this.zone=o,t.dataset.zone=String(o),document.activeElement!==e.setpoint&&(e.setpoint.value=ht(Ke(o))),e.base.textContent=ue((p=C(h.baseSetpoint(o)))!=null?p:C(h.setpoint(o)));let n=C(h.coordinatorOffset(o));e.offset.textContent=n==null?"---":(n>0?"+":"")+Number(n).toFixed(1)+"\xB0C",e.temp.textContent=ue(C(h.temp(o))),e.ret.textContent=ue(C("sensor-manifold_return_temperature")),e.valve.textContent=mt(C(h.valve(o)));let s=e.badge;s.textContent=ds(a,r);let c=r?a==="HEATING"?"badge-heating":a==="IDLE"?"badge-idle":a==="FAULT"?"badge-fault":"":"badge-disabled";s.className="zd-badge"+(c?" "+c:""),e.toggle.classList.toggle("on",r)},commitSetpoint(t){let e=this.zone,o=ls(t);if(o==null)return null;let a=Et(o-go(e));return Jt(e,a),Ke(e)},incSetpoint(){let t=this.zone,e=Ke(t),o=Et((e==null?20:e)+bo);Jt(t,Et(o-go(t)))},decSetpoint(){let t=this.zone,e=Ke(t),o=Et((e==null?20:e)-bo);Jt(t,Et(o-go(t)))},toggleEnabled(){let t=this.zone,e=ce(h.enabled(t));_a(t,!e)}},onMount(t,e){let o={setpoint:e.querySelector(".zd-setpoint"),temp:e.querySelector(".zd-temp"),base:e.querySelector(".zd-base"),offset:e.querySelector(".zd-offset"),ret:e.querySelector(".zd-ret"),valve:e.querySelector(".zd-valve"),badge:e.querySelector(".zd-badge"),toggle:e.querySelector(".btn-toggle"),inc:e.querySelector(".btn-inc"),dec:e.querySelector(".btn-dec")};o.inc.onclick=()=>t.incSetpoint(),o.dec.onclick=()=>t.decSetpoint(),o.toggle.onclick=()=>t.toggleEnabled();let a=()=>{let s=t.commitSetpoint(o.setpoint.value);o.setpoint.value=s!=null?ht(s):ht(Ke(t.zone))};o.setpoint.addEventListener("keydown",s=>{s.key==="Enter"?(s.preventDefault(),o.setpoint.blur()):s.key==="Escape"?(s.preventDefault(),o.setpoint.value=ht(Ke(t.zone)),o.setpoint.blur()):s.key==="ArrowUp"?(s.preventDefault(),t.incSetpoint(),o.setpoint.value=ht(Ke(t.zone))):s.key==="ArrowDown"&&(s.preventDefault(),t.decSetpoint(),o.setpoint.value=ht(Ke(t.zone)))}),o.setpoint.addEventListener("blur",a),o.setpoint.addEventListener("focus",()=>{requestAnimationFrame(()=>o.setpoint.select())});let r=()=>t.update(e,o),n=s=>{let c=D("selectedZone");(s===h.temp(c)||s===h.setpoint(c)||s===h.baseSetpoint(c)||s===h.effectiveSetpoint(c)||s===h.coordinatorOffset(c)||s===h.valve(c)||s===h.state(c)||s===h.enabled(c))&&r()};for(let s=1;s<=6;s++)k(h.temp(s),n),k(h.setpoint(s),n),k(h.baseSetpoint(s),n),k(h.effectiveSetpoint(s),n),k(h.coordinatorOffset(s),n),k(h.valve(s),n),k(h.state(s),n),k(h.enabled(s),n);k("sensor-manifold_return_temperature",r),U("selectedZone",r),E(e),r()}});var ps=`
...
`,Pc=O({tag:"zone-room-card",render:vs,onMount(t,e){let o=e.querySelector(".zr-friendly");function a(){return D("selectedZone")}let r=xe(e);r.text(o,{read:()=>kt(a())||"",commit:n=>Aa(a(),n)}),U("selectedZone",r.discard),U("zoneNames",r.refresh),E(e),r.refresh()}});var xs=`
...
`,Vc=O({tag:"zone-actuator-card",render:ws,onMount(t,e){var p,u,l;let o=Number(D("selectedZone")||1),a={orip:e.querySelector(".za-orip"),crip:e.querySelector(".za-crip"),ofac:e.querySelector(".za-ofac"),cfac:e.querySelector(".za-cfac"),ph:e.querySelector(".za-ph"),fault:e.querySelector(".za-fault"),faultVal:e.querySelector(".za-fault-val"),faultBtn:e.querySelector(".recovery-fault-btn"),factorsBtn:e.querySelector(".recovery-factors-btn"),relearnBtn:e.querySelector(".recovery-relearn-btn"),status:e.querySelector(".za-status")};function r(){o=Number(D("selectedZone")||1),a.orip.textContent=zn(C(h.motorOpenRipples(o))),a.crip.textContent=zn(C(h.motorCloseRipples(o))),a.ofac.textContent=kn(C(h.motorOpenFactor(o))),a.cfac.textContent=kn(C(h.motorCloseFactor(o))),a.ph.textContent=ys(C(h.preheatAdvance(o)));let m=String(L(h.motorLastFault(o))||"").toUpperCase(),g=m&&m!=="NONE"&&m!=="OK";a.fault.hidden=!g,g&&(a.faultVal.textContent=m)}let n=null;function s(m,g){a.status.textContent=m,a.status.className="za-status show "+(g?"ok":"err"),clearTimeout(n),n=setTimeout(()=>{a.status.classList.remove("show")},4e3)}function c(m,g){let f=m(o);s(g,!0),f&&typeof f.then=="function"&&f.then(b=>{b&&b.ok===!1&&s(d("diagnostics.recovery.rejected"),!1)}).catch(()=>s(d("diagnostics.recovery.unreachable"),!1))}(p=a.faultBtn)==null||p.addEventListener("click",()=>{c(Da,"\u2713 "+d("diagnostics.recovery.faultSent",{zone:Se(o)}))}),(u=a.factorsBtn)==null||u.addEventListener("click",()=>{confirm(d("diagnostics.recovery.confirmFactors",{zone:Se(o)}))&&c(Ra,"\u2713 "+d("diagnostics.recovery.factorsReset",{zone:Se(o)}))}),(l=a.relearnBtn)==null||l.addEventListener("click",()=>{confirm(d("diagnostics.recovery.confirmRelearn",{zone:Se(o)}))&&c(Pa,"\u2713 "+d("diagnostics.recovery.relearnStarted",{zone:Se(o)}))}),U("selectedZone",r);for(let m=1;m<=6;m++)k(h.motorOpenRipples(m),r),k(h.motorCloseRipples(m),r),k(h.motorOpenFactor(m),r),k(h.motorCloseFactor(m),r),k(h.preheatAdvance(m),r),k(h.motorLastFault(m),r);E(e),r()}});var De=6,ks="var(--flow-disabled)",_n="var(--flow-unknown)",Cn="var(--accent)",Ft="var(--flow-return)",Go="var(--text-strong)",zs="var(--flow-disabled)",nt="var(--flow-label)",fo="var(--flow-disabled)",Ko="var(--flow-label)",Ln="var(--flow-label)",Sn="var(--flow-return)",Ss="#66BB6A",_s="#FF6361",ee={w:1160,h:372,boxX:440,boxY:26,boxW:280,boxH:90,srcY:116,fanY:168,zoneY:262,zoneXs:[92,286,480,674,868,1062],srcSpread:15,bgDstHW:28,srcHW:7},Z={w:760,h:424,boxX:26,boxY:148,boxW:168,boxH:92,srcX:196,endX:386,nameX:446,midY:190,zoneYs:[56,120,184,248,312,376],spread:10,bgDstHW:15,srcHW:4},Cs=`
.flow-wrap {
  width: 100%;
  border-radius: 8px;
  overflow: hidden;
...
`;function Is(t){let e=Rs[t.level]||{label:"?",color:"var(--text-secondary)"},o=Nn(t.tag||""),a=Nn(t.msg||"");return'<div class="log-line"><span class="lv" style="color:'+e.color+'">'+e.label+'</span><span class="tag">'+o+'</span><span class="msg">'+a+"</span></div>"}function Nn(t){return String(t).replace(/[&<>]/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;"})[e])}var ad=O({tag:"logs-view",render:Os,onMount(t,e){let o=e.querySelector(".logs-stream"),a=e.querySelector(".pause-btn"),r=e.querySelector(".clear-btn"),n=e.querySelector(".download-btn"),s=e.querySelector(".bottom-btn"),c=!1;function p(){o.scrollTop=o.scrollHeight}function u(){if(c)return;let l=Kt();if(!l||!l.length){o.innerHTML='<div class="logs-empty">'+d("logs.waiting")+"</div>";return}let m=o.scrollHeight-o.scrollTop-o.clientHeight<40;o.innerHTML=l.map(Is).join(""),m&&p()}a.addEventListener("click",()=>{c=!c,a.textContent=c?d("logs.resume"):d("logs.pause"),a.classList.toggle("on",c),c||u()}),r.addEventListener("click",()=>{da()}),n.addEventListener("click",()=>{n.disabled=!0,Ua().catch(l=>{console.error("[Logs] download failed:",l),window.alert(d("logs.downloadFailed"))}).finally(()=>{n.disabled=!1})}),s.addEventListener("click",()=>{p()}),U("deviceLog",u),E(e),u()}});var qs=`
.diag-i2c {
  background: var(--panel-bg-vibrant);
  border: 1px solid var(--panel-border);
  border-radius: 8px;
...
`;I("settings-return-temp-card",fi);function rt(t){return!!(t&&t!=="None")}function hi(t){return"Probe "+t}function vi(){for(let t=1;t<=6;t++)if(rt(L(h.probe(t))))return!0;return!1}function xi(){let t="";for(let e=1;e<=8;e++)t+='<option value="Probe '+e+'">Probe '+e+"</option>";return t}var yi=()=>{let t=xi(),e="";for(let o=1;o<=6;o++)e+=`
