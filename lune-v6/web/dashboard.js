...
`;I("ui-kit",Fr);function he(t){let e=d(t);return`<span class="help-badge" tabindex="0" role="img" aria-label="${String(e).replace(/"/g,"&quot;")}" data-i18n-label="${t}">?<span class="help-tip" data-i18n="${t}">${e}</span></span>`}function Tr(t,e){let o=Math.abs(Number(t));return!Number.isFinite(o)||o<1e3?e:Math.pow(10,Math.floor(Math.log10(o))-1)}function Nr(t){let e=String(t),o=e.indexOf(".");return o<0?0:e.length-o-1}function xe(t,e={}){let o=t.querySelector(e.title||".ui-card-title"),a=document.createElement("div");a.className="ui-form-banner",a.innerHTML='<span class="ui-form-banner-msg" data-i18n="form.unsaved">Unsaved changes</span><span class="ui-form-banner-btns"><button type="button" class="ui-form-discard" data-i18n="form.discard">Discard</button><button type="button" class="ui-form-apply" data-i18n="form.apply">Apply</button></span>',o?o.insertAdjacentElement("afterend",a):t.insertAdjacentElement("afterbegin",a);let r=[],n=()=>a.classList.toggle("show",r.some(h=>h.dirty)),s=(h,y)=>{h.dirty=y,n()};function c(h){return h.markDirty=()=>s(h,!0),r.push(h),h}function p(h,y){let T={dirty:!1,input:h},S=y.baseStep!=null?y.baseStep:parseFloat(h.step)||1,M=Nr(S),_=y.min!=null?y.min:h.min!==""?parseFloat(h.min):-1/0,R=y.max!=null?y.max:h.max!==""?parseFloat(h.max):1/0,B=H=>M>0?Number(H).toFixed(M):String(Math.round(Number(H)));if(!y.nostep){let H=document.createElement("div");H.className="ui-stepper",h.parentNode.insertBefore(H,h);let P=document.createElement("button");P.type="button",P.className="ui-step-btn",P.textContent="\u2212",P.setAttribute("aria-label",d("common.decrease"));let X=document.createElement("button");X.type="button",X.className="ui-step-btn",X.textContent="+",X.setAttribute("aria-label",d("common.increase")),H.appendChild(P),H.appendChild(h),H.appendChild(X);let te=$=>{if(h.disabled)return;let A=parseFloat(h.value);Number.isFinite(A)||(A=parseFloat(h.placeholder)),Number.isFinite(A)||(A=0);let z=Math.min(R,Math.max(_,A+$*Tr(A,S)));h.value=B(z),s(T,!0)};P.addEventListener("click",()=>te(-1)),X.addEventListener("click",()=>te(1)),h.addEventListener("keydown",$=>{$.key==="Enter"&&h.blur()})}return h.addEventListener("input",()=>s(T,!0)),T.sync=()=>{let H=y.read();h.value=H!=null&&Number.isFinite(Number(H))?B(H):""},T.commit=()=>{let H=parseFloat(h.value);Number.isFinite(H)&&y.commit(Math.min(R,Math.max(_,H)))},c(T)}function u(h,y){let T={dirty:!1,input:h};return h.addEventListener("input",()=>s(T,!0)),T.sync=()=>{let S=y.read();h.value=S!=null?S:""},T.commit=()=>y.commit(h.value.trim()),c(T)}function l(h,y){let T={dirty:!1,input:h};return h.addEventListener("change",()=>s(T,!0)),T.sync=()=>{let S=y.read();S!=null&&(h.value=S)},T.commit=()=>y.commit(h.value),c(T)}function m(h,y){let T={dirty:!1,input:h,staged:!1},S=h.closest(".ui-row"),M=()=>{h.classList.toggle("on",T.staged),S&&S.classList.toggle("is-on",T.staged),h.setAttribute("aria-checked",T.staged?"true":"false"),y.onChange&&y.onChange(T.staged)};return h.addEventListener("click",()=>{T.staged=!T.staged,s(T,!0),M()}),T.sync=()=>{T.staged=!!y.read(),M()},T.commit=()=>y.commit(T.staged),c(T)}function g(h){let y={dirty:!1,sync:h.sync,commit:h.commit};return c(y)}let f=()=>r.forEach(h=>{!h.dirty&&h.sync&&h.sync()}),b=()=>{r.forEach(h=>{h.dirty&&(h.commit&&h.commit(),h.dirty=!1)}),n(),e.onApply&&e.onApply()},w=()=>{r.forEach(h=>{h.dirty=!1,h.sync&&h.sync()}),n(),e.onDiscard&&e.onDiscard()};return a.querySelector(".ui-form-apply").addEventListener("click",b),a.querySelector(".ui-form-discard").addEventListener("click",w),E(a),{num:p,text:u,select:l,toggle:m,custom:g,refresh:f,apply:b,discard:w,isDirty:()=>r.some(h=>h.dirty)}}var Dr=`
...
`;I("hv6-header",Dr);var Rr=()=>`
  ...
  </header>`,tt=t=>`<svg class="menu-icon" viewBox="0 0 24 24" aria-hidden="true">${t}</svg>`,no=t=>`<span class="v6-nav-dot${t==="warn"?" is-warn":""}" data-nav-dot hidden aria-hidden="true"></span>`,Pr=()=>`
  <nav class="v6-side-nav" aria-label="Primary navigation">
    <div class="v6-nav-group"><div class="v6-nav-heading">Home</div>
      ...
      <div class="v6-nav-zones">
        <a href="#" class="v6-side-link" data-section="zones">${tt('<path d="M5 19V9l7-5 7 5v10"/><path d="M9 19v-6h6v6"/>')}<span>Zones</span>${no("warn")}</a>
      </div>
    </div>
    <div class="v6-nav-group"><div class="v6-nav-heading">System</div>
      <a href="#" class="v6-side-link" data-section="diagnostics">${tt('<path d="M4 19h16M6 16V8m4 8V4m4 12v-6m4 6V7"/><path d="m5 5 3 2 4-4 4 3 3-2"/>')}<span>Diagnostics</span>${no("warn")}</a>
    ...
    <button type="button" class="v6-side-link v6-more-toggle" aria-expanded="false">${tt('<circle cx="5" cy="12" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="19" cy="12" r="1.5"/>')}<span>More</span>${no()}</button>
  ...
  </nav>`,tn={overview:["Overview","Local heating status and current exceptions"],zones:["Zones","Physical loops, applied targets and valve state"],diagnostics:["Diagnostics","Health, evidence and recovery"],motorlab:["Motor lab","Instrumented stroke capture and endstop thresholds"],settings:["Settings","Device configuration and safety"],help:["Help","Guidance for operating Lune V6"]};function an(t){t&&(Pe(t.section),t.focus==="touch"&&requestAnimationFrame(()=>{let e=document.querySelector(".touch-settings");e&&(e.open=!0);let o=document.querySelector(".settings-touch-card");o&&o.scrollIntoView({behavior:"smooth",block:"center"})}))}function on(t){return t?t.kind==="touch"?d("status.attention.approveTouch"):t.kind==="faults"?t.count===1?d("status.attention.zoneFaultOne"):d("status.attention.zoneFaultMany",{count:t.count}):"":""}O({tag:"hv6-header",render:Rr,onMount(t,e){let o=e.querySelector("#hdr-live"),a=e.querySelector("#v6-view-title"),r=e.querySelector("#v6-view-subtitle"),n=e.querySelector("#hdr-update"),s=e.querySelector("#hdr-attention");function c(){let l=D("firmwareUpdateAvailable");n.hidden=!l,l&&(n.textContent=d("status.updateAvailable",{version:l.latest}),n.title=d("settings.firmware.badgeTitle"))}function p(){let l=Co(),m=D("section")||"overview",g=!!(l&&l.section!==m);s.hidden=!g,g?(s.textContent=on(l),s.title=on(l),s.dataset.kind=l.kind):delete s.dataset.kind}n.addEventListener("click",()=>{Pe("settings");let l=document.querySelector(".settings-firmware-card");if(!l)return;let m=l.closest("details");m&&(m.open=!0),l.scrollIntoView({behavior:"smooth",block:"center"})}),s.addEventListener("click",()=>{an(Co())});function u(){let l=D("section")||"overview",m=tn[l]||tn.overview;a.textContent=m[0],r.textContent=m[1],o.textContent=D("live")?d("status.live"):d("status.offline"),o.classList.toggle("is-live",!!D("live")),p()}U("section",u),U("live",u),U("firmwareUpdateAvailable",c),k(i.authorityProposalPending,p);for(let l=1;l<=6;l++)k(v.state(l),p),k(v.motorLastFault(l),p);E(e),u(),c(),p()}});O({tag:"hv6-sidebar",render:Pr,onMount(t,e){let o=e,a=e.querySelectorAll("[data-section]"),r=e.querySelector(".v6-more-toggle"),n=e.querySelector('[data-section="settings"]'),s=e.querySelector('[data-section="zones"]'),c=e.querySelector('[data-section="diagnostics"]');function p(m,g,f,b){if(!m)return;let w=m.querySelector("[data-nav-dot]");w&&(w.hidden=!g,g?(m.setAttribute("aria-label",`${f}, ${b}`),m.title=b):(m.removeAttribute("aria-label"),m.removeAttribute("title")))}function u(){let m=jt(),g=_o(),f=g===1?d("status.attention.zoneFaultOne"):d("status.attention.zoneFaultMany",{count:g});if(p(n,m,d("nav.settings"),d("status.attention.approveTouch")),p(s,g>0,d("nav.zones"),f),p(c,g>0,d("nav.diagnostics"),f),r){let b=r.querySelector("[data-nav-dot]");b&&(b.hidden=!m,b.classList.toggle("is-warn",!1)),m?(r.setAttribute("aria-label",d("status.attention.moreHasSettings")),r.title=d("status.attention.approveTouch")):(r.removeAttribute("aria-label"),r.removeAttribute("title"))}}function l(){let m=D("section");a.forEach(g=>{g.dataset.section&&(g.dataset.section===m?g.classList.add("active"):g.classList.remove("active"),g.setAttribute("aria-current",g.dataset.section===m?"page":"false"))}),u()}a.forEach(m=>m.addEventListener("click",g=>{g.preventDefault();let f=m.dataset.section;f==="settings"&&jt()?an({kind:"touch",section:"settings",focus:"touch"}):Pe(f),o.classList.contains("more-open")&&(o.classList.remove("more-open"),r&&r.setAttribute("aria-expanded","false"))})),r&&r.addEventListener("click",()=>{let m=o.classList.toggle("more-open");r.setAttribute("aria-expanded",String(m))}),U("section",l),k(i.authorityProposalPending,u);for(let m=1;m<=6;m++)k(v.state(m),u),k(v.motorLastFault(m),u);E(e),l()}});function ue(t){return t!=null&&!isNaN(t)?Math.round(t*10)/10+"\xB0C":"---"}function mt(t){return t!=null&&!isNaN(t)?(t|0)+"%":"---"}function nn(t){if(t==null||isNaN(t)||t<0)return"---";t=t|0;var e=t/86400|0,o=t%86400/3600|0,a=t%3600/60|0;return e>0?e+"d "+o+"h "+a+"m":o>0?o+"h "+a+"m":a+"m"}var Or=`
.connectivity-card {
...
`,Ll=O({tag:"connectivity-card",render:Ir,onMount(t,e){let o=e.querySelector(".cc-ip"),a=e.querySelector(".cc-ssid"),r=e.querySelector(".cc-mac"),n=e.querySelector(".cc-up"),s=e.querySelector(".cc-ver"),c=0,p=Date.now(),u=!1;function l(){if(!u){n.textContent="---";return}let f=Math.max(0,Math.floor((Date.now()-p)/1e3)),b=nn(c+f);n.textContent!==b&&(n.textContent=b)}function m(){o.textContent=L(i.ip)||"---",a.textContent=L(i.ssid)||"---",r.textContent=L(i.mac)||"---",s.textContent=L(i.firmware)||"---";let f=C(i.uptime);if(f!=null&&!isNaN(f)&&f>=0){let b=f|0;(!u||b!==c)&&(c=b,p=Date.now(),u=!0)}l()}k(i.ip,m),k(i.ssid,m),k(i.mac,m),k(i.firmware,m),k(i.uptime,m);let g=setInterval(l,1e3);e.addEventListener("hv6-unmount",()=>clearInterval(g),{once:!0}),E(e),m()}});var qr="http://www.w3.org/2000/svg",Hr=`
.chart-card {
  border: 1px solid var(--panel-border);
  border-radius: 8px;
  background: var(--panel-bg-vibrant);
...
`;I("chart-kit",Hr);function G(t,e,o){let a=document.createElementNS(qr,t);if(e)for(let r in e)a.setAttribute(r,e[r]);return o!=null&&(a.textContent=o),a}function gt(t){if(!t.length)return"";if(t.length<3)return"M "+t.map(a=>`${a.x.toFixed(2)} ${a.y.toFixed(2)}`).join(" L ");let e=.16,o=`M ${t[0].x.toFixed(2)} ${t[0].y.toFixed(2)}`;for(let a=0;a<t.length-1;a++){let r=t[a-1]||t[a],n=t[a],s=t[a+1],c=t[a+2]||s,p=n.x+(s.x-r.x)*e,u=n.y+(s.y-r.y)*e,l=s.x-(c.x-n.x)*e,m=s.y-(c.y-n.y)*e;o+=` C ${p.toFixed(2)} ${u.toFixed(2)}, ${l.toFixed(2)} ${m.toFixed(2)}, ${s.x.toFixed(2)} ${s.y.toFixed(2)}`}return o}function ro(t,e,o){let a=t.filter(c=>Number.isFinite(c));if(!a.length)return{min:e,max:o};let r=Math.min(...a),n=Math.max(...a);r===n&&(r-=1,n+=1);let s=(n-r)*.12;return{min:r-s,max:n+s}}function bt(t,e,o){let a=document.createElement("div");a.className="chart-tooltip",e.appendChild(a);let r=G("g",{class:"chart-cursor",style:"display:none"}),n=G("line",{class:"chart-cursor-line",y1:o.plotTop,y2:o.plotBottom});r.appendChild(n);let s=[];t.appendChild(r);function c(m){let g=0,f=1/0;for(let b=0;b<o.count;b++){let w=Math.abs(m-o.xAt(b));w<f&&(f=w,g=b)}return g}function p(m){let g=t.getScreenCTM();if(!g)return null;let f=t.createSVGPoint();return f.x=m.clientX,f.y=m.clientY,f.matrixTransform(g.inverse())}function u(m){if(!o.count)return;let g=p(m);if(!g)return;let f=c(g.x),b=o.xAt(f);n.setAttribute("x1",b),n.setAttribute("x2",b);let w=o.dots(f);for(;s.length<w.length;){let S=G("circle",{class:"chart-cursor-dot",r:3.4});r.appendChild(S),s.push(S)}s.forEach((S,M)=>{M<w.length?(S.setAttribute("cx",b),S.setAttribute("cy",w[M].y),S.setAttribute("fill",w[M].color),S.style.display=""):S.style.display="none"}),r.style.display="";let h=o.rows(f).map(S=>`<div class="tt-row"><span class="tt-swatch" style="background:${S.color}"></span>${S.label}<span class="tt-val">${S.value}</span></div>`).join("");a.innerHTML=`<div class="tt-time">${o.label(f)}</div>${h}`,a.classList.add("show");let y=e.getBoundingClientRect(),T=m.clientX-y.left+14;T+a.offsetWidth>y.width-6&&(T=m.clientX-y.left-a.offsetWidth-14),a.style.left=Math.max(6,T)+"px",a.style.top=Math.max(6,m.clientY-y.top+12)+"px"}function l(){a.classList.remove("show"),r.style.display="none"}return t.addEventListener("pointermove",u),t.addEventListener("pointerleave",l),()=>{t.removeEventListener("pointermove",u),t.removeEventListener("pointerleave",l),a.remove()}}var Lt=1e3,$o=180,Ne=14,Br=42,$r=44,Ue=42,lo=Lt-Ue-Br,Ze=$o-Ne-$r,ot=Ne+Ze,jo=24*3600,rn=ye+2,sn=ye+3,so=ye+4,jr="var(--series-warm)",Vr="var(--series-cool)",ln="var(--series-solar)",Zr=`
...
`;I("graph-widgets",Zr);var cn=()=>'<div class="chart-card"><div class="chart-head"><span class="chart-title" data-i18n="overview.graph.flowReturnDemand">Flow / Return / Demand</span><span class="chart-sub gw-dt">\u2014</span></div><div class="gw-controls" role="toolbar" data-i18n-label="overview.graph.layers" aria-label="Flow chart layers"><button type="button" class="gw-toggle" data-layer="flow" aria-pressed="true" data-i18n="overview.graph.layers.flow">Flow</button><button type="button" class="gw-toggle" data-layer="return" aria-pressed="true" data-i18n="overview.graph.layers.return">Return</button><button type="button" class="gw-toggle" data-layer="demand" aria-pressed="true" data-i18n="overview.graph.layers.demand">Demand</button></div><svg class="gw-flow"></svg></div>',dn=()=>'<div class="chart-card"><div class="chart-head"><span class="chart-title" data-i18n="overview.graph.demandIndex">Demand Index</span><span class="chart-sub gw-demand-text">\u2014</span></div><svg class="gw-demand"></svg></div>',Ur=t=>t.variant==="flow-return"?`<div class="graph-widgets">${cn()}</div>`:t.variant==="demand"?`<div class="graph-widgets">${dn()}</div>`:`<div class="graph-widgets">${cn()}${dn()}</div>`;function pn(t,e){return Number.isFinite(t)?e==="%"?Math.round(t)+"%":t.toFixed(1):"\u2014"}function Wr(t,e){return Number.isFinite(t)?e==="%"?Math.round(t)+"%":t.toFixed(1)+"\xB0":"\u2014"}function Vo(t,e,o){let a=[];for(let r=0;r<t.length;r++){let n=t[r];if(!n||n[0]<o)continue;let s=n[e];s==null||!Number.isFinite(s)||a.push({t:n[0],v:s})}return a}var co=(t,e)=>Ue+Math.max(0,Math.min(1,(t-e)/jo))*lo;function Kr(t,e,o){let a=Number(Date.now()/1e3)|0,r=3600,n=Math.ceil((a-jo)/r)*r,s=Math.floor(a/r)*r,c=Math.floor(a/r)*r;for(let u=n;u<=s;u+=r){let l=o-(a-u),m=co(l,e),g=new Date(u*1e3),f=u===c,b=ot+16;t.appendChild(G("text",{x:m,y:b,"text-anchor":"end",transform:`rotate(-45 ${m.toFixed(1)} ${b})`,class:"chart-hour"+(f?" now":"")},String(g.getHours()).padStart(2,"0")))}let p=co(o,e);t.appendChild(G("line",{x1:p,y1:Ne,x2:p,y2:ot,stroke:"var(--series-solar)","stroke-width":"1","stroke-dasharray":"2 3",opacity:".55","vector-effect":"non-scaling-stroke"}))}function Gr(t){let e=[];if(t.forEach(n=>n.forEach(s=>e.push(s.v))),!e.length)return{min:0,max:10};let o=Math.min(...e),a=Math.max(...e);o===a&&(o-=.5,a+=.5);let r=(a-o)*.1;return o-=r,a+=r,{min:o,max:a}}function Xr(t,e,o){let a=t.filter(r=>r.unit==="C").map(r=>Vo(e,r.index,o));return Gr(a)}function un(t,e,o,a,r,n){t.innerHTML="",t.setAttribute("viewBox",`0 0 ${Lt} ${$o}`),t.setAttribute("preserveAspectRatio","xMidYMid meet");let s=o.map(b=>Vo(a,b.index,r));if(!s.some(b=>b.length))return t.appendChild(G("text",{x:Lt/2,y:$o/2,"text-anchor":"middle",class:"chart-empty"},"Collecting history\u2026")),null;let c=Xr(o,a,r),p=Math.max(.001,c.max-c.min),u=b=>Ne+(1-(b-c.min)/p)*Ze,l=b=>Ne+(1-Math.max(0,Math.min(100,b))/100)*Ze,m=(b,w)=>b.unit==="%"?l(w):u(w);for(let b=0;b<3;b++){let w=b/2,h=Ne+w*Ze;t.appendChild(G("line",{x1:Ue,y1:h,x2:Ue+lo,y2:h,class:"chart-grid"})),o.some(y=>y.unit==="C")&&t.appendChild(G("text",{x:Ue-6,y:h+4,"text-anchor":"end",class:"chart-tick"},pn(c.max-p*w,"C")+"\xB0")),o.some(y=>y.unit==="%")&&t.appendChild(G("text",{x:Ue+lo+6,y:h+4,"text-anchor":"start",class:"chart-tick"},pn(100-100*w,"%")))}t.appendChild(G("line",{x1:Ue,y1:ot,x2:Ue+lo,y2:ot,class:"chart-axis"})),o.some(b=>b.unit==="C")&&t.appendChild(G("text",{x:9,y:Ne+Ze/2,transform:`rotate(-90 9 ${(Ne+Ze/2).toFixed(1)})`,"text-anchor":"middle",class:"chart-axis-label"},d("overview.graph.axis.temp"))),o.some(b=>b.unit==="%")&&t.appendChild(G("text",{x:Lt-9,y:Ne+Ze/2,transform:`rotate(90 ${Lt-9} ${(Ne+Ze/2).toFixed(1)})`,"text-anchor":"middle",class:"chart-axis-label"},d("overview.graph.axis.demand"))),Kr(t,r,n),o.forEach((b,w)=>{let h=s[w].map(T=>({x:co(T.t,r),y:m(b,T.v)}));if(!h.length)return;let y=gt(h);b.fill&&t.appendChild(G("path",{d:y+` L ${h[h.length-1].x.toFixed(1)} ${ot} L ${h[0].x.toFixed(1)} ${ot} Z`,fill:b.fill,stroke:"none"})),t.appendChild(G("path",{d:y,fill:"none",stroke:b.color,"stroke-width":String(b.width||2.2),"stroke-linecap":"round","stroke-linejoin":"round"}))});let g=[];for(let b=0;b<a.length;b++){let w=a[b];if(!w||w[0]<r)continue;let h=o.map(y=>w[y.index]);h.every(y=>y==null||!Number.isFinite(y))||g.push({t:w[0],vals:h})}if(!g.length)return null;let f=Date.now();return bt(t,e,{count:g.length,plotTop:Ne,plotBottom:ot,xAt:b=>co(g[b].t,r),label:b=>new Date(f-(n-g[b].t)*1e3).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),dots:b=>o.map((w,h)=>({y:m(w,g[b].vals[h]),color:w.color})).filter((w,h)=>Number.isFinite(g[b].vals[h])),rows:b=>o.map((w,h)=>({color:w.color,label:w.label,value:Wr(g[b].vals[h],w.unit)})).filter((w,h)=>Number.isFinite(g[b].vals[h]))})}function io(t,e,o){let a=Vo(t,e,o);return a.length?a[a.length-1].v:null}var Pl=O({tag:"graph-widgets",state:t=>({variant:t&&t.variant||"both"}),render:Ur,onMount(t,e){let o=e.querySelector(".gw-dt"),a=e.querySelector(".gw-demand-text"),r=e.querySelector(".gw-flow"),n=e.querySelector(".gw-demand"),s=Array.from(e.querySelectorAll(".gw-toggle")),c={flow:!0,return:!0,demand:!0},p=null,u=null;function l(){s.forEach(f=>{let b=f.dataset.layer;f.classList.toggle("is-off",!c[b]),f.setAttribute("aria-pressed",c[b]?"true":"false")})}function m(){let f=[];return c.flow&&f.push({index:rn,color:jr,label:d("overview.graph.layers.flow"),unit:"C",width:2.4}),c.return&&f.push({index:sn,color:Vr,label:d("overview.graph.layers.return"),unit:"C",width:2}),c.demand&&f.push({index:so,color:ln,label:d("overview.graph.layers.demand"),unit:"%",width:1.8,fill:"rgba(255,193,77,.10)"}),f}function g(){let f=D("zoneStateHistory"),b=f&&Array.isArray(f.entries)?f.entries:[],w=f&&f.uptime_s||Number(Date.now()/1e3)|0,h=w-jo;if(r){p&&p();let y=io(b,rn,h),T=io(b,sn,h),S=io(b,so,h),M=[];y!=null&&T!=null&&M.push("\u0394 "+(y-T).toFixed(1)+"\xB0"),S!=null&&M.push(Math.round(S)+"%"),o.textContent=M.length?M.join(" \xB7 "):"\u2014",p=un(r,r.closest(".chart-card"),m(),b,h,w)}if(n){u&&u();let y=io(b,so,h);a.textContent=y!=null?Math.round(y)+"%":"\u2014",u=un(n,n.closest(".chart-card"),[{index:so,color:ln,label:d("overview.graph.layers.demand"),unit:"%",width:2.2,fill:"var(--series-cool-fill)"}],b,h,w)}}s.forEach(f=>{f.addEventListener("click",()=>{let b=f.dataset.layer;c[b]=!c[b],!c.flow&&!c.return&&!c.demand&&(c[b]=!0),l(),g()})}),U("zoneStateHistory",g),E(e),l(),g()}});var We={0:{labelKey:"state.off",color:"#2c4875"},1:{labelKey:"state.manual",color:"#7aa7ce"},2:{labelKey:"state.calibrating",color:"#ffd380"},3:{labelKey:"state.waitCal",color:"#4e6977"},4:{labelKey:"state.waitTemp",color:"#4e6977"},5:{labelKey:"state.heating",color:"var(--accent)"},6:{labelKey:"state.idle",color:"#39354c"},7:{labelKey:"state.overheated",color:"#ff6361"},255:{labelKey:"",color:"transparent"}},Mt=24*3600,Yr=Mt,At=18,Wo=4,at=54,uo=32,ft=4,mo=10,bn=6,fn="#ffc14d",Zo=9,mn=ye+1,vn=ft+ye*(At+Wo)-Wo,Uo=vn+bn,po=vn+bn+mo+uo,Jr=`
.timeline-card {
  border: 1px solid var(--panel-border);
  border-radius: 8px;
  background: var(--panel-bg-vibrant);
...
`;I("zone-state-timeline",Jr);var Qr=()=>`
...
`;I("zone-grid",os);var as=()=>'<div class="zone-grid" aria-label="Zones"></div>',Wl=O({tag:"zone-grid",state:t=>({selection:t.selection!==!1,navigate:t.navigate!==!1}),render:as,onMount(t,e){for(let o=1;o<=6;o++)e.appendChild(Q("zone-card",{zone:o,selection:t.selection,navigate:t.navigate}))}});var ns=`
.zone-card {
...
`,tc=O({tag:"zone-card",state:t=>({zone:t.zone,selection:t.selection!==!1,navigate:t.navigate!==!1}),render:rs,onMount(t,e){let o=t.zone,a=v.temp(o),r=v.state(o),n=v.enabled(o),s=e.querySelector(".zc-state-label"),c=e.querySelector(".zc-zone-name"),p=e.querySelector(".zc-friendly"),u=e.querySelector(".zc-temp"),l=e.querySelector(".zc-target"),m=e.querySelector(".zc-valve-value");function g(){var R;let b=ce(n),w=String(L(r)||"").toUpperCase()||"OFF",h=String(L(v.motorLastFault(o))||"").toUpperCase(),y=h&&h!=="NONE"&&h!=="OK",T=b&&(w==="FAULT"||y)?"FAULT":w,S=t.selection&&D("selectedZone")===o,M=pt(o);c.innerHTML=Oe(o),p.textContent=M?"":"---",p.hidden=!!M,e.setAttribute("aria-label",Se(o)),u.textContent=ue(C(a)),l.textContent=d("zone.card.setpoint",{value:ue((R=C(v.effectiveSetpoint(o)))!=null?R:C(v.setpoint(o)))}),m.textContent=mt(C(v.valve(o)));let _=b?T:"OFF";s.textContent=_==="HEATING"?d("state.heating"):_==="IDLE"?d("state.idle"):_==="FAULT"?d("common.fault"):_==="MANUAL"?d("state.manual"):_==="OVERHEATED"?d("state.overheated"):_==="CALIBRATING"?d("state.calibrating"):d("state.off"),e.title=y?d("zone.card.fault",{fault:h}):"",e.classList.toggle("active",S),S?e.setAttribute("aria-current","location"):e.removeAttribute("aria-current"),e.setAttribute("aria-label",`${c.textContent}, ${u.textContent}, ${l.textContent}, ${s.textContent}. Open details.`),e.classList.toggle("disabled",!b),e.classList.toggle("zs-heating",b&&_==="HEATING"),e.classList.toggle("zs-fault",b&&_==="FAULT"),e.classList.toggle("zs-idle",b&&_==="IDLE"),e.classList.toggle("zs-off",!b||_==="OFF")}function f(){Vt(o),t.navigate&&Pe("zones"),e.dispatchEvent(new CustomEvent("zone-open",{bubbles:!0,detail:{zone:o}}))}e.addEventListener("click",f),k(a,g),k(v.setpoint(o),g),k(v.effectiveSetpoint(o),g),k(v.valve(o),g),k(r,g),k(n,g),k(v.motorLastFault(o),g),U("selectedZone",g),U("zoneNames",g),g()}});var ss=`
...
`,hn=5,xn=35,bo=.5;function vt(t){return t==null||Number.isNaN(Number(t))?"":(Math.round(Number(t)*10)/10).toFixed(1)}function ls(t){if(t==null)return null;let e=String(t).trim().replace(",",".").replace(/[^\d.+-]/g,"");if(!e)return null;let o=Number(e);if(!Number.isFinite(o))return null;let a=Math.round(o/bo)*bo;return Math.min(xn,Math.max(hn,Number(a.toFixed(1))))}function cs(t){let e=Number(C(v.setpoint(t)));return Number.isFinite(e)?e:null}function go(t){let e=Number(C(v.coordinatorOffset(t)));return Number.isFinite(e)?e:0}function Ke(t){let e=Number(C(v.effectiveSetpoint(t)));if(Number.isFinite(e))return e;let o=cs(t);return o==null?null:Number((o+go(t)).toFixed(1))}function Et(t){return Math.min(xn,Math.max(hn,Number(Number(t).toFixed(1))))}function ds(t,e){if(!e)return d("common.disabled");let o=String(t||"IDLE").toUpperCase();return o==="HEATING"?d("state.heating"):o==="IDLE"?d("state.idle"):o==="OFF"?d("state.off"):o==="FAULT"?d("common.fault"):o==="MANUAL"?d("state.manual"):o==="OVERHEATED"?d("state.overheated"):o==="CALIBRATING"?d("state.calibrating"):o}var dc=O({tag:"zone-detail",state:t=>({zone:t.zone,temp:"---",setpoint:"---",valve:"---",state:"---"}),render:is,methods:{update(t,e){var p;let o=D("selectedZone"),a=String(L(v.state(o))||"").toUpperCase(),r=ce(v.enabled(o));this.zone=o,t.dataset.zone=String(o),document.activeElement!==e.setpoint&&(e.setpoint.value=vt(Ke(o))),e.base.textContent=ue((p=C(v.baseSetpoint(o)))!=null?p:C(v.setpoint(o)));let n=C(v.coordinatorOffset(o));e.offset.textContent=n==null?"---":(n>0?"+":"")+Number(n).toFixed(1)+"\xB0C",e.temp.textContent=ue(C(v.temp(o))),e.ret.textContent=ue(C("sensor-manifold_return_temperature")),e.valve.textContent=mt(C(v.valve(o)));let s=e.badge;s.textContent=ds(a,r);let c=r?a==="HEATING"?"badge-heating":a==="IDLE"?"badge-idle":a==="FAULT"?"badge-fault":"":"badge-disabled";s.className="zd-badge"+(c?" "+c:""),e.toggle.classList.toggle("on",r)},commitSetpoint(t){let e=this.zone,o=ls(t);if(o==null)return null;let a=Et(o-go(e));return Jt(e,a),Ke(e)},incSetpoint(){let t=this.zone,e=Ke(t),o=Et((e==null?20:e)+bo);Jt(t,Et(o-go(t)))},decSetpoint(){let t=this.zone,e=Ke(t),o=Et((e==null?20:e)-bo);Jt(t,Et(o-go(t)))},toggleEnabled(){let t=this.zone,e=ce(v.enabled(t));_a(t,!e)}},onMount(t,e){let o={setpoint:e.querySelector(".zd-setpoint"),temp:e.querySelector(".zd-temp"),base:e.querySelector(".zd-base"),offset:e.querySelector(".zd-offset"),ret:e.querySelector(".zd-ret"),valve:e.querySelector(".zd-valve"),badge:e.querySelector(".zd-badge"),toggle:e.querySelector(".btn-toggle"),inc:e.querySelector(".btn-inc"),dec:e.querySelector(".btn-dec")};o.inc.onclick=()=>t.incSetpoint(),o.dec.onclick=()=>t.decSetpoint(),o.toggle.onclick=()=>t.toggleEnabled();let a=()=>{let s=t.commitSetpoint(o.setpoint.value);o.setpoint.value=s!=null?vt(s):vt(Ke(t.zone))};o.setpoint.addEventListener("keydown",s=>{s.key==="Enter"?(s.preventDefault(),o.setpoint.blur()):s.key==="Escape"?(s.preventDefault(),o.setpoint.value=vt(Ke(t.zone)),o.setpoint.blur()):s.key==="ArrowUp"?(s.preventDefault(),t.incSetpoint(),o.setpoint.value=vt(Ke(t.zone))):s.key==="ArrowDown"&&(s.preventDefault(),t.decSetpoint(),o.setpoint.value=vt(Ke(t.zone)))}),o.setpoint.addEventListener("blur",a),o.setpoint.addEventListener("focus",()=>{requestAnimationFrame(()=>o.setpoint.select())});let r=()=>t.update(e,o),n=s=>{let c=D("selectedZone");(s===v.temp(c)||s===v.setpoint(c)||s===v.baseSetpoint(c)||s===v.effectiveSetpoint(c)||s===v.coordinatorOffset(c)||s===v.valve(c)||s===v.state(c)||s===v.enabled(c))&&r()};for(let s=1;s<=6;s++)k(v.temp(s),n),k(v.setpoint(s),n),k(v.baseSetpoint(s),n),k(v.effectiveSetpoint(s),n),k(v.coordinatorOffset(s),n),k(v.valve(s),n),k(v.state(s),n),k(v.enabled(s),n);k("sensor-manifold_return_temperature",r),U("selectedZone",r),E(e),r()}});var ps=`
...
`;I("zone-sensor-card",ps);var us=()=>`
  ...
  `;function ms(t){return t==="BLE"||t==="BLE Sensor"?"BLE Sensor":"Local Probe"}function gs(t){return t==="BLE Sensor"?"BLE":"Local Probe"}function yn(t,e){let o='<option value="Local Probe" data-i18n="zone.sensor.localProbe">'+d("zone.sensor.localProbe")+'</option><option value="BLE Sensor" data-i18n="zone.sensor.bleSource">'+d("zone.sensor.bleSource")+"</option>";t.innerHTML!==o&&(t.innerHTML=o),t.value=e}var xc=O({tag:"zone-sensor-card",render:us,onMount(t,e){let o=e.querySelector(".zs-source"),a=e.querySelector(".zs-ble"),r=e.querySelector(".zs-row-ble"),n=e.querySelector(".zs-scan"),s=e.querySelector(".zs-scan-list"),c=0;function p(){return D("selectedZone")}function u(){r.style.display=o.value==="BLE Sensor"?"":"none"}let l=xe(e);yn(o,"Local Probe"),l.select(o,{read:()=>ms(String(L(v.tempSource(p()))||"")),commit:b=>Ve(p(),"zone_temp_source",gs(b))});let m=l.text(a,{read:()=>L(v.ble(p()))||"",commit:b=>Ro(p(),"zone_ble_mac",b)});o.addEventListener("change",u);function g(){let b=p();c!==b?(c=b,s.style.display="none",l.discard()):l.refresh(),u()}function f(b){let w=p();(b===v.tempSource(w)||b===v.ble(w))&&(l.refresh(),u())}n.addEventListener("click",()=>{if(n.disabled)return;n.disabled=!0,n.textContent="\u2026",s.style.display="",s.innerHTML='<div class="scan-msg">'+d("zone.sensor.scanning")+"</div>";let b=new AbortController,w=setTimeout(()=>b.abort(),8e3);fetch("/api/hv6/v1/ble-scan",{cache:"no-store",signal:b.signal}).then(h=>{if(!h.ok)throw new Error("HTTP "+h.status);return h.json()}).then(h=>{if(clearTimeout(w),n.disabled=!1,n.textContent=d("zone.sensor.scan"),!h.ok||!h.sensors||h.sensors.length===0){s.innerHTML='<div class="scan-msg">'+d("zone.sensor.noSensors")+"</div>";return}let y=p(),T=(L(v.ble(y))||"").toUpperCase(),S=_=>String(_).replace(/[&<>"']/g,R=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[R]),M="";for(let _ of h.sensors){let R=_.mac.toUpperCase(),B=_.name?S(_.name):"",H=_.temp_c!=null?_.temp_c.toFixed(1)+"\xB0C":"\u2014",P=_.rssi!=null?_.rssi+" dBm":"",X=_.age_s<60?d("common.secondsAgo",{value:_.age_s}):d("common.minutesAgo",{value:Math.round(_.age_s/60)}),te="";R===T?te='<span class="ble-badge">'+d("zone.sensor.assignedThisZone")+"</span>":_.zone>0&&(te='<span class="ble-badge">'+d("zone.sensor.zoneBadge",{zone:_.zone})+"</span>");let $=B?`<div class="ble-mac">${B}</div><div class="ble-meta">${R}</div>`:`<div class="ble-mac">${R}</div>`;M+=`<div class="ble-scan-item">
              <div>
                ${$}
            ...
            </div>`}s.innerHTML=M,s.querySelectorAll(".btn-assign").forEach(_=>{_.addEventListener("click",()=>{a.value=_.dataset.mac,m.markDirty(),s.style.display="none"})})}).catch(h=>{clearTimeout(w),n.disabled=!1,n.textContent=d("zone.sensor.scan");let y=h&&h.name==="AbortError"?d("zone.sensor.scanTimeout"):d("zone.sensor.scanFailed");s.innerHTML='<div class="scan-msg">'+y+"</div>"})}),U("selectedZone",g);for(let b=1;b<=6;b++)k(v.tempSource(b),f),k(v.ble(b),f);E(e),g()}});var bs=".zone-coordination-card { height: 100%; }";I("zone-coordination-card",bs);var fs=()=>`
...
`;function wn(t,e){let o=t.value,a='<option value="None" data-i18n="common.none">'+d("common.none")+"</option>";for(let r=1;r<=6;r++)r!==e&&(a+='<option value="Zone '+r+'">'+d("common.zone")+" "+r+"</option>");t.innerHTML=a,t.value=o||"None"}var Mc=O({tag:"zone-coordination-card",render:fs,onMount(t,e){let o=e.querySelector(".zc-sync"),a=0;function r(){return D("selectedZone")}let n=xe(e);n.select(o,{read:()=>L(v.syncTo(r()))||"None",commit:p=>Ve(r(),"zone_sync_to",p)});function s(){let p=r();a!==p?(wn(o,p),a=p,n.discard()):n.refresh()}function c(p){let u=r();(p===v.syncTo(u)||/^select-zone_\d+_sync_to$/.test(p))&&n.refresh()}U("selectedZone",s);for(let p=1;p<=6;p++)k(v.syncTo(p),c);E(e),s()}});var vs=".zone-room-card { height: 100%; }";I("zone-room-card",vs);var hs=()=>`
...
`,Pc=O({tag:"zone-room-card",render:hs,onMount(t,e){let o=e.querySelector(".zr-friendly");function a(){return D("selectedZone")}let r=xe(e);r.text(o,{read:()=>kt(a())||"",commit:n=>Aa(a(),n)}),U("selectedZone",r.discard),U("zoneNames",r.refresh),E(e),r.refresh()}});var xs=`
...
`;I("zone-actuator-card",xs);function kn(t){return t!=null?Number(t).toFixed(2)+"x":"---"}function zn(t){return t!=null?Number(t).toFixed(0):"---"}function ys(t){return t!=null?Number(t).toFixed(2)+"C":"---"}var ws=()=>`
...
`,Vc=O({tag:"zone-actuator-card",render:ws,onMount(t,e){var p,u,l;let o=Number(D("selectedZone")||1),a={orip:e.querySelector(".za-orip"),crip:e.querySelector(".za-crip"),ofac:e.querySelector(".za-ofac"),cfac:e.querySelector(".za-cfac"),ph:e.querySelector(".za-ph"),fault:e.querySelector(".za-fault"),faultVal:e.querySelector(".za-fault-val"),faultBtn:e.querySelector(".recovery-fault-btn"),factorsBtn:e.querySelector(".recovery-factors-btn"),relearnBtn:e.querySelector(".recovery-relearn-btn"),status:e.querySelector(".za-status")};function r(){o=Number(D("selectedZone")||1),a.orip.textContent=zn(C(v.motorOpenRipples(o))),a.crip.textContent=zn(C(v.motorCloseRipples(o))),a.ofac.textContent=kn(C(v.motorOpenFactor(o))),a.cfac.textContent=kn(C(v.motorCloseFactor(o))),a.ph.textContent=ys(C(v.preheatAdvance(o)));let m=String(L(v.motorLastFault(o))||"").toUpperCase(),g=m&&m!=="NONE"&&m!=="OK";a.fault.hidden=!g,g&&(a.faultVal.textContent=m)}let n=null;function s(m,g){a.status.textContent=m,a.status.className="za-status show "+(g?"ok":"err"),clearTimeout(n),n=setTimeout(()=>{a.status.classList.remove("show")},4e3)}function c(m,g){let f=m(o);s(g,!0),f&&typeof f.then=="function"&&f.then(b=>{b&&b.ok===!1&&s(d("diagnostics.recovery.rejected"),!1)}).catch(()=>s(d("diagnostics.recovery.unreachable"),!1))}(p=a.faultBtn)==null||p.addEventListener("click",()=>{c(Da,"\u2713 "+d("diagnostics.recovery.faultSent",{zone:Se(o)}))}),(u=a.factorsBtn)==null||u.addEventListener("click",()=>{confirm(d("diagnostics.recovery.confirmFactors",{zone:Se(o)}))&&c(Ra,"\u2713 "+d("diagnostics.recovery.factorsReset",{zone:Se(o)}))}),(l=a.relearnBtn)==null||l.addEventListener("click",()=>{confirm(d("diagnostics.recovery.confirmRelearn",{zone:Se(o)}))&&c(Pa,"\u2713 "+d("diagnostics.recovery.relearnStarted",{zone:Se(o)}))}),U("selectedZone",r);for(let m=1;m<=6;m++)k(v.motorOpenRipples(m),r),k(v.motorCloseRipples(m),r),k(v.motorOpenFactor(m),r),k(v.motorCloseFactor(m),r),k(v.preheatAdvance(m),r),k(v.motorLastFault(m),r);E(e),r()}});var De=6,ks="var(--flow-disabled)",_n="var(--flow-unknown)",Cn="var(--accent)",Ft="var(--flow-return)",Go="var(--text-strong)",zs="var(--flow-disabled)",nt="var(--flow-label)",fo="var(--flow-disabled)",Ko="var(--flow-label)",Ln="var(--flow-label)",Sn="var(--flow-return)",Ss="#66BB6A",_s="#FF6361",ee={w:1160,h:372,boxX:440,boxY:26,boxW:280,boxH:90,srcY:116,fanY:168,zoneY:262,zoneXs:[92,286,480,674,868,1062],srcSpread:15,bgDstHW:28,srcHW:7},Z={w:760,h:424,boxX:26,boxY:148,boxW:168,boxH:92,srcX:196,endX:386,nameX:446,midY:190,zoneYs:[56,120,184,248,312,376],spread:10,bgDstHW:15,srcHW:4},Cs=`
.flow-wrap {
  width: 100%;
  border-radius: 8px;
  overflow: hidden;
...
`;I("flow-diagram",Cs);function Ls(t,e){let o=String(kt(t)||"").trim();if(!o)return"";let a=o.toUpperCase();return a.length>e?a.slice(0,Math.max(1,e-1))+"\u2026":a}function Ms(t){if(!t)return null;let e=String(t).match(/(\d+)/);if(!e)return null;let o=Number(e[1]);return Number.isFinite(o)&&o>=1&&o<=8?o:null}function As(t,e){return e?t==null||Number.isNaN(t)?_n:t>0?Cn:nt:ks}function Mn(t){let e=t==="desktop"?"0 1":"1 0",o=[];o.push("<defs>");for(let a=1;a<=De;a++)o.push('<linearGradient id="'+t+"-rg"+a+'" x1="0" y1="0" x2="'+e.split(" ")[0]+'" y2="'+e.split(" ")[1]+'">'),o.push('<stop id="'+t+"-rgs"+a+'" offset="0%" stop-color="var(--accent)" stop-opacity=".96"/>'),o.push('<stop id="'+t+"-rga"+a+'" offset="100%" stop-color="var(--accent)" stop-opacity=".7"/>'),o.push("</linearGradient>");return o.push("</defs>"),o.join("")}function Es(t){let e=ee.boxX+ee.boxW/2+(t-2.5)*ee.srcSpread,o=ee.zoneXs[t];return"M"+e.toFixed(1)+" "+ee.srcY+" C"+e.toFixed(1)+" "+ee.fanY+" "+o.toFixed(1)+" "+(ee.fanY+34)+" "+o.toFixed(1)+" "+(ee.zoneY-20)}function Fs(t){let e=Z.midY+(t-2.5)*Z.spread,o=Z.zoneYs[t],a=Z.endX-Z.srcX;return"M"+Z.srcX+" "+e.toFixed(1)+" C"+(Z.srcX+a*.34)+" "+e.toFixed(1)+" "+(Z.srcX+a*.7)+" "+o.toFixed(1)+" "+Z.endX+" "+o.toFixed(1)}function An(t,e,o){let a=ee.boxX+ee.boxW/2+(t-2.5)*ee.srcSpread,r=ee.srcY,n=ee.zoneXs[t],s=ee.zoneY-20,c=ee.fanY,p=ee.fanY+34;return"M"+(a-e).toFixed(1)+" "+r+" C"+(a-e).toFixed(1)+" "+c+" "+(n-o).toFixed(1)+" "+p+" "+(n-o).toFixed(1)+" "+s+" L"+(n+o).toFixed(1)+" "+s+" C"+(n+o).toFixed(1)+" "+p+" "+(a+e).toFixed(1)+" "+c+" "+(a+e).toFixed(1)+" "+r+"Z"}function En(t,e,o){let a=Z.midY+(t-2.5)*Z.spread,r=Z.zoneYs[t],n=Z.endX-Z.srcX,s=Z.srcX+n*.34,c=Z.srcX+n*.7;return"M"+Z.srcX+" "+(a-e).toFixed(1)+" C"+s+" "+(a-e).toFixed(1)+" "+c+" "+(r-o).toFixed(1)+" "+Z.endX+" "+(r-o).toFixed(1)+" L"+Z.endX+" "+(r+o).toFixed(1)+" C"+c+" "+(r+o).toFixed(1)+" "+s+" "+(a+e).toFixed(1)+" "+Z.srcX+" "+(a+e).toFixed(1)+"Z"}function Fn(t,e,o){return'<rect width="'+t+'" height="'+e+'" rx="10" fill="var(--surface-raised)"/>'}function Tn(t){let e=t==="desktop"?ee:Z,o=t==="desktop"?e.boxY+34:e.boxY+36,a=t==="desktop"?e.boxY+74:e.boxY+76;return'<rect x="'+e.boxX+'" y="'+e.boxY+'" width="'+e.boxW+'" height="'+e.boxH+'" rx="7" fill="var(--flow-source-bg)" stroke="var(--accent)" stroke-width="2"/><text id="'+t+'-fd-flow-label" x="'+(e.boxX+e.boxW/2)+'" y="'+o+'" text-anchor="middle" font-size="'+(t==="desktop"?28:27)+'" font-weight="800" fill="var(--accent)" letter-spacing="2">'+d("overview.flowDiagram.flow")+'</text><text id="'+t+'-fd-flow-temp" class="flow-metric" x="'+(e.boxX+e.boxW/2)+'" y="'+a+'" text-anchor="middle" font-size="'+(t==="desktop"?40:37)+'" fill="var(--text-strong)">---</text>'}function Ts(){let t=[],e=ee.w,o=ee.h,a=ee.zoneY-20;t.push('<svg class="flow-svg flow-svg-desktop" viewBox="0 5 '+e+" "+(o-5)+'" preserveAspectRatio="xMidYMid meet" aria-hidden="true" focusable="false">'),t.push(Mn("desktop")),t.push(Fn(e,o,"desktop")),t.push(Tn("desktop")),t.push('<text id="desktop-fd-ret-temp" x="'+(ee.boxX+ee.boxW+24)+'" y="'+(ee.boxY+28)+'" font-size="24" font-weight="800" fill="'+Ft+'" font-family="var(--mono)">'+d("overview.flowDiagram.returnShort")+" ---</text>"),t.push('<text id="desktop-fd-dt-label" x="'+(ee.boxX+ee.boxW+24)+'" y="'+(ee.boxY+54)+'" font-size="19" font-weight="800" fill="'+Ln+'" letter-spacing="1.4">'+d("overview.flowDiagram.dt")+"</text>"),t.push('<text id="desktop-fd-dt" x="'+(ee.boxX+ee.boxW+24)+'" y="'+(ee.boxY+86)+'" class="flow-metric" font-size="34" fill="var(--accent)">---</text>');for(let r=1;r<=De;r++)t.push('<path id="desktop-fd-track-'+r+'" class="flow-track" d="'+Es(r-1)+'" opacity=".7"/>');for(let r=1;r<=De;r++)t.push('<path id="desktop-fd-path-'+r+'" class="flow-ribbon" d="'+An(r-1,ee.srcHW,ee.bgDstHW)+'" fill="url(#desktop-rg'+r+')" opacity="1"/>');t.push('<line x1="54" y1="'+a+'" x2="'+(e-54)+'" y2="'+a+'" stroke="var(--flow-track)" stroke-width="2" opacity=".72"/>');for(let r=1;r<=De;r++){let n=ee.zoneXs[r-1];t.push('<g class="flow-zone-hit">'),t.push('<line id="desktop-fd-tick-'+r+'" x1="'+n+'" y1="'+(a-10)+'" x2="'+n+'" y2="'+(a+10)+'" stroke="var(--flow-track)" stroke-width="2"/>'),t.push('<text id="desktop-fd-zn'+r+'" x="'+n+'" y="'+(a-18)+'" text-anchor="middle" font-size="22" fill="'+Go+'" font-weight="800" letter-spacing="1.5">Z'+r+"</text>"),t.push('<text id="desktop-fd-zf'+r+'" x="'+n+'" y="'+(a+30)+'" text-anchor="middle" font-size="17.5" fill="'+nt+'" font-weight="700" letter-spacing=".35">---</text>'),t.push('<text id="desktop-fd-zsp'+r+'" x="'+n+'" y="'+(a+30)+'" text-anchor="middle" font-size="15.5" fill="'+fo+'" font-weight="600" font-family="var(--mono)"></text>'),t.push('<text id="desktop-fd-zt'+r+'" x="'+n+'" y="'+(a+60)+'" text-anchor="middle" class="flow-metric" font-size="24" fill="var(--text-strong)">---\xB0C</text>'),t.push('<text id="desktop-fd-zv'+r+'" x="'+(n-40)+'" y="'+(a+90)+'" text-anchor="middle" class="flow-metric" font-size="20" fill="'+nt+'">---%</text>'),t.push('<text id="desktop-fd-zr'+r+'" x="'+(n+40)+'" y="'+(a+90)+'" text-anchor="middle" class="flow-metric" font-size="20" fill="'+Ft+'">---</text>'),t.push("</g>")}return t.push("</svg>"),t.join("")}function Ns(){let t=[],e=Z.w,o=Z.h;t.push('<svg class="flow-svg flow-svg-mobile" viewBox="0 0 '+e+" "+o+'" preserveAspectRatio="xMidYMid meet" aria-hidden="true" focusable="false">'),t.push(Mn("mobile")),t.push(Fn(e,o,"mobile")),t.push(Tn("mobile"));for(let a=1;a<=De;a++)t.push('<path id="mobile-fd-track-'+a+'" class="flow-track" d="'+Fs(a-1)+'" opacity=".7"/>');for(let a=1;a<=De;a++)t.push('<path id="mobile-fd-path-'+a+'" class="flow-ribbon" d="'+En(a-1,Z.srcHW,Z.bgDstHW)+'" fill="url(#mobile-rg'+a+')" opacity="1"/>');t.push('<rect x="'+(Z.boxX+6)+'" y="'+(Z.boxY+Z.boxH+10)+'" width="'+(Z.boxW-12)+'" height="84" rx="8" fill="var(--flow-source-bg)" stroke="var(--flow-return)" stroke-opacity=".7"/>'),t.push('<text id="mobile-fd-ret-temp" x="'+(Z.boxX+Z.boxW/2)+'" y="'+(Z.boxY+Z.boxH+36)+'" text-anchor="middle" font-size="21" font-weight="800" fill="'+Ft+'" font-family="var(--mono)">'+d("overview.flowDiagram.returnShort")+" ---</text>"),t.push('<text id="mobile-fd-dt-label" x="'+(Z.boxX+Z.boxW/2)+'" y="'+(Z.boxY+Z.boxH+56)+'" text-anchor="middle" font-size="15.5" font-weight="800" fill="'+Ln+'" letter-spacing=".7">'+d("overview.flowDiagram.dt")+"</text>"),t.push('<text id="mobile-fd-dt" x="'+(Z.boxX+Z.boxW/2)+'" y="'+(Z.boxY+Z.boxH+82)+'" text-anchor="middle" class="flow-metric" font-size="27" fill="var(--accent)">---</text>'),t.push('<line x1="'+Z.endX+'" y1="38" x2="'+Z.endX+'" y2="'+(o-28)+'" stroke="var(--flow-track)" stroke-width="2" opacity=".72"/>'),t.push('<text id="mobile-fd-temp-head" x="506" y="34" font-size="17" fill="'+Ko+'" font-weight="700" letter-spacing="1">'+d("overview.graph.layers.temp").toUpperCase()+"</text>"),t.push('<text id="mobile-fd-flow-head" x="592" y="34" font-size="17" fill="'+Ko+'" font-weight="700" letter-spacing="1">'+d("overview.flowDiagram.flow")+"</text>"),t.push('<text id="mobile-fd-ret-head" x="678" y="34" font-size="17" fill="'+Ko+'" font-weight="700" letter-spacing="1">'+d("overview.flowDiagram.returnShort")+"</text>");for(let a=1;a<=De;a++){let r=Z.zoneYs[a-1];t.push('<line id="mobile-fd-tick-'+a+'" x1="'+(Z.endX-10)+'" y1="'+r+'" x2="'+(Z.endX+10)+'" y2="'+r+'" stroke="var(--flow-track)" stroke-width="2"/>'),t.push('<text id="mobile-fd-zn'+a+'" x="'+(Z.endX-14)+'" y="'+(r+7)+'" text-anchor="end" font-size="21" fill="'+Go+'" font-weight="800" letter-spacing="1.1">Z'+a+"</text>"),t.push('<text id="mobile-fd-zf'+a+'" x="'+Z.nameX+'" y="'+(r-12)+'" text-anchor="middle" font-size="17" fill="'+nt+'" font-weight="700" letter-spacing=".3">---</text>'),t.push('<text id="mobile-fd-zsp'+a+'" x="'+Z.nameX+'" y="'+(r+12)+'" text-anchor="middle" font-size="15.5" fill="'+fo+'" font-weight="600" font-family="var(--mono)"></text>'),t.push('<text id="mobile-fd-zt'+a+'" x="506" y="'+(r+7)+'" class="flow-metric" font-size="22" fill="var(--text-strong)">---\xB0C</text>'),t.push('<text id="mobile-fd-zv'+a+'" x="592" y="'+(r+7)+'" class="flow-metric" font-size="22" fill="'+nt+'">---%</text>'),t.push('<text id="mobile-fd-zr'+a+'" x="678" y="'+(r+7)+'" class="flow-metric" font-size="22" fill="'+Ft+'">---</text>')}return t.push("</svg>"),t.join("")}var Ds=()=>'<div class="flow-wrap" role="img" aria-label="'+d("overview.flowDiagram.flow")+'">'+Ts()+Ns()+"</div>";O({tag:"flow-diagram",render:Ds,onMount(t,e){let o=["desktop","mobile"],a={};o.forEach(u=>{a[u]={flowEl:e.querySelector("#"+u+"-fd-flow-temp"),flowLabelEl:e.querySelector("#"+u+"-fd-flow-label"),retEl:e.querySelector("#"+u+"-fd-ret-temp"),dtLabelEl:e.querySelector("#"+u+"-fd-dt-label"),dtEl:e.querySelector("#"+u+"-fd-dt"),zones:new Array(De+1)};for(let l=1;l<=De;l++)a[u].zones[l]={textTemp:e.querySelector("#"+u+"-fd-zt"+l),textSetpoint:e.querySelector("#"+u+"-fd-zsp"+l),textFlow:e.querySelector("#"+u+"-fd-zv"+l),textRet:e.querySelector("#"+u+"-fd-zr"+l),label:e.querySelector("#"+u+"-fd-zn"+l),friendly:e.querySelector("#"+u+"-fd-zf"+l),track:e.querySelector("#"+u+"-fd-track-"+l),tick:e.querySelector("#"+u+"-fd-tick-"+l),path:e.querySelector("#"+u+"-fd-path-"+l)}});function r(u,l){u&&(u.textContent=l)}function n(u,l,m,g,f){let b=a[u];r(b.flowLabelEl,d("overview.flowDiagram.flow")),r(b.flowEl,ue(l)),r(b.retEl,d("overview.flowDiagram.returnShort")+" "+ue(m)),r(b.dtLabelEl,d("overview.flowDiagram.dt")),r(b.dtEl,g==null?"---":g.toFixed(1)+"\xB0C"),b.dtEl&&b.dtEl.setAttribute("fill",f)}function s(){r(e.querySelector("#mobile-fd-temp-head"),d("overview.graph.layers.temp").toUpperCase()),r(e.querySelector("#mobile-fd-flow-head"),d("overview.flowDiagram.flow")),r(e.querySelector("#mobile-fd-ret-head"),d("overview.flowDiagram.returnShort"))}function c(u,l,m){let g=a[u].zones[l];if(!g)return;let{enabled:f,pct:b,temp:w,setpoint:h,valve:y,returnTemp:T,hasReturn:S}=m,M=Ls(l,u==="desktop"?12:11),_=ue(w),R=h!=null?ue(h):"";r(g.label,"Z"+l),r(g.friendly,u==="desktop"?(M||"---")+(R?" ("+R+")":""):M||"---"),r(g.textTemp,_),r(g.textSetpoint,u==="desktop"?"":R?"("+R+")":""),r(g.textFlow,mt(y)),r(g.textRet,S?ue(T):"---"),g.label.setAttribute("fill",f?Go:zs),g.friendly.setAttribute("fill",f?nt:fo),g.textSetpoint.setAttribute("fill",f?nt:fo),g.textFlow.setAttribute("fill",As(b,f)),g.textRet.setAttribute("fill",S&&f?Ft:_n);let B=f&&b!=null&&b>0;g.track.setAttribute("opacity",f?".78":".38"),g.track.setAttribute("stroke-dasharray",f?"none":"5 7"),g.tick.setAttribute("stroke",B?Cn:"var(--flow-track)"),g.tick.setAttribute("stroke-width",B?"3":"2");let H=g.path;if(!B)H.setAttribute("opacity","0");else{let P=u==="desktop"?ee:Z,X=Math.max(2.5,b*P.bgDstHW),te=Math.max(1.3,b*P.srcHW);H.setAttribute("d",u==="desktop"?An(l-1,te,X):En(l-1,te,X)),H.setAttribute("fill","url(#"+u+"-rg"+l+")"),H.setAttribute("opacity",".96")}}function p(){let u=C(i.flow),l=C(i.ret),m=u!=null&&l!=null?Number(u)-Number(l):null,g=m==null||m<3?Sn:m>8?_s:Ss;o.forEach(f=>n(f,u,l,m,g));for(let f=1;f<=De;f++){let b=C(v.temp(f)),w=C(v.setpoint(f)),h=C(v.valve(f)),y=ce(v.enabled(f)),T=String(L(v.tempSource(f))||"Local Probe"),S=Ms(L(v.probe(f))||""),M=S?C(v.probeTemp(S)):null,_=T!=="Local Probe"&&M!=null&&!Number.isNaN(Number(M)),R=h!=null?Math.max(0,Math.min(100,Number(h)))/100:null,B={enabled:y,pct:R,temp:b,setpoint:w,valve:h,returnTemp:M,hasReturn:_};o.forEach(H=>c(H,f,B))}}k(i.flow,p),k(i.ret,p),U("zoneNames",p);for(let u=1;u<=De;u++)k(v.temp(u),p),k(v.setpoint(u),p),k(v.valve(u),p),k(v.enabled(u),p),k(v.probe(u),p),k(v.tempSource(u),p);for(let u=1;u<=8;u++)k(v.probeTemp(u),p);s(),p()}});var Rs={1:{label:"E",color:"#ff6361"},2:{label:"W",color:"#ffd380"},3:{label:"I",color:"#79d17e"},4:{label:"C",color:"#7aa7ce"},5:{label:"D",color:"rgba(214,228,255,.7)"},6:{label:"V",color:"rgba(214,228,255,.5)"},7:{label:"VV",color:"rgba(214,228,255,.4)"}},Ps=`
.logs-view {
  background: var(--panel-bg-vibrant);
  border: 1px solid var(--panel-border);
  border-radius: 8px;
...
`;function Is(t){let e=Rs[t.level]||{label:"?",color:"var(--text-secondary)"},o=Nn(t.tag||""),a=Nn(t.msg||"");return'<div class="log-line"><span class="lv" style="color:'+e.color+'">'+e.label+'</span><span class="tag">'+o+'</span><span class="msg">'+a+"</span></div>"}function Nn(t){return String(t).replace(/[&<>]/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;"})[e])}var ad=O({tag:"logs-view",render:Os,onMount(t,e){let o=e.querySelector(".logs-stream"),a=e.querySelector(".pause-btn"),r=e.querySelector(".clear-btn"),n=e.querySelector(".download-btn"),s=!1;function c(){if(s)return;let p=Kt();if(!p||!p.length){o.innerHTML='<div class="logs-empty">'+d("logs.waiting")+"</div>";return}let u=o.scrollHeight-o.scrollTop-o.clientHeight<40;o.innerHTML=p.map(Is).join(""),u&&(o.scrollTop=o.scrollHeight)}a.addEventListener("click",()=>{s=!s,a.textContent=s?d("logs.resume"):d("logs.pause"),a.classList.toggle("on",s),s||c()}),r.addEventListener("click",()=>{da()}),n.addEventListener("click",()=>{n.disabled=!0,Ua().catch(p=>{console.error("[Logs] download failed:",p),window.alert(d("logs.downloadFailed"))}).finally(()=>{n.disabled=!1})}),U("deviceLog",c),E(e),c()}});var qs=`
.diag-i2c {
  background: var(--panel-bg-vibrant);
  border: 1px solid var(--panel-border);
  border-radius: 8px;
...
`,fd=O({tag:"diag-manual-badge",render:$s,onMount(t,e){let o=e.classList.contains("diag-manual-badge")?e:e.querySelector(".diag-manual-badge");function a(){let r=!!D("manualMode");o&&o.classList.toggle("on",r)}U("manualMode",a),E(e),a()}});var js=`
.diag-zone-motor {
  background: var(--panel-bg-vibrant);
  border: 1px solid var(--panel-border);
  border-radius: 8px;
...
`;I("diag-zone-motor",js);var Vs=t=>{let e=t.zone||D("selectedZone")||1,o="";for(let a=1;a<=6;a++)o+='<option value="'+a+'"'+(a===e?" selected":"")+">"+d("common.zone")+" "+a+"</option>";return`
  ...
  `},Sd=O({tag:"diag-zone-motor-card",render:Vs,onMount(t,e){let o=Number(t.zone||D("selectedZone")||1),a=!!D("manualMode"),r=e.querySelector(".manual-mode-toggle"),n=e.querySelector(".motor-gated"),s=e.querySelector(".motor-zone-select"),c=e.querySelector(".motor-target-input"),p=e.querySelector(".motor-open-btn"),u=e.querySelector(".motor-close-btn"),l=e.querySelector(".motor-stop-btn"),m=()=>{let w=s.value||String(o),h="";for(let y=1;y<=6;y++)h+='<option value="'+y+'">'+d("common.zone")+" "+y+"</option>";s.innerHTML=h,s.value=w};function g(w){a=!!w,r&&(r.classList.toggle("on",a),r.setAttribute("aria-checked",a?"true":"false")),n&&n.classList.toggle("locked",!a),[s,c,p,u,l].forEach(h=>{h&&(h.disabled=!a)})}function f(){let w=!a;if(g(w),w){Ct(!0);for(let h=1;h<=6;h++)Po(h)}else Ct(!1)}function b(){let w=C(v.motorTarget(o));c&&w!=null?c.value=Number(w).toFixed(0):c&&(c.value="0")}s==null||s.addEventListener("change",()=>{o=Number(s.value||1),b()}),r==null||r.addEventListener("click",f),r==null||r.addEventListener("keydown",w=>{w.key!==" "&&w.key!=="Enter"||(w.preventDefault(),f())});for(let w=1;w<=6;w++)k(v.motorTarget(w),b);b(),g(a),U("manualMode",()=>{g(!!D("manualMode"))}),E(e),c==null||c.addEventListener("change",w=>{if(!a)return;let h=w.target.value;Ea(o,h)}),p==null||p.addEventListener("click",()=>{a&&Qt(o,1e4)}),u==null||u.addEventListener("click",()=>{a&&eo(o,1e4)}),l==null||l.addEventListener("click",()=>{a&&Po(o)})}});var Tt={FREE_TRAVEL:0,CONTACT:1,UNDER_LOAD:2,STOPPING:3};function Nt(t){switch(Number(t)){case Tt.CONTACT:return"contact";case Tt.UNDER_LOAD:return"load";case Tt.STOPPING:return"stopping";default:return"free"}}function Yo(t){let e=t||[];for(let o=0;o<e.length;o++){let a=Number(e[o].stroke_phase)||0;if(a===Tt.CONTACT||a===Tt.UNDER_LOAD)return e[o]}return null}function Ge(t,e,o){if(e[o]==null)return null;let a=Number(t[e[o]]);return Number.isFinite(a)?a:null}function Dn(t){let e=String(t||"").split(/\r?\n/).filter(n=>n.trim());if(e.length<2)return[];let o=e[0].split(",").map(n=>n.trim()),a={};for(let n=0;n<o.length;n++)a[o[n]]=n;let r=[];for(let n=1;n<e.length;n++){let s=e[n].split(",");if(s.length<6)continue;let c=p=>Number(s[a[p]]);r.push({t_ms:c("t_ms")||0,motion_count:c("motion_count")||0,current_ma:c("current_ma"),adc_current_raw:Ge(s,a,"adc_current_raw"),drive_on:c("drive_on")===1,direction_open:c("direction_open")===1,armed:c("armed")===1,stroke_phase:c("stroke_phase")||0,tacho_period_us:Ge(s,a,"tacho_period_us"),tacho_amp_raw:Ge(s,a,"tacho_amp_raw"),bemf_raw_a:Ge(s,a,"bemf_raw_a"),bemf_raw_b:Ge(s,a,"bemf_raw_b"),bemf_differential_raw:Ge(s,a,"bemf_differential_raw"),bemf_separation_us:Ge(s,a,"bemf_separation_us"),bemf_valid:a.bemf_valid!=null?c("bemf_valid")===1:null,bemf_moving:a.bemf_moving!=null?c("bemf_moving")===1:null,invalid_bemf_samples:Ge(s,a,"invalid_bemf_samples")})}return r}function Zs(t){let e=0,o=0;for(let a=0;a<t.length;a++)t[a].drive_on&&(t[a].direction_open?e+=1:o+=1);return e>=o?"open":"close"}function Us(t,e){if(!t.length)return null;let o=t.slice().sort((r,n)=>r-n),a=Math.min(o.length-1,Math.max(0,Math.round((o.length-1)*e)));return o[a]}function Ee(t){return Math.round(t*10)/10}function Xo(t,e,o){return Math.min(o,Math.max(e,t))}function Ws(t){let e=Number(t);return!Number.isFinite(e)||e<=0?null:1e6/e}function Rn(t){let e=[];if(!t.length)return e;let o=t[0].t_ms,a=t[t.length-1].t_ms;for(let r=o;r+500<=a;r+=500){let n=t.reduce((p,u)=>Math.abs(u.t_ms-r)<Math.abs(p.t_ms-r)?u:p,t[0]),s=t.reduce((p,u)=>Math.abs(u.t_ms-(r+500))<Math.abs(p.t_ms-(r+500))?u:p,t[0]),c=(s.t_ms-n.t_ms)/1e3;c>.2&&e.push({t_ms:r+500,slope:(s.current_ma-n.current_ma)/c})}return e}function Dt(t){let e=t||[],o=[];for(let s=0;s<e.length;s++){let c=e[s],p=c.tacho_period_us!=null?c.tacho_period_us:c.tacho_cadence_us!=null?c.tacho_cadence_us:null;o.push({t_ms:c.t_ms,period_us:p,rate_hz:Ws(p)})}let a=e.filter(s=>s.drive_on&&Number.isFinite(s.current_ma)),r=a.length>=2?a:e.filter(s=>Number.isFinite(s.current_ma)),n=Rn(r);return{cadence:o,slopes:n,count:e.length,truncated:e.length>=2e3,window_ms:2e3*2}}function Pn(t,e){let o=e||Zs(t),a=t.filter(A=>A.drive_on&&Number.isFinite(A.current_ma)&&(o==="open"?A.direction_open:!A.direction_open));if(a.length<8)return{direction:o,ok:!1,reason:"too_few_samples"};let r=a[0].t_ms,n=a[a.length-1].t_ms,s=a.filter(A=>A.t_ms>=r+650),c=s.length>12?s:a,p=Math.max(1,n-(c[0]?c[0].t_ms:r)),u=c.filter(A=>A.t_ms<c[0].t_ms+p*.7),l=c.filter(A=>A.t_ms>=c[0].t_ms+p*.8),m=(u.length?u:c).map(A=>A.current_ma),g=(l.length?l:c.slice(-Math.max(4,c.length/8|0))).map(A=>A.current_ma),f=m.reduce((A,z)=>A+z,0)/m.length,b=Math.max(...a.map(A=>A.current_ma)),w=Math.max(...g),h=Math.max(0,a[a.length-1].motion_count-a[0].motion_count),y=Rn(c),T=y.filter(A=>A.t_ms<c[0].t_ms+p*.7).map(A=>A.slope),S=y.filter(A=>A.t_ms>=c[0].t_ms+p*.75).map(A=>A.slope),M=S.length?Math.max(...S):0,_=Us(T.map(Math.abs),.9)||0,R=o==="close"?.55:.68,B=f>.5?w/f:0,H=Ee(Xo(1+R*Math.max(0,B-1),1.25,2.4)),P=Ee(Xo(Math.max(_*2.2,M*.42,.4),.4,8)),X=Ee(Xo(1+.35*Math.max(0,B-1),1.15,1.8)),te=o==="open"?1.15:null,$=Yo(a);return{direction:o,ok:!0,start_ms:r,end_ms:n,runtime_ms:n-r,mean_ma:Ee(f),peak_ma:Ee(b),stall_peak_ma:Ee(w),ripples:h,max_stall_slope_ma_s:Ee(M),travel_slope_ma_s:Ee(_),measured_factor:Ee(B),suggested_factor:H,suggested_slope:P,suggested_slope_floor:X,suggested_ripple_limit:te,pin_seen:!!$,pin_t_ms:$?$.t_ms:null,pin_motion_count:$?$.motion_count:null,pin_current_ma:$?Ee($.current_ma):null,samples:a}}function On(t,e){if(!t||!t.ok)return[];let o=t.mean_ma,a=Number(e&&e.factor)||t.suggested_factor;return[{id:"mean",value:o},{id:"threshold",value:Ee(o*a)},{id:"suggested",value:Ee(o*t.suggested_factor)},{id:"cap",value:100}]}var Rt=920,Pt=200,In=56,re={t:16,r:18,b:32,l:52},Re=Rt-re.l-re.r,xt=Pt-re.t-re.b,Jo="var(--accent)",ta="var(--series-cool)",Ks="var(--state-warn)",Gs="var(--state-ok)",Xs="var(--state-danger)",vo="var(--state-warn)",Qo="var(--series-cool)",ea="var(--accent)",Ys="var(--state-ok)",qn={free:"rgba(var(--accent-rgb),.10)",contact:"rgba(245,158,11,.22)",load:"rgba(52,211,153,.20)",stopping:"rgba(239,68,68,.18)"},Hn={free:"",contact:"lab-hatch-contact",load:"lab-hatch-load",stopping:"lab-hatch-stop"},Js=`
...
`;I("motor-lab-charts",Js);function ht(t,e,o=xt){let a=e.max-e.min||1;return re.t+o-(t-e.min)/a*o}function ho(t,e,o,a=Pt){t.appendChild(G("text",{class:"chart-axis-label",x:re.l,y:a-8},"0 s")),t.appendChild(G("text",{class:"chart-axis-label",x:re.l+Re-48,y:a-8},(o/1e3).toFixed(1)+" s"))}function oa(t,e,o=xt){for(let a=0;a<=4;a++){let r=re.t+o*a/4;t.appendChild(G("line",{class:"chart-grid",x1:re.l,x2:re.l+Re,y1:r,y2:r}));let n=e.max-(e.max-e.min)*a/4;t.appendChild(G("text",{class:"chart-tick",x:8,y:r+4},n.toFixed(0)))}}function Qs(t){if(!t.length)return[];let e=[],o=0,a=Number(t[0].stroke_phase)||0;for(let r=1;r<t.length;r++){let n=Number(t[r].stroke_phase)||0;n!==a&&(e.push({phase:a,start:o,end:r-1}),o=r,a=n)}return e.push({phase:a,start:o,end:t.length-1}),e}function ei(t){let e=t.querySelector("defs");return e||(e=G("defs"),[["lab-hatch-contact","M0 4 L4 0","var(--state-warn)"],["lab-hatch-load","M0 0 L4 4","var(--state-ok)"],["lab-hatch-stop","M0 2 L4 2","var(--state-danger)"]].forEach(([a,r,n])=>{let s=G("pattern",{id:a,width:4,height:4,patternUnits:"userSpaceOnUse"});s.appendChild(G("path",{d:r,stroke:n,"stroke-width":"1",fill:"none"})),e.appendChild(s)}),t.appendChild(e),e)}function Ot(t,e,o){let a=document.createElement("div");a.className="chart-card",a.setAttribute("role","img"),a.setAttribute("aria-label",d(o||t));let r=document.createElement("div");return r.className="chart-head",r.innerHTML='<span class="chart-title">'+d(t)+'</span><span class="chart-sub">'+e+"</span>",a.appendChild(r),a}function ti(t,e,o){let a=Ot(e,d(o||"diagnostics.lab.empty"),e),r=document.createElement("div");r.className="lab-empty",r.textContent=d("diagnostics.lab.empty"),a.appendChild(r),t.appendChild(a)}function xo(t,e,o){return t?d("diagnostics.lab.res.live"):o&&o.truncated?d("diagnostics.lab.res.traceTruncated",{n:2e3}):e&&e.ok?d("diagnostics.lab.res.trace",{direction:d("diagnostics.lab.dir."+e.direction),ms:e.runtime_ms}):d("diagnostics.lab.res.traceReady")}function oi(t,e,o,a,r,n){if(!n.current&&!n.overlays)return;let s=Dt(e),c=Ot("diagnostics.lab.chart.current",xo(r,o,s),"diagnostics.lab.chart.currentAria");if(s.truncated){let y=document.createElement("div");y.className="lab-chart-warn",y.textContent=d("diagnostics.lab.res.ringWarn",{n:2e3,s:s.window_ms/1e3}),c.appendChild(y)}if(!e.length){let y=document.createElement("div");y.className="lab-empty",y.textContent=d("diagnostics.lab.empty"),c.appendChild(y),t.appendChild(c);return}let p=e.map(y=>y.current_ma).filter(Number.isFinite),u=(a||[]).map(y=>y.value).filter(Number.isFinite),l=ro(p.concat([0,40],u),0,50);l.min=0;let m=e[0].t_ms,g=Math.max(1,e[e.length-1].t_ms-m),f=y=>re.l+(e[y].t_ms-m)/g*Re,b=e.map((y,T)=>({x:f(T),y:ht(y.current_ma,l)})),w=G("svg",{viewBox:"0 0 "+Rt+" "+Pt,role:"img"});if(oa(w,l),ho(w,m,g),n.overlays){let y={mean:ta,threshold:Ks,suggested:Gs,cap:Xs};(a||[]).forEach(T=>{let S=ht(T.value,l);w.appendChild(G("line",{x1:re.l,x2:re.l+Re,y1:S,y2:S,stroke:y[T.id],"stroke-dasharray":T.id==="mean"?"0":"5 4","stroke-width":T.id==="mean"?"1.4":"1.2","vector-effect":"non-scaling-stroke",opacity:T.id==="cap"?".45":".9"}))})}let h=o&&o.ok&&o.pin_seen?{t_ms:o.pin_t_ms,current_ma:o.pin_current_ma,motion_count:o.pin_motion_count,stroke_phase:1}:Yo(e);if(h&&Number.isFinite(h.t_ms)){let y=re.l+(h.t_ms-m)/g*Re;w.appendChild(G("line",{x1:y,x2:y,y1:re.t,y2:re.t+xt,stroke:vo,"stroke-dasharray":"3 4","stroke-width":"1.4","vector-effect":"non-scaling-stroke",opacity:".95"})),w.appendChild(G("circle",{cx:y,cy:ht(Number(h.current_ma)||0,l),r:4.2,fill:vo,stroke:"var(--bg)","stroke-width":"1.5"})),w.appendChild(G("text",{class:"chart-tick",x:Math.min(y+6,re.l+Re-64),y:re.t+12,fill:vo},d("diagnostics.lab.pinMark")))}n.current&&w.appendChild(G("path",{d:gt(b),fill:"none",stroke:Jo,"stroke-width":"2.2","vector-effect":"non-scaling-stroke"})),c.appendChild(w),t.appendChild(c),bt(w,c,{count:e.length,plotTop:re.t,plotBottom:re.t+xt,xAt:f,label:y=>((e[y].t_ms-m)/1e3).toFixed(2)+" s",dots:y=>n.current?[{y:b[y].y,color:Jo}]:[],rows:y=>[{color:Jo,label:d("diagnostics.lab.currentMa"),value:Number(e[y].current_ma).toFixed(1)+" mA"},{color:ta,label:d("diagnostics.lab.motion"),value:String(e[y].motion_count)},{color:vo,label:d("diagnostics.lab.stroke"),value:d("diagnostics.lab.stroke."+Nt(e[y].stroke_phase))}]})}function ai(t,e,o,a){if(!a.phase)return;let r=Dt(e),n=Ot("diagnostics.lab.chart.phase",xo(o,null,r),"diagnostics.lab.chart.phaseAria");if(!e.length){let g=document.createElement("div");g.className="lab-empty",g.textContent=d("diagnostics.lab.empty"),n.appendChild(g),t.appendChild(n);return}let s=e[0].t_ms,c=Math.max(1,e[e.length-1].t_ms-s),p=In+re.b,u=G("svg",{viewBox:"0 0 "+Rt+" "+p,class:"lab-phase-strip",role:"img"});ei(u);let l=10,m=In-18;Qs(e).forEach(g=>{let f=re.l+(e[g.start].t_ms-s)/c*Re,b=re.l+(e[g.end].t_ms-s)/c*Re,w=Nt(g.phase),h=Math.max(2,b-f);u.appendChild(G("rect",{x:f,y:l,width:h,height:m,fill:qn[w]||qn.free,stroke:"var(--separator)","stroke-width":"1"})),Hn[w]&&u.appendChild(G("rect",{x:f,y:l,width:h,height:m,fill:"url(#"+Hn[w]+")",opacity:".55"})),h>54&&u.appendChild(G("text",{x:f+6,y:l+m/2+3},d("diagnostics.lab.stroke."+w))),u.appendChild(G("line",{x1:b,x2:b,y1:l,y2:l+m,stroke:"var(--text-muted)","stroke-width":"1",opacity:".55"}))}),ho(u,s,c,p),n.appendChild(u),t.appendChild(n)}function ni(t,e,o,a){if(!a.cadence)return;let r=Dt(e),n=Ot("diagnostics.lab.chart.cadence",xo(o,null,r),"diagnostics.lab.chart.cadenceAria"),s=r.cadence.map(f=>f.rate_hz).filter(f=>f!=null&&Number.isFinite(f));if(!s.length){let f=document.createElement("div");f.className="lab-empty",f.textContent=d("diagnostics.lab.chart.cadenceEmpty"),n.appendChild(f),t.appendChild(n);return}let c=ro(s.concat([0]),0,Math.max(10,...s));c.min=0;let p=e[0].t_ms,u=Math.max(1,e[e.length-1].t_ms-p),l=[],m=[];r.cadence.forEach((f,b)=>{f.rate_hz==null||!Number.isFinite(f.rate_hz)||(l.push({x:re.l+(f.t_ms-p)/u*Re,y:ht(f.rate_hz,c)}),m.push(b))});let g=G("svg",{viewBox:"0 0 "+Rt+" "+Pt,role:"img"});oa(g,c),ho(g,p,u),g.appendChild(G("path",{d:gt(l),fill:"none",stroke:Qo,"stroke-width":"2","vector-effect":"non-scaling-stroke"})),n.appendChild(g),t.appendChild(n),bt(g,n,{count:l.length,plotTop:re.t,plotBottom:re.t+xt,xAt:f=>l[f].x,label:f=>((r.cadence[m[f]].t_ms-p)/1e3).toFixed(2)+" s",dots:f=>[{y:l[f].y,color:Qo}],rows:f=>{let b=r.cadence[m[f]];return[{color:Qo,label:d("diagnostics.lab.cadence"),value:b.rate_hz.toFixed(1)+" /s"},{color:ta,label:d("diagnostics.lab.tachoPeriod"),value:Math.round(b.period_us)+" \xB5s"}]}})}function ri(t,e,o,a,r){if(!r.slope)return;let n=Dt(e),s=Ot("diagnostics.lab.chart.slope",xo(a,o,n),"diagnostics.lab.chart.slopeAria");if(!n.slopes.length){let b=document.createElement("div");b.className="lab-empty",b.textContent=d("diagnostics.lab.chart.slopeEmpty"),s.appendChild(b),t.appendChild(s);return}let c=n.slopes.map(b=>b.slope),p=o&&o.ok?o.suggested_slope:null,u=ro(c.concat(p!=null?[p,0]:[0]),-2,8),l=e[0].t_ms,m=Math.max(1,e[e.length-1].t_ms-l),g=n.slopes.map(b=>({x:re.l+(b.t_ms-l)/m*Re,y:ht(b.slope,u)})),f=G("svg",{viewBox:"0 0 "+Rt+" "+Pt,role:"img"});if(oa(f,u),ho(f,l,m),p!=null){let b=ht(p,u);f.appendChild(G("line",{x1:re.l,x2:re.l+Re,y1:b,y2:b,stroke:Ys,"stroke-dasharray":"5 4","stroke-width":"1.3","vector-effect":"non-scaling-stroke"}))}f.appendChild(G("path",{d:gt(g),fill:"none",stroke:ea,"stroke-width":"2","vector-effect":"non-scaling-stroke"})),s.appendChild(f),t.appendChild(s),bt(f,s,{count:g.length,plotTop:re.t,plotBottom:re.t+xt,xAt:b=>g[b].x,label:b=>((n.slopes[b].t_ms-l)/1e3).toFixed(2)+" s",dots:b=>[{y:g[b].y,color:ea}],rows:b=>[{color:ea,label:d("diagnostics.lab.slope"),value:n.slopes[b].slope.toFixed(2)+" mA/s"}]})}function $n(t,e){let o=e&&e.samples||[],a=e&&e.analysis,r=!!(e&&e.live),n=e&&e.overlays,s=Object.assign({current:!0,overlays:!0,phase:!0,cadence:!0,slope:!0},e&&e.visible);t.innerHTML="";let c=document.createElement("div");c.className="motor-lab-charts";let p=document.createElement("div");return p.className="gw-controls",p.setAttribute("role","toolbar"),p.setAttribute("aria-label",d("diagnostics.lab.chart.layers")),[["current","diagnostics.lab.chart.layer.current"],["overlays","diagnostics.lab.chart.layer.overlays"],["phase","diagnostics.lab.chart.layer.phase"],["cadence","diagnostics.lab.chart.layer.cadence"],["slope","diagnostics.lab.chart.layer.slope"]].forEach(([l,m])=>{let g=document.createElement("button");g.type="button",g.className="gw-toggle"+(s[l]?"":" is-off"),g.dataset.layer=l,g.setAttribute("aria-pressed",s[l]?"true":"false"),g.textContent=d(m),p.appendChild(g)}),c.appendChild(p),o.length?(oi(c,o,a,n,r,s),ai(c,o,r,s),ni(c,o,r,s),ri(c,o,a,r,s)):ti(c,"diagnostics.lab.chart.current","diagnostics.lab.chartLive"),t.appendChild(c),p}var si=250,jn=2e4,It=["setup","arm","seat","open","close","review"],ii=`
...
`;I("settings-manifold-card",pi);var ui=()=>{let t="";for(let o=1;o<=8;o++)t+="<option>Probe "+o+"</option>";let e="";for(let o=1;o<=8;o++)e+='<div class="probe-cell"><div class="probe-name">Probe '+o+'</div><div class="probe-temp" data-probe="'+o+'">---</div></div>';return`
  ...
  `},np=O({tag:"settings-manifold-card",render:ui,onMount(t,e){let o=e.querySelector(".sm-type"),a=e.querySelector(".sm-flow"),r=e.querySelector(".sm-ret"),n=xe(e);n.select(o,{read:()=>L(i.manifoldType)||"NO (Normally Open)",commit:c=>Ae("manifold_type",c)}),n.select(a,{read:()=>L(i.manifoldFlowProbe)||"Probe 7",commit:c=>Ae("manifold_flow_probe",c)}),n.select(r,{read:()=>L(i.manifoldReturnProbe)||"Probe 8",commit:c=>Ae("manifold_return_probe",c)});function s(){for(let c=1;c<=8;c++){let p=e.querySelector('[data-probe="'+c+'"]');p&&(p.textContent=ue(C(v.probeTemp(c))))}}k(i.manifoldType,n.refresh),k(i.manifoldFlowProbe,n.refresh),k(i.manifoldReturnProbe,n.refresh);for(let c=1;c<=8;c++)k(v.probeTemp(c),s);E(e),n.refresh(),s()}});var mi=`
...
`,xp=O({tag:"settings-minimum-flow-card",render:bi,onMount(t,e){let o=e.querySelector(".smf-always"),a=e.querySelector(".smf-pct"),r=e.querySelector(".smf-pct-row"),n=xe(e),s=c=>{r.hidden=!c,r.setAttribute("aria-hidden",c?"false":"true"),a.disabled=!c};n.toggle(o,{read:()=>ce(i.minimumFlowAlways),onChange:s,commit:c=>{let p=c?"on":"off";x(i.minimumFlowAlways,{state:p}),Ae("minimum_flow_always",p).catch(()=>x(i.minimumFlowAlways,{state:c?"off":"on"}))}}),n.num(a,{read:()=>C(i.minZoneFlowPct),commit:c=>{x(i.minZoneFlowPct,{value:c}),Me("min_zone_flow_pct",c)}}),k(i.minimumFlowAlways,n.refresh),k(i.minZoneFlowPct,n.refresh),E(e),n.refresh()}});var fi=`
...
`;I("settings-return-temp-card",fi);function rt(t){return!!(t&&t!=="None")}function vi(t){return"Probe "+t}function hi(){for(let t=1;t<=6;t++)if(rt(L(v.probe(t))))return!0;return!1}function xi(){let t="";for(let e=1;e<=8;e++)t+='<option value="Probe '+e+'">Probe '+e+"</option>";return t}var yi=()=>{let t=xi(),e="";for(let o=1;o<=6;o++)e+=`
  ...
  `},Mp=O({tag:"settings-return-temp-card",render:yi,onMount(t,e){let o=e.querySelector(".srt-enabled"),a=e.querySelector(".srt-zones"),r=Array.from(e.querySelectorAll(".srt-probe")),n=Object.create(null);function s(m){return n[m]||vi(m)}function c(m){a.hidden=!m,a.setAttribute("aria-hidden",m?"false":"true");for(let g of r)g.disabled=!m}function p(){for(let m=1;m<=6;m++){let g=e.querySelector('[data-zone-label="'+m+'"]');g&&(g.innerHTML=Oe(m))}}let u=xe(e),l;for(let m of r){let g=Number(m.dataset.zone);u.select(m,{read:()=>{let f=L(v.probe(g));return rt(f)?(n[g]=f,f):s(g)},commit:f=>{!l||!l.staged||(n[g]=f,Ve(g,"zone_probe",f))}})}l=u.toggle(o,{read:()=>hi(),onChange:m=>{if(m)for(let g of r){let f=Number(g.dataset.zone);rt(g.value)||(g.value=s(f)),rt(g.value)&&(n[f]=g.value)}else for(let g of r){let f=Number(g.dataset.zone);rt(g.value)&&(n[f]=g.value)}c(m)},commit:m=>{if(!m){for(let g of r){let f=Number(g.dataset.zone);rt(g.value)&&(n[f]=g.value),Ve(f,"zone_probe","None")}return}for(let g of r){let f=Number(g.dataset.zone),b=rt(g.value)?g.value:s(f);n[f]=b,Ve(f,"zone_probe",b)}}});for(let m=1;m<=6;m++)k(v.probe(m),u.refresh),k(v.name(m),p);E(e),p(),u.refresh()}});var wi=[{value:"15",labelKey:"settings.bleClock.interval15"},{value:"60",labelKey:"settings.bleClock.interval60"},{value:"360",labelKey:"settings.bleClock.interval360"},{value:"1440",labelKey:"settings.bleClock.interval1440"}];function ki(){if(String(L(i.bleClockSyncAdvertising)||"").toLowerCase()==="on")return d("common.clockSyncing");let t=String(L(i.bleClockSyncLastError)||"").trim();if(t==="clock_invalid")return d("settings.bleClock.waitingClock");if(t==="ble_busy")return d("settings.bleClock.busy");if(t)return t;let e=Number(C(i.bleClockSyncLastOkS)||0);if(!e)return d("settings.bleClock.never");let o=Math.max(0,Math.round(Date.now()/1e3)-e);if(o<60)return d("common.secondsAgo",{value:o});if(o<3600)return d("common.minutesAgo",{value:Math.round(o/60)});let a=Math.round(o/3600);return d("settings.bleClock.hoursAgo",{value:a})}var zi=()=>`
...
`;I("settings-motor-calibration-card",Ci);var yo=[{cls:"safe-runtime",key:"generic_runtime_limit_seconds",id:i.genericRuntimeLimitSeconds,labelKey:"settings.motor.maxSafeRuntime",unit:"s"},{cls:"close-threshold",key:"close_threshold_multiplier",id:i.closeThresholdMultiplier,labelKey:"settings.motor.closeThreshold",unit:"x"},{cls:"close-slope-threshold",key:"close_slope_threshold",id:i.closeSlopeThreshold,labelKey:"settings.motor.closeSlope",unit:"mA/s"},{cls:"close-slope-floor",key:"close_slope_current_factor",id:i.closeSlopeCurrentFactor,labelKey:"settings.motor.closeSlopeFloor",unit:"x"},{cls:"open-threshold",key:"open_threshold_multiplier",id:i.openThresholdMultiplier,labelKey:"settings.motor.openThreshold",unit:"x"},{cls:"open-slope-threshold",key:"open_slope_threshold",id:i.openSlopeThreshold,labelKey:"settings.motor.openSlope",unit:"mA/s"},{cls:"open-slope-floor",key:"open_slope_current_factor",id:i.openSlopeCurrentFactor,labelKey:"settings.motor.openSlopeFloor",unit:"x"},{cls:"open-ripple-limit",key:"open_ripple_limit_factor",id:i.openRippleLimitFactor,labelKey:"settings.motor.openRippleLimit",unit:"x"},{cls:"relearn-movements",key:"relearn_after_movements",id:i.relearnAfterMovements,labelKey:"settings.motor.relearnMovements",unit:"count"},{cls:"relearn-hours",key:"relearn_after_hours",id:i.relearnAfterHours,labelKey:"settings.motor.relearnHours",unit:"h"},{cls:"learn-min-samples",key:"learned_factor_min_samples",id:i.learnedFactorMinSamples,labelKey:"settings.motor.learnMinSamples",unit:"count"},{cls:"learn-max-deviation",key:"learned_factor_max_deviation_pct",id:i.learnedFactorMaxDeviationPct,labelKey:"settings.motor.learnMaxDeviation",unit:"%"}],Li=()=>{let t="";for(let e=0;e<yo.length;e++){let o=yo[e];if(o.key==="generic_runtime_limit_seconds")continue;let a=Mi(o.key)?"1":"0.1";t+='<div class="ui-row"><span class="ui-label"><span data-i18n="'+o.labelKey+'">'+d(o.labelKey)+"</span> ("+o.unit+')</span><span class="ui-field"><input type="number" class="ui-input smc-'+o.cls+'" value="0" step="'+a+'"></span></div>'}return`
  ...
  `};function Mi(t){return t==="learned_factor_min_samples"||t==="generic_runtime_limit_seconds"||t==="relearn_after_movements"||t==="relearn_after_hours"}var Yp=O({tag:"settings-motor-calibration-card",render:Li,onMount(t,e){let o=e.querySelector(".smc-profile"),a=e.querySelector(".smc-safe-runtime"),r=e.querySelector(".mc-drivers-toggle"),n=xe(e);function s(p){if(p==="HmIP VdMot"&&Me("hmip_runtime_limit_seconds",40),p==="Generic"){let u=Number(C(i.genericRuntimeLimitSeconds));(!Number.isFinite(u)||u<=0)&&Me("generic_runtime_limit_seconds",45)}}n.toggle(r,{read:()=>ce(i.drivers),commit:p=>_t(p)}),n.select(o,{read:()=>L(i.motorProfileDefault)||"HmIP VdMot",commit:p=>{Ae("motor_profile_default",p),s(p)}});function c(){let p=L(i.motorProfileDefault)||"HmIP VdMot";a.disabled=p==="HmIP VdMot"}n.num(a,{read:()=>(L(i.motorProfileDefault)||"HmIP VdMot")==="HmIP VdMot"?40:C(i.genericRuntimeLimitSeconds),commit:p=>{o.value==="Generic"&&Me("generic_runtime_limit_seconds",p)}});for(let p=0;p<yo.length;p++){let u=yo[p];if(u.key==="generic_runtime_limit_seconds")continue;let l=e.querySelector(".smc-"+u.cls);l&&(n.num(l,{read:()=>C(u.id),commit:m=>Me(u.key,m)}),k(u.id,n.refresh))}k(i.drivers,n.refresh),k(i.motorProfileDefault,()=>{n.refresh(),c()}),k(i.genericRuntimeLimitSeconds,n.refresh),k(i.hmipRuntimeLimitSeconds,n.refresh),E(e),s(L(i.motorProfileDefault)||"HmIP VdMot"),n.refresh(),c()}});var Ai=600*1e3,Zn=600,Ei=`
...
`;function Un(t){let e=String(t||"").trim().replace(/^v/i,"").match(/^(\d+)\.(\d+)\.(\d+)/);return e?[Number(e[1]),Number(e[2]),Number(e[3])]:null}function Ht(t,e){let o=Un(t);if(!o)return!1;let a=Un(e);if(!a)return!0;for(let r=0;r<3;r++)if(o[r]!==a[r])return o[r]>a[r];return!1}function Wn(t){return String(t).replace(/[&<>]/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;"})[e])}function Ti(t){let e=String(t||"").trim();return e.length<=Zn?e:e.slice(0,Zn).replace(/\s+\S*$/,"")+"\u2026"}function Ni(){let t=document.querySelector(".settings-backup-card");if(!t)return;let e=t.closest("details");e&&(e.open=!0),t.scrollIntoView({behavior:"smooth",block:"center"});let o=t.querySelector(".sbk-save");o&&o.focus({preventScroll:!0})}var su=O({tag:"settings-firmware-card",render:Fi,onMount(t,e){let o=e.querySelector(".sfw-version"),a=e.querySelector(".sfw-status"),r=e.querySelector(".sfw-check"),n=e.querySelector(".sfw-banner"),s=e.querySelector(".sfw-hop"),c=e.querySelector(".sfw-notes"),p=e.querySelector(".sfw-install"),u=e.querySelector(".sfw-asset"),l=e.querySelector(".sfw-jump"),m=e.querySelector(".sfw-file"),g=e.querySelector(".sfw-choose"),f=e.querySelector(".sfw-upload"),b=e.querySelector(".sfw-filename"),w=e.querySelector(".sfw-progress"),h=w.querySelector("i"),y=e.querySelector(".sfw-upload-status"),T=null,S=!1,M=0,_=!1,R=()=>L(i.firmware)||D("firmwareVersion")||"",B=(A,z)=>{a.textContent=A||"",a.className="ui-sublabel sfw-status"+(z?" "+z:"")},H=()=>{let A=dt.firmware_update;if(!A||A.available!==!0)return null;let z=String(A.latest||"").trim();return z?{tag:z,notes:d("settings.firmware.deviceReported"),asset:Io(z)}:null},P=()=>{let A=H();return T?A&&Ht(A.tag,T.tag)?A:T:A},X=()=>{o.textContent=R()||d("settings.firmware.unknownVersion")},te=()=>{let A=P(),z=!!A&&Ht(A.tag,R());if(n.hidden=!z,!z){Te("firmwareUpdateAvailable",null);return}s.innerHTML=Wn(R()||d("settings.firmware.unknownVersion"))+" <span>\u2192</span> "+Wn(A.tag),c.textContent=Ti(A.notes)||d("common.noData"),u.href=A.asset.url,u.setAttribute("download",A.asset.name),u.title=A.asset.name,Te("firmwareUpdateAvailable",{current:R(),latest:A.tag,url:A.asset.url})},$=A=>{S||!A&&M&&Date.now()-M<Ai||(S=!0,M=Date.now(),r.disabled=!0,B(d("settings.firmware.checking")),Promise.resolve(Ia()).catch(()=>{}),Ba().then(z=>{T=z,te();let q=Ht(z.tag,R());B(q?d("settings.firmware.availableStatus",{version:z.tag}):d("settings.firmware.upToDate"),q?null:"ok")}).catch(z=>{T=null,te();let q=H();if(q){B(Ht(q.tag,R())?d("settings.firmware.availableStatus",{version:q.tag}):d("settings.firmware.upToDate"),Ht(q.tag,R())?null:"ok");return}if((z instanceof je?z.code:"network")==="no_releases"){B(d("settings.firmware.noReleases"),"ok");return}B(d("settings.firmware.checkFailed"),"err")}).finally(()=>{S=!1,r.disabled=!1}))};r.addEventListener("click",()=>$(!0)),l.addEventListener("click",Ni),p.addEventListener("click",()=>{let A=P();A&&window.confirm(d("settings.firmware.confirmInstall",{version:A.tag}))&&(p.disabled=!0,p.textContent=d("settings.firmware.installing"),Promise.resolve(qa()).then(()=>B(d("settings.firmware.installStarted"))).catch(()=>{B(d("settings.firmware.installFailed"),"err"),p.disabled=!1,p.textContent=d("settings.firmware.install")}))}),g.addEventListener("click",()=>m.click()),m.addEventListener("change",()=>{let A=m.files&&m.files[0];b.textContent=A?A.name:d("settings.firmware.noFile"),f.disabled=!A||_,y.textContent="",y.className="ui-note sfw-upload-status"}),f.addEventListener("click",()=>{let A=m.files&&m.files[0];!A||_||window.confirm(d("settings.firmware.confirmUpload",{file:A.name}))&&(_=!0,f.disabled=!0,g.disabled=!0,w.hidden=!1,h.style.width="0%",y.className="ui-note sfw-upload-status",y.textContent=d("settings.firmware.uploading",{value:0}),Promise.resolve(Ha()).catch(z=>console.warn("[Firmware] prepare rejected, continuing with upload:",z)).then(()=>$a(A,z=>{h.style.width=z+"%",y.textContent=d("settings.firmware.uploading",{value:z})})).then(()=>{h.style.width="100%",y.className="ui-note sfw-upload-status",y.textContent=d("settings.firmware.uploadDone")}).catch(z=>{console.error("[Firmware] upload failed:",z),w.hidden=!0,y.textContent=d("settings.firmware.uploadFailed")}).finally(()=>{_=!1,g.disabled=!1,f.disabled=!1}))}),U("section",()=>{D("section")==="settings"&&$(!1)}),k(i.firmware,()=>{X(),te()}),k("firmware_update",te),E(e),X(),D("section")==="settings"&&$(!1)}});var Di=`
...
`,mu=O({tag:"settings-backup-card",render:Ri,onMount(t,e){let o=e.querySelector(".sbk-save"),a=e.querySelector(".sbk-learned"),r=e.querySelector(".sbk-file"),n=e.querySelector(".sbk-choose"),s=e.querySelector(".sbk-restore"),c=e.querySelector(".sbk-filename"),p=e.querySelector(".sbk-status"),u=e.querySelector(".sbk-result"),l=!0,m=!1,g=(f,b)=>{p.textContent=f||"",p.className="sbk-status"+(b?" "+b:"")};a.addEventListener("click",()=>{l=!l,a.classList.toggle("on",l),a.setAttribute("aria-checked",l?"true":"false")}),o.addEventListener("click",()=>{m||(m=!0,o.disabled=!0,u.textContent="",g(d("settings.backup.saving")),ja(!0).then(f=>{if(!to(f))throw new Error("unexpected_export_payload");g(d("settings.backup.saved",{file:Za(f)}),"ok")}).catch(f=>{console.error("[Backup] export failed:",f),g(d("settings.backup.saveFailed"),"err")}).finally(()=>{m=!1,o.disabled=!1}))}),n.addEventListener("click",()=>r.click()),r.addEventListener("change",()=>{let f=r.files&&r.files[0];c.textContent=f?f.name:d("settings.backup.noFile"),s.disabled=!f||m,u.textContent="",g("")}),s.addEventListener("click",async()=>{let f=r.files&&r.files[0];if(!f||m)return;let b="";try{b=await f.text()}catch(h){g(d("settings.backup.readFailed"),"err");return}let w=null;try{w=JSON.parse(b)}catch(h){g(d("settings.backup.invalidFile"),"err");return}if(!to(w)){g(d("settings.backup.invalidFile"),"err");return}window.confirm(d("settings.backup.confirmRestore",{file:f.name}))&&(m=!0,s.disabled=!0,u.textContent="",g(d("settings.backup.restoring")),Va(w,l).then(h=>{g(d("settings.backup.restored"),"ok"),u.textContent=d("settings.backup.result",{applied:h.applied,skipped:h.skipped,ignored:h.ignored})}).catch(h=>{console.error("[Backup] restore failed:",h),g(d("settings.backup.restoreFailed"),"err")}).finally(()=>{m=!1,s.disabled=!1}))}),E(e)}});var st=Object.freeze({refinedEmber:"refined-ember",deepForest:"deep-forest"}),Gn="lune-dashboard-theme",Xn="(prefers-color-scheme: dark)",yt=null,Kn=!1;function Yn(t){return Object.values(st).includes(t)?t:st.refinedEmber}function wo(){try{return Yn(localStorage.getItem(Gn))}catch(t){return st.refinedEmber}}function Pi(){return typeof window=="undefined"||typeof window.matchMedia!="function"||window.matchMedia(Xn).matches?"dark":"light"}function Oi(){let t=Pi();if(typeof document=="undefined")return t;let e=document.documentElement;if(e.dataset.colorScheme=t,e.style.colorScheme=t,!Kn&&typeof window!="undefined"&&typeof window.matchMedia=="function"){yt=window.matchMedia(Xn);let o=()=>{let a=yt.matches?"dark":"light";e.dataset.colorScheme=a,e.style.colorScheme=a,window.dispatchEvent(new CustomEvent("lune-color-scheme-change",{detail:a}))};typeof yt.addEventListener=="function"?yt.addEventListener("change",o):typeof yt.addListener=="function"&&yt.addListener(o),Kn=!0}return t}function ra(t=wo()){let e=Yn(t);if(typeof document=="undefined")return e;Oi();let o=document.documentElement;return Object.values(st).forEach(a=>o.classList.remove(`theme-${a}`)),o.classList.add(`theme-${e}`),o.dataset.theme=e,e}function Jn(t){let e=ra(t);try{localStorage.setItem(Gn,e)}catch(o){}return typeof window!="undefined"&&window.dispatchEvent(new CustomEvent("lune-theme-change",{detail:e})),e}var Ii=[{value:st.refinedEmber,labelKey:"settings.appearance.refinedEmber"},{value:st.deepForest,labelKey:"settings.appearance.deepForest"}],qi=()=>`
...
`,yu=O({tag:"settings-appearance-card",render:qi,onMount(t,e){let o=e.querySelector(".sap-theme"),a=()=>{let r=o.value||wo();o.innerHTML=Ii.map(n=>`<option value="${n.value}">${d(n.labelKey)}</option>`).join(""),o.value=r};a(),o.value=wo(),o.addEventListener("change",()=>Jn(o.value)),window.addEventListener("lune-theme-change",r=>{r.detail&&(o.value=r.detail)}),E(e)}});var Hi=`
...
`,Au=O({tag:"smart-preheat-card",render:Bi,onMount(t,e){let o=e.querySelector(".absorb-toggle"),a=e.querySelector(".absorb-badge"),r=e.querySelector(".absorb-band"),n=e.querySelector(".absorb-delta"),s=e.querySelector(".absorb-body"),c=xe(e),p=l=>{s&&s.classList.toggle("is-disabled",!l)};c.toggle(o,{read:()=>ce(i.preheatAbsorbEnabled),onChange:p,commit:l=>{let m=l?"on":"off";x(i.preheatAbsorbEnabled,{state:m}),Ae("preheat_absorb_enabled",m)}}),c.num(r,{read:()=>C(i.preheatAbsorbBandC),commit:l=>{x(i.preheatAbsorbBandC,{value:l}),Me("preheat_absorb_band_c",l)}}),c.num(n,{read:()=>C(i.preheatDetectDeltaC),commit:l=>{x(i.preheatDetectDeltaC,{value:l}),Me("preheat_detect_delta_c",l)}});function u(){let l=String(L(i.preheatAbsorbing)||"").toLowerCase()==="active";a.textContent=l?d("common.active"):d("common.idle"),a.classList.toggle("active",l)}k(i.preheatAbsorbEnabled,c.refresh),k(i.preheatAbsorbing,u),k(i.preheatAbsorbBandC,c.refresh),k(i.preheatDetectDeltaC,c.refresh),E(e),c.refresh(),u()}});function Qn(t){let e=String(t||"").trim();return/^v?\d+\.\d+\.\d+-.+/.test(e)}ra();var $i=`
