...
`;R("ui-kit",Zr);function te(t){let e=f(t);return`<span class="help-badge" tabindex="0" role="img" aria-label="${String(e).replace(/"/g,"&quot;")}" data-i18n-label="${t}">?<span class="help-tip" data-i18n="${t}">${e}</span></span>`}function Ur(t,e){let o=Math.abs(Number(t));return!Number.isFinite(o)||o<1e3?e:Math.pow(10,Math.floor(Math.log10(o))-1)}function Wr(t){let e=String(t),o=e.indexOf(".");return o<0?0:e.length-o-1}function ie(t,e={}){let o=t.querySelector(e.title||".ui-card-title"),r=document.createElement("div");r.className="ui-form-banner",r.innerHTML='<span class="ui-form-banner-msg" data-i18n="form.unsaved">Unsaved changes</span><span class="ui-form-banner-btns"><button type="button" class="ui-form-discard" data-i18n="form.discard">Discard</button><button type="button" class="ui-form-apply" data-i18n="form.apply">Apply</button></span>',o?o.insertAdjacentElement("afterend",r):t.insertAdjacentElement("afterbegin",r);let n=[],a=()=>r.classList.toggle("show",n.some(c=>c.dirty)),i=(c,z)=>{c.dirty=z,a()};function l(c){return c.markDirty=()=>i(c,!0),n.push(c),c}function b(c,z){let C={dirty:!1,input:c},k=z.baseStep!=null?z.baseStep:parseFloat(c.step)||1,T=Wr(k),A=z.min!=null?z.min:c.min!==""?parseFloat(c.min):-1/0,M=z.max!=null?z.max:c.max!==""?parseFloat(c.max):1/0,P=q=>T>0?Number(q).toFixed(T):String(Math.round(Number(q)));if(!z.nostep){let q=document.createElement("div");q.className="ui-stepper",c.parentNode.insertBefore(q,c);let D=document.createElement("button");D.type="button",D.className="ui-step-btn",D.textContent="\u2212",D.setAttribute("aria-label",f("common.decrease"));let U=document.createElement("button");U.type="button",U.className="ui-step-btn",U.textContent="+",U.setAttribute("aria-label",f("common.increase")),q.appendChild(D),q.appendChild(c),q.appendChild(U);let G=J=>{if(c.disabled)return;let F=parseFloat(c.value);Number.isFinite(F)||(F=parseFloat(c.placeholder)),Number.isFinite(F)||(F=0);let Z=Math.min(M,Math.max(A,F+J*Ur(F,k)));c.value=P(Z),i(C,!0)};D.addEventListener("click",()=>G(-1)),U.addEventListener("click",()=>G(1)),c.addEventListener("keydown",J=>{J.key==="Enter"&&c.blur()})}return c.addEventListener("input",()=>i(C,!0)),C.sync=()=>{let q=z.read();c.value=q!=null&&Number.isFinite(Number(q))?P(q):""},C.commit=()=>{let q=parseFloat(c.value);Number.isFinite(q)&&z.commit(Math.min(M,Math.max(A,q)))},l(C)}function u(c,z){let C={dirty:!1,input:c};return c.addEventListener("input",()=>i(C,!0)),C.sync=()=>{let k=z.read();c.value=k!=null?k:""},C.commit=()=>z.commit(c.value.trim()),l(C)}function m(c,z){let C={dirty:!1,input:c};return c.addEventListener("change",()=>i(C,!0)),C.sync=()=>{let k=z.read();k!=null&&(c.value=k)},C.commit=()=>z.commit(c.value),l(C)}function x(c,z){let C={dirty:!1,input:c,staged:!1},k=c.closest(".ui-row"),T=()=>{c.classList.toggle("on",C.staged),k&&k.classList.toggle("is-on",C.staged),c.setAttribute("aria-checked",C.staged?"true":"false"),z.onChange&&z.onChange(C.staged)};return c.addEventListener("click",()=>{C.staged=!C.staged,i(C,!0),T()}),C.sync=()=>{C.staged=!!z.read(),T()},C.commit=()=>z.commit(C.staged),l(C)}function d(c){let z={dirty:!1,sync:c.sync,commit:c.commit};return l(z)}let h=()=>n.forEach(c=>{!c.dirty&&c.sync&&c.sync()}),g=()=>{n.forEach(c=>{c.dirty&&(c.commit&&c.commit(),c.dirty=!1)}),a(),e.onApply&&e.onApply()},y=()=>{n.forEach(c=>{c.dirty=!1,c.sync&&c.sync()}),a(),e.onDiscard&&e.onDiscard()};return r.querySelector(".ui-form-apply").addEventListener("click",g),r.querySelector(".ui-form-discard").addEventListener("click",y),_(r),{num:b,text:u,select:m,toggle:x,custom:d,refresh:h,apply:g,discard:y,isDirty:()=>n.some(c=>c.dirty)}}var Gr=`
...
`;R("hv6-header",Gr);var Kr=()=>`
  ...
  </header>`,Fe=t=>`<svg class="menu-icon" viewBox="0 0 24 24" aria-hidden="true">${t}</svg>`,Xr=()=>`
  <nav class="v6-side-nav" aria-label="Primary navigation">
    <div class="v6-nav-group"><div class="v6-nav-heading">Home</div>
      <a href="#" class="v6-side-link" data-section="overview">${Fe('<rect x="4" y="4" width="6" height="9"/><rect x="14" y="4" width="6" height="4"/><rect x="4" y="17" width="6" height="3"/><rect x="14" y="12" width="6" height="8"/>')}<span>Overview</span></a>
      <a href="#" class="v6-side-link" data-section="zones">${Fe('<path d="M5 19V9l7-5 7 5v10"/><path d="M9 19v-6h6v6"/>')}<span>Zones</span></a>
    </div>
    <div class="v6-nav-group"><div class="v6-nav-heading">System</div>
      <a href="#" class="v6-side-link" data-section="diagnostics">${Fe('<path d="M4 19h16M6 16V8m4 8V4m4 12v-6m4 6V7"/><path d="m5 5 3 2 4-4 4 3 3-2"/>')}<span>Diagnostics</span></a>
      <a href="#" class="v6-side-link" data-section="settings">${Fe('<circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .3 1.9l.1.1-2.8 2.8-.1-.1a1.7 1.7 0 0 0-1.9-.3 1.7 1.7 0 0 0-1 1.6v.2h-4V21a1.7 1.7 0 0 0-1-1.6 1.7 1.7 0 0 0-1.9.3l-.1.1L4.2 17l.1-.1a1.7 1.7 0 0 0 .3-1.9A1.7 1.7 0 0 0 3 14H2.8v-4H3a1.7 1.7 0 0 0 1.6-1 1.7 1.7 0 0 0-.3-1.9L4.2 7 7 4.2l.1.1A1.7 1.7 0 0 0 9 4.6a1.7 1.7 0 0 0 1-1.6v-.2h4V3a1.7 1.7 0 0 0 1 1.6 1.7 1.7 0 0 0 1.9-.3l.1-.1L19.8 7l-.1.1a1.7 1.7 0 0 0-.3 1.9 1.7 1.7 0 0 0 1.6 1h.2v4H21a1.7 1.7 0 0 0-1.6 1Z"/>')}<span>Settings</span></a>
    </div>
    <button type="button" class="v6-side-link v6-more-toggle" aria-expanded="false">${Fe('<circle cx="5" cy="12" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="19" cy="12" r="1.5"/>')}<span>More</span></button>
    <div class="v6-side-utility"><a href="#" class="v6-side-link" data-section="help">${Fe('<circle cx="12" cy="12" r="9"/><path d="M9.8 9a2.4 2.4 0 1 1 3.7 2c-.9.6-1.5 1.1-1.5 2.3M12 17h.01"/>')}<span>Help</span></a></div>
  </nav>`,Do={overview:["Overview","Local heating status and current exceptions"],zones:["Zones","Physical loops, applied targets and valve state"],diagnostics:["Diagnostics","Health, evidence and recovery"],settings:["Settings","Device configuration and safety"],help:["Help","Guidance for operating Lune V6"]};N({tag:"hv6-header",render:Kr,onMount(t,e){let o=e.querySelector("#hdr-live"),r=e.querySelector("#v6-view-title"),n=e.querySelector("#v6-view-subtitle"),a=e.querySelector("#hdr-update");function i(){let b=E("firmwareUpdateAvailable");a.hidden=!b,b&&(a.textContent=f("status.updateAvailable",{version:b.latest}),a.title=f("settings.firmware.badgeTitle"))}a.addEventListener("click",()=>{fe("settings");let b=document.querySelector(".settings-firmware-card");if(!b)return;let u=b.closest("details");u&&(u.open=!0),b.scrollIntoView({behavior:"smooth",block:"center"})});function l(){let b=E("section")||"overview",u=Do[b]||Do.overview;r.textContent=u[0],n.textContent=u[1],o.textContent=E("live")?f("status.live"):f("status.offline"),o.classList.toggle("is-live",!!E("live"))}B("section",l),B("live",l),B("firmwareUpdateAvailable",i),_(e),l(),i()}});N({tag:"hv6-sidebar",render:Xr,onMount(t,e){let o=e.querySelector(".v6-side-nav"),r=e.querySelectorAll("[data-section]"),n=e.querySelector(".v6-more-toggle");function a(){let i=E("section");r.forEach(l=>{l.dataset.section&&(l.dataset.section===i?l.classList.add("active"):l.classList.remove("active"),l.setAttribute("aria-current",l.dataset.section===i?"page":"false"))})}r.forEach(i=>i.addEventListener("click",l=>{l.preventDefault(),fe(i.dataset.section),o&&o.classList.contains("more-open")&&(o.classList.remove("more-open"),n&&n.setAttribute("aria-expanded","false"))})),n&&o&&n.addEventListener("click",()=>{let i=o.classList.toggle("more-open");n.setAttribute("aria-expanded",String(i))}),B("section",a),_(e),a()}});function Y(t){return t!=null&&!isNaN(t)?Math.round(t*10)/10+"\xB0C":"---"}function Te(t){return t!=null&&!isNaN(t)?(t|0)+"%":"---"}function Ro(t){if(t==null||isNaN(t)||t<0)return"---";t=t|0;var e=t/86400|0,o=t%86400/3600|0,r=t%3600/60|0;return e>0?e+"d "+o+"h "+r+"m":o>0?o+"h "+r+"m":r+"m"}var Yr=`
.connectivity-card {
...
`,fs=N({tag:"connectivity-card",render:Jr,onMount(t,e){let o=e.querySelector(".cc-ip"),r=e.querySelector(".cc-ssid"),n=e.querySelector(".cc-mac"),a=e.querySelector(".cc-up"),i=e.querySelector(".cc-ver"),l=0,b=Date.now(),u=!1;function m(){if(!u){a.textContent="---";return}let h=Math.max(0,Math.floor((Date.now()-b)/1e3)),g=Ro(l+h);a.textContent!==g&&(a.textContent=g)}function x(){o.textContent=S(s.ip)||"---",r.textContent=S(s.ssid)||"---",n.textContent=S(s.mac)||"---",i.textContent=S(s.firmware)||"---";let h=L(s.uptime);if(h!=null&&!isNaN(h)&&h>=0){let g=h|0;(!u||g!==l)&&(l=g,b=Date.now(),u=!0)}m()}w(s.ip,x),w(s.ssid,x),w(s.mac,x),w(s.firmware,x),w(s.uptime,x);let d=setInterval(m,1e3);e.addEventListener("hv6-unmount",()=>clearInterval(d),{once:!0}),_(e),x()}});var Qr="http://www.w3.org/2000/svg",en=`
.chart-card {
  border: 1px solid var(--panel-border);
  border-radius: 8px;
  background: var(--panel-bg-vibrant);
...
`;R("chart-kit",en);function ne(t,e,o){let r=document.createElementNS(Qr,t);if(e)for(let n in e)r.setAttribute(n,e[n]);return o!=null&&(r.textContent=o),r}function Oo(t){if(!t.length)return"";if(t.length<3)return"M "+t.map(r=>`${r.x.toFixed(2)} ${r.y.toFixed(2)}`).join(" L ");let e=.16,o=`M ${t[0].x.toFixed(2)} ${t[0].y.toFixed(2)}`;for(let r=0;r<t.length-1;r++){let n=t[r-1]||t[r],a=t[r],i=t[r+1],l=t[r+2]||i,b=a.x+(i.x-n.x)*e,u=a.y+(i.y-n.y)*e,m=i.x-(l.x-a.x)*e,x=i.y-(l.y-a.y)*e;o+=` C ${b.toFixed(2)} ${u.toFixed(2)}, ${m.toFixed(2)} ${x.toFixed(2)}, ${i.x.toFixed(2)} ${i.y.toFixed(2)}`}return o}function Io(t,e,o){let r=document.createElement("div");r.className="chart-tooltip",e.appendChild(r);let n=ne("g",{class:"chart-cursor",style:"display:none"}),a=ne("line",{class:"chart-cursor-line",y1:o.plotTop,y2:o.plotBottom});n.appendChild(a);let i=[];t.appendChild(n);function l(x){let d=0,h=1/0;for(let g=0;g<o.count;g++){let y=Math.abs(x-o.xAt(g));y<h&&(h=y,d=g)}return d}function b(x){let d=t.getScreenCTM();if(!d)return null;let h=t.createSVGPoint();return h.x=x.clientX,h.y=x.clientY,h.matrixTransform(d.inverse())}function u(x){if(!o.count)return;let d=b(x);if(!d)return;let h=l(d.x),g=o.xAt(h);a.setAttribute("x1",g),a.setAttribute("x2",g);let y=o.dots(h);for(;i.length<y.length;){let k=ne("circle",{class:"chart-cursor-dot",r:3.4});n.appendChild(k),i.push(k)}i.forEach((k,T)=>{T<y.length?(k.setAttribute("cx",g),k.setAttribute("cy",y[T].y),k.setAttribute("fill",y[T].color),k.style.display=""):k.style.display="none"}),n.style.display="";let c=o.rows(h).map(k=>`<div class="tt-row"><span class="tt-swatch" style="background:${k.color}"></span>${k.label}<span class="tt-val">${k.value}</span></div>`).join("");r.innerHTML=`<div class="tt-time">${o.label(h)}</div>${c}`,r.classList.add("show");let z=e.getBoundingClientRect(),C=x.clientX-z.left+14;C+r.offsetWidth>z.width-6&&(C=x.clientX-z.left-r.offsetWidth-14),r.style.left=Math.max(6,C)+"px",r.style.top=Math.max(6,x.clientY-z.top+12)+"px"}function m(){r.classList.remove("show"),n.style.display="none"}return t.addEventListener("pointermove",u),t.addEventListener("pointerleave",m),()=>{t.removeEventListener("pointermove",u),t.removeEventListener("pointerleave",m),r.remove()}}var qe=1e3,Nt=180,de=14,tn=42,on=44,we=42,st=qe-we-tn,ye=Nt-de-on,Se=de+ye,Dt=24*3600,Po=ee+2,qo=ee+3,nt=ee+4,rn="var(--series-warm)",nn="var(--series-cool)",Ho="var(--series-solar)",an=`
...
`;R("graph-widgets",an);var Bo=()=>'<div class="chart-card"><div class="chart-head"><span class="chart-title" data-i18n="overview.graph.flowReturnDemand">Flow / Return / Demand</span><span class="chart-sub gw-dt">\u2014</span></div><div class="gw-controls" role="toolbar" data-i18n-label="overview.graph.layers" aria-label="Flow chart layers"><button type="button" class="gw-toggle" data-layer="flow" aria-pressed="true" data-i18n="overview.graph.layers.flow">Flow</button><button type="button" class="gw-toggle" data-layer="return" aria-pressed="true" data-i18n="overview.graph.layers.return">Return</button><button type="button" class="gw-toggle" data-layer="demand" aria-pressed="true" data-i18n="overview.graph.layers.demand">Demand</button></div><svg class="gw-flow"></svg></div>',$o=()=>'<div class="chart-card"><div class="chart-head"><span class="chart-title" data-i18n="overview.graph.demandIndex">Demand Index</span><span class="chart-sub gw-demand-text">\u2014</span></div><svg class="gw-demand"></svg></div>',sn=t=>t.variant==="flow-return"?`<div class="graph-widgets">${Bo()}</div>`:t.variant==="demand"?`<div class="graph-widgets">${$o()}</div>`:`<div class="graph-widgets">${Bo()}${$o()}</div>`;function Vo(t,e){return Number.isFinite(t)?e==="%"?Math.round(t)+"%":t.toFixed(1):"\u2014"}function ln(t,e){return Number.isFinite(t)?e==="%"?Math.round(t)+"%":t.toFixed(1)+"\xB0":"\u2014"}function Rt(t,e,o){let r=[];for(let n=0;n<t.length;n++){let a=t[n];if(!a||a[0]<o)continue;let i=a[e];i==null||!Number.isFinite(i)||r.push({t:a[0],v:i})}return r}var it=(t,e)=>we+Math.max(0,Math.min(1,(t-e)/Dt))*st;function cn(t,e,o){let r=Number(Date.now()/1e3)|0,n=3600,a=Math.ceil((r-Dt)/n)*n,i=Math.floor(r/n)*n,l=Math.floor(r/n)*n;for(let u=a;u<=i;u+=n){let m=o-(r-u),x=it(m,e),d=new Date(u*1e3),h=u===l,g=Se+16;t.appendChild(ne("text",{x,y:g,"text-anchor":"end",transform:`rotate(-45 ${x.toFixed(1)} ${g})`,class:"chart-hour"+(h?" now":"")},String(d.getHours()).padStart(2,"0")))}let b=it(o,e);t.appendChild(ne("line",{x1:b,y1:de,x2:b,y2:Se,stroke:"var(--series-solar)","stroke-width":"1","stroke-dasharray":"2 3",opacity:".55","vector-effect":"non-scaling-stroke"}))}function dn(t){let e=[];if(t.forEach(a=>a.forEach(i=>e.push(i.v))),!e.length)return{min:0,max:10};let o=Math.min(...e),r=Math.max(...e);o===r&&(o-=.5,r+=.5);let n=(r-o)*.1;return o-=n,r+=n,{min:o,max:r}}function pn(t,e,o){let r=t.filter(n=>n.unit==="C").map(n=>Rt(e,n.index,o));return dn(r)}function jo(t,e,o,r,n,a){t.innerHTML="",t.setAttribute("viewBox",`0 0 ${qe} ${Nt}`),t.setAttribute("preserveAspectRatio","xMidYMid meet");let i=o.map(g=>Rt(r,g.index,n));if(!i.some(g=>g.length))return t.appendChild(ne("text",{x:qe/2,y:Nt/2,"text-anchor":"middle",class:"chart-empty"},"Collecting history\u2026")),null;let l=pn(o,r,n),b=Math.max(.001,l.max-l.min),u=g=>de+(1-(g-l.min)/b)*ye,m=g=>de+(1-Math.max(0,Math.min(100,g))/100)*ye,x=(g,y)=>g.unit==="%"?m(y):u(y);for(let g=0;g<3;g++){let y=g/2,c=de+y*ye;t.appendChild(ne("line",{x1:we,y1:c,x2:we+st,y2:c,class:"chart-grid"})),o.some(z=>z.unit==="C")&&t.appendChild(ne("text",{x:we-6,y:c+4,"text-anchor":"end",class:"chart-tick"},Vo(l.max-b*y,"C")+"\xB0")),o.some(z=>z.unit==="%")&&t.appendChild(ne("text",{x:we+st+6,y:c+4,"text-anchor":"start",class:"chart-tick"},Vo(100-100*y,"%")))}t.appendChild(ne("line",{x1:we,y1:Se,x2:we+st,y2:Se,class:"chart-axis"})),o.some(g=>g.unit==="C")&&t.appendChild(ne("text",{x:9,y:de+ye/2,transform:`rotate(-90 9 ${(de+ye/2).toFixed(1)})`,"text-anchor":"middle",class:"chart-axis-label"},f("overview.graph.axis.temp"))),o.some(g=>g.unit==="%")&&t.appendChild(ne("text",{x:qe-9,y:de+ye/2,transform:`rotate(90 ${qe-9} ${(de+ye/2).toFixed(1)})`,"text-anchor":"middle",class:"chart-axis-label"},f("overview.graph.axis.demand"))),cn(t,n,a),o.forEach((g,y)=>{let c=i[y].map(C=>({x:it(C.t,n),y:x(g,C.v)}));if(!c.length)return;let z=Oo(c);g.fill&&t.appendChild(ne("path",{d:z+` L ${c[c.length-1].x.toFixed(1)} ${Se} L ${c[0].x.toFixed(1)} ${Se} Z`,fill:g.fill,stroke:"none"})),t.appendChild(ne("path",{d:z,fill:"none",stroke:g.color,"stroke-width":String(g.width||2.2),"stroke-linecap":"round","stroke-linejoin":"round"}))});let d=[];for(let g=0;g<r.length;g++){let y=r[g];if(!y||y[0]<n)continue;let c=o.map(z=>y[z.index]);c.every(z=>z==null||!Number.isFinite(z))||d.push({t:y[0],vals:c})}if(!d.length)return null;let h=Date.now();return Io(t,e,{count:d.length,plotTop:de,plotBottom:Se,xAt:g=>it(d[g].t,n),label:g=>new Date(h-(a-d[g].t)*1e3).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),dots:g=>o.map((y,c)=>({y:x(y,d[g].vals[c]),color:y.color})).filter((y,c)=>Number.isFinite(d[g].vals[c])),rows:g=>o.map((y,c)=>({color:y.color,label:y.label,value:ln(d[g].vals[c],y.unit)})).filter((y,c)=>Number.isFinite(d[g].vals[c]))})}function at(t,e,o){let r=Rt(t,e,o);return r.length?r[r.length-1].v:null}var Ss=N({tag:"graph-widgets",state:t=>({variant:t&&t.variant||"both"}),render:sn,onMount(t,e){let o=e.querySelector(".gw-dt"),r=e.querySelector(".gw-demand-text"),n=e.querySelector(".gw-flow"),a=e.querySelector(".gw-demand"),i=Array.from(e.querySelectorAll(".gw-toggle")),l={flow:!0,return:!0,demand:!0},b=null,u=null;function m(){i.forEach(h=>{let g=h.dataset.layer;h.classList.toggle("is-off",!l[g]),h.setAttribute("aria-pressed",l[g]?"true":"false")})}function x(){let h=[];return l.flow&&h.push({index:Po,color:rn,label:f("overview.graph.layers.flow"),unit:"C",width:2.4}),l.return&&h.push({index:qo,color:nn,label:f("overview.graph.layers.return"),unit:"C",width:2}),l.demand&&h.push({index:nt,color:Ho,label:f("overview.graph.layers.demand"),unit:"%",width:1.8,fill:"rgba(255,193,77,.10)"}),h}function d(){let h=E("zoneStateHistory"),g=h&&Array.isArray(h.entries)?h.entries:[],y=h&&h.uptime_s||Number(Date.now()/1e3)|0,c=y-Dt;if(n){b&&b();let z=at(g,Po,c),C=at(g,qo,c),k=at(g,nt,c),T=[];z!=null&&C!=null&&T.push("\u0394 "+(z-C).toFixed(1)+"\xB0"),k!=null&&T.push(Math.round(k)+"%"),o.textContent=T.length?T.join(" \xB7 "):"\u2014",b=jo(n,n.closest(".chart-card"),x(),g,c,y)}if(a){u&&u();let z=at(g,nt,c);r.textContent=z!=null?Math.round(z)+"%":"\u2014",u=jo(a,a.closest(".chart-card"),[{index:nt,color:Ho,label:f("overview.graph.layers.demand"),unit:"%",width:2.2,fill:"var(--series-cool-fill)"}],g,c,y)}}i.forEach(h=>{h.addEventListener("click",()=>{let g=h.dataset.layer;l[g]=!l[g],!l.flow&&!l.return&&!l.demand&&(l[g]=!0),m(),d()})}),B("zoneStateHistory",d),_(e),m(),d()}});var ze={0:{labelKey:"state.off",color:"#2c4875"},1:{labelKey:"state.manual",color:"#7aa7ce"},2:{labelKey:"state.calibrating",color:"#ffd380"},3:{labelKey:"state.waitCal",color:"#4e6977"},4:{labelKey:"state.waitTemp",color:"#4e6977"},5:{labelKey:"state.heating",color:"var(--accent)"},6:{labelKey:"state.idle",color:"#39354c"},7:{labelKey:"state.overheated",color:"#ff6361"},255:{labelKey:"",color:"transparent"}},He=24*3600,un=He,Be=18,Pt=4,_e=54,ct=32,Ne=4,dt=10,Wo=6,Go="#ffc14d",Ot=9,Zo=ee+1,Ko=Ne+ee*(Be+Pt)-Pt,It=Ko+Wo,lt=Ko+Wo+dt+ct,mn=`
.timeline-card {
  border: 1px solid var(--panel-border);
  border-radius: 8px;
  background: var(--panel-bg-vibrant);
...
`;R("zone-state-timeline",mn);var gn=()=>`
...
`;function fn(t,e){if(!t||!t.entries||t.entries.length===0)return null;let o=t.entries,r=t.uptime_s||e||0,n=Number(Date.now()/1e3)|0,a=1e3,i=a-_e;function l(k){let T=(k+He)/un;return _e+Math.max(0,Math.min(1,T))*i}function b(k){return k-r}let u="http://www.w3.org/2000/svg",m=document.createElementNS(u,"svg");m.setAttribute("viewBox","0 0 "+a+" "+lt),m.classList.add("timeline-svg");let x=document.createElementNS(u,"rect");x.setAttribute("x",_e),x.setAttribute("y",Ne),x.setAttribute("width",i),x.setAttribute("height",lt-Ne-ct),x.setAttribute("fill","rgba(0,32,46,0.55)"),x.setAttribute("rx","4"),m.appendChild(x);let d=l(0),h=[-24,-18,-12,-6,0].map(k=>k*3600);for(let k of h){let T=l(k),A=document.createElementNS(u,"line");A.setAttribute("x1",T),A.setAttribute("y1",Ne),A.setAttribute("x2",T),A.setAttribute("y2",lt-ct),A.setAttribute("stroke",k===0?"var(--series-solar)":"rgba(120,146,200,.16)"),A.setAttribute("stroke-width","1"),k===0&&(A.setAttribute("stroke-dasharray","2 3"),A.setAttribute("opacity",".55"),A.setAttribute("vector-effect","non-scaling-stroke")),m.appendChild(A)}m.appendChild(bn(u,"text",{x:d+4,y:Ne+11,"text-anchor":"start",fill:"rgba(255,211,128,.92)","font-size":"9","font-family":"Montserrat, sans-serif","font-weight":"600"},"now"));for(let k=0;k<ee;k++){let T=Ne+k*(Be+Pt),A=document.createElementNS(u,"rect");A.setAttribute("x",_e),A.setAttribute("y",T),A.setAttribute("width",i),A.setAttribute("height",Be),A.setAttribute("fill",k%2===0?"rgba(124,155,208,0.05)":"rgba(124,155,208,0.00)"),m.appendChild(A);let M=document.createElementNS(u,"text");M.setAttribute("x",_e-4),M.setAttribute("y",T+Be/2+1),M.setAttribute("text-anchor","end"),M.setAttribute("dominant-baseline","middle"),M.setAttribute("fill","rgba(233,222,210,.62)"),M.setAttribute("font-size","9.5"),M.setAttribute("font-family","Montserrat, sans-serif"),M.setAttribute("font-weight","600"),M.textContent="Z"+(k+1),m.appendChild(M);let P=o.map(D=>({rel:b(D[0]),state:D[k+1]})).filter(D=>D.rel>=-He&&D.rel<=0),q=(D,U,G)=>{if(G===255)return;let J=ze[G]||ze[255];if(J.color==="transparent")return;let F=l(D),Z=l(U),ue=Math.max(1,Z-F),me=document.createElementNS(u,"rect");me.setAttribute("x",F),me.setAttribute("y",T+(Be-Ot)/2),me.setAttribute("width",ue),me.setAttribute("height",Ot),me.setAttribute("fill",J.color),me.setAttribute("rx",String(Ot/2)),me.setAttribute("opacity","0.9"),m.appendChild(me)};if(P.length){let D=P[0].rel,U=P[0].state;for(let G=1;G<P.length;G++){let J=P[G];J.state!==U&&(q(D,J.rel,U),D=J.rel,U=J.state)}q(D,0,U)}}{let k=document.createElementNS(u,"rect");k.setAttribute("x",_e),k.setAttribute("y",It),k.setAttribute("width",i),k.setAttribute("height",dt),k.setAttribute("fill","rgba(188,80,144,0.10)"),k.setAttribute("rx","2"),m.appendChild(k);let T=document.createElementNS(u,"text");T.setAttribute("x",_e-4),T.setAttribute("y",It+dt/2+1),T.setAttribute("text-anchor","end"),T.setAttribute("dominant-baseline","middle"),T.setAttribute("fill","rgba(233,222,210,.62)"),T.setAttribute("font-size","8.5"),T.setAttribute("font-family","Montserrat, sans-serif"),T.setAttribute("font-weight","600"),T.textContent=f("overview.timeline.absorb"),m.appendChild(T);let A=o.map(M=>({rel:b(M[0]),on:M.length>Zo?M[Zo]:0})).filter(M=>M.rel>=-He&&M.rel<=0);if(A.length){let M=(D,U)=>{let G=l(D),J=Math.max(1,l(U)-G),F=document.createElementNS(u,"rect");F.setAttribute("x",G),F.setAttribute("y",It),F.setAttribute("width",J),F.setAttribute("height",dt),F.setAttribute("fill",Go),F.setAttribute("rx","2"),F.setAttribute("opacity","0.9"),m.appendChild(F)},P=A[0].rel,q=A[0].on;for(let D=1;D<A.length;D++)A[D].on!==q&&(q&&M(P,A[D].rel),P=A[D].rel,q=A[D].on);q&&M(P,0)}}let g=lt-ct+15,y=3600,c=Math.ceil((n-He)/y)*y,z=Math.floor(n/y)*y,C=Math.floor(n/y)*y;for(let k=c;k<=z;k+=y){let T=k-n,A=l(T),M=new Date(k*1e3),P=String(M.getHours()).padStart(2,"0"),q=k===C,D=document.createElementNS(u,"text");D.setAttribute("x",A),D.setAttribute("y",g),D.setAttribute("text-anchor","end"),D.setAttribute("fill",q?"rgba(255,211,128,.95)":"rgba(202,219,248,.72)"),D.setAttribute("font-size","9"),D.setAttribute("font-family",'"Montserrat", sans-serif'),D.setAttribute("font-weight","500"),D.setAttribute("font-variant-numeric","tabular-nums lining-nums"),D.setAttribute("font-feature-settings",'"tnum" 1, "lnum" 1'),D.setAttribute("letter-spacing","0"),D.setAttribute("transform",`rotate(-45 ${A.toFixed(1)} ${g})`),D.textContent=P,m.appendChild(D)}return m}function bn(t,e,o,r){let n=document.createElementNS(t,e);for(let a in o)n.setAttribute(a,o[a]);return r!=null&&(n.textContent=r),n}function Uo(t){t.innerHTML="";let e=[{code:5,...ze[5]},{code:6,...ze[6]},{code:0,...ze[0]},{code:1,...ze[1]},{code:7,...ze[7]},{code:2,...ze[2]}];for(let r of e){let n=document.createElement("div");n.className="tl-legend-item",n.innerHTML='<span class="tl-legend-dot" style="background:'+r.color+'"></span>'+(r.labelKey?f(r.labelKey):""),t.appendChild(n)}let o=document.createElement("div");o.className="tl-legend-item",o.innerHTML='<span class="tl-legend-dot" style="background:'+Go+'"></span>'+f("overview.timeline.preheatAbsorption"),t.appendChild(o)}var Fs=N({tag:"zone-state-timeline",render:gn,onMount(t,e){let o=e.querySelector(".tl-body"),r=e.querySelector(".timeline-legend");Uo(r);function n(){let a=E("zoneStateHistory"),i=(()=>{let b=E&&E("zoneStateHistory");return b&&b.uptime_s||Number(Date.now()/1e3)|0})();if(o.innerHTML="",!a||!a.entries||a.entries.length===0){let b=document.createElement("div");b.className="timeline-empty",b.textContent=f("overview.timeline.noHistory"),o.appendChild(b);return}let l=fn(a,i);l&&o.appendChild(l)}B("zoneStateHistory",n),B("zoneNames",n),w(s.drivers,n);for(let a=1;a<=ee;a++)w(p.enabled(a),n),w(p.state(a),n),w(p.temp(a),n),w(p.setpoint(a),n),w(p.preheatAdvance(a),n);_(e),n()}});var vn=`
.zone-grid {
    display: grid;
...
`;R("zone-grid",vn);var hn=()=>'<div class="zone-grid" aria-label="Zones"></div>',Rs=N({tag:"zone-grid",state:t=>({selection:t.selection!==!1,navigate:t.navigate!==!1}),render:hn,onMount(t,e){for(let o=1;o<=6;o++)e.appendChild(j("zone-card",{zone:o,selection:t.selection,navigate:t.navigate}))}});var xn=`
.zone-card {
...
`,Vs=N({tag:"zone-card",state:t=>({zone:t.zone,selection:t.selection!==!1,navigate:t.navigate!==!1}),render:yn,onMount(t,e){let o=t.zone,r=p.temp(o),n=p.state(o),a=p.enabled(o),i=e.querySelector(".zc-state-label"),l=e.querySelector(".zc-zone-name"),b=e.querySelector(".zc-friendly"),u=e.querySelector(".zc-temp"),m=e.querySelector(".zc-target"),x=e.querySelector(".zc-valve-value");function d(){var M;let g=X(a),y=String(S(n)||"").toUpperCase()||"OFF",c=String(S(p.motorLastFault(o))||"").toUpperCase(),z=c&&c!=="NONE"&&c!=="OK",C=g&&(y==="FAULT"||z)?"FAULT":y,k=t.selection&&E("selectedZone")===o,T=ve(o);l.textContent=T||"Zone "+o,b.textContent="Zone "+o+" \xB7 physical loop",u.textContent=Y(L(r)),m.textContent="Applied "+Y((M=L(p.effectiveSetpoint(o)))!=null?M:L(p.setpoint(o))),x.textContent=Te(L(p.valve(o)));let A=g?C:"OFF";i.textContent=A==="HEATING"?f("state.heating"):A==="IDLE"?f("state.idle"):A==="FAULT"?f("common.fault"):A==="MANUAL"?f("state.manual"):A==="OVERHEATED"?f("state.overheated"):A==="CALIBRATING"?f("state.calibrating"):f("state.off"),e.title=z?f("zone.card.fault",{fault:c}):"",e.classList.toggle("active",k),k?e.setAttribute("aria-current","location"):e.removeAttribute("aria-current"),e.setAttribute("aria-label",`${l.textContent}, ${u.textContent}, ${m.textContent}, ${i.textContent}. Open details.`),e.classList.toggle("disabled",!g),e.classList.toggle("zs-heating",g&&A==="HEATING"),e.classList.toggle("zs-fault",g&&A==="FAULT"),e.classList.toggle("zs-idle",g&&A==="IDLE"),e.classList.toggle("zs-off",!g||A==="OFF")}function h(){Oe(o),t.navigate&&fe("zones"),e.dispatchEvent(new CustomEvent("zone-open",{bubbles:!0,detail:{zone:o}}))}e.addEventListener("click",h),w(r,d),w(p.setpoint(o),d),w(p.effectiveSetpoint(o),d),w(p.valve(o),d),w(n,d),w(a,d),w(p.motorLastFault(o),d),B("selectedZone",d),B("zoneNames",d),d()}});var wn=`
...
`,Jo=5,Qo=35,ut=.5;function Xo(t){return t!=null?Number(t).toFixed(2)+"x":"---"}function Yo(t){return t!=null?Number(t).toFixed(0):"---"}function kn(t){return t!=null?Number(t).toFixed(2)+"C":"---"}function De(t){return t==null||Number.isNaN(Number(t))?"":(Math.round(Number(t)*10)/10).toFixed(1)}function Sn(t){if(t==null)return null;let e=String(t).trim().replace(",",".").replace(/[^\d.+-]/g,"");if(!e)return null;let o=Number(e);if(!Number.isFinite(o))return null;let r=Math.round(o/ut)*ut;return Math.min(Qo,Math.max(Jo,Number(r.toFixed(1))))}function _n(t){let e=Number(L(p.setpoint(t)));return Number.isFinite(e)?e:null}function pt(t){let e=Number(L(p.coordinatorOffset(t)));return Number.isFinite(e)?e:0}function ke(t){let e=Number(L(p.effectiveSetpoint(t)));if(Number.isFinite(e))return e;let o=_n(t);return o==null?null:Number((o+pt(t)).toFixed(1))}function $e(t){return Math.min(Qo,Math.max(Jo,Number(Number(t).toFixed(1))))}function Cn(t,e){if(!e)return f("common.disabled");let o=String(t||"IDLE").toUpperCase();return o==="HEATING"?f("state.heating"):o==="IDLE"?f("state.idle"):o==="OFF"?f("state.off"):o==="FAULT"?f("common.fault"):o==="MANUAL"?f("state.manual"):o==="OVERHEATED"?f("state.overheated"):o==="CALIBRATING"?f("state.calibrating"):o}var Js=N({tag:"zone-detail",state:t=>({zone:t.zone,temp:"---",setpoint:"---",valve:"---",state:"---"}),render:zn,methods:{update(t,e){var m;let o=E("selectedZone"),r=String(S(p.state(o))||"").toUpperCase(),n=X(p.enabled(o));this.zone=o,t.dataset.zone=String(o),e.title.textContent=ae(o),document.activeElement!==e.setpoint&&(e.setpoint.value=De(ke(o))),e.base.textContent=Y((m=L(p.baseSetpoint(o)))!=null?m:L(p.setpoint(o)));let a=L(p.coordinatorOffset(o));e.offset.textContent=a==null?"---":(a>0?"+":"")+Number(a).toFixed(1)+"\xB0C",e.temp.textContent=Y(L(p.temp(o))),e.ret.textContent=Y(L("sensor-manifold_return_temperature")),e.valve.textContent=Te(L(p.valve(o)));let i=e.badge;i.textContent=Cn(r,n);let l=n?r==="HEATING"?"badge-heating":r==="IDLE"?"badge-idle":r==="FAULT"?"badge-fault":"":"badge-disabled";i.className="zd-badge"+(l?" "+l:""),e.toggle.classList.toggle("on",n),e.orip.textContent=Yo(L(p.motorOpenRipples(o))),e.crip.textContent=Yo(L(p.motorCloseRipples(o))),e.ofac.textContent=Xo(L(p.motorOpenFactor(o))),e.cfac.textContent=Xo(L(p.motorCloseFactor(o))),e.ph.textContent=kn(L(p.preheatAdvance(o)));let b=String(S(p.motorLastFault(o))||"").toUpperCase(),u=b&&b!=="NONE"&&b!=="OK";e.fault.hidden=!u,u&&(e.faultVal.textContent=b)},commitSetpoint(t){let e=this.zone,o=Sn(t);if(o==null)return null;let r=$e(o-pt(e));return Qe(e,r),ke(e)},incSetpoint(){let t=this.zone,e=ke(t),o=$e((e==null?20:e)+ut);Qe(t,$e(o-pt(t)))},decSetpoint(){let t=this.zone,e=ke(t),o=$e((e==null?20:e)-ut);Qe(t,$e(o-pt(t)))},toggleEnabled(){let t=this.zone,e=X(p.enabled(t));ro(t,!e)}},onMount(t,e){let o={title:e.querySelector(".zd-title"),setpoint:e.querySelector(".zd-setpoint"),temp:e.querySelector(".zd-temp"),base:e.querySelector(".zd-base"),offset:e.querySelector(".zd-offset"),ret:e.querySelector(".zd-ret"),valve:e.querySelector(".zd-valve"),badge:e.querySelector(".zd-badge"),toggle:e.querySelector(".btn-toggle"),inc:e.querySelector(".btn-inc"),dec:e.querySelector(".btn-dec"),orip:e.querySelector(".zd-orip"),crip:e.querySelector(".zd-crip"),ofac:e.querySelector(".zd-ofac"),cfac:e.querySelector(".zd-cfac"),ph:e.querySelector(".zd-ph"),fault:e.querySelector(".zd-fault"),faultVal:e.querySelector(".zd-fault-val")};o.inc.onclick=()=>t.incSetpoint(),o.dec.onclick=()=>t.decSetpoint(),o.toggle.onclick=()=>t.toggleEnabled();let r=()=>{let i=t.commitSetpoint(o.setpoint.value);o.setpoint.value=i!=null?De(i):De(ke(t.zone))};o.setpoint.addEventListener("keydown",i=>{i.key==="Enter"?(i.preventDefault(),o.setpoint.blur()):i.key==="Escape"?(i.preventDefault(),o.setpoint.value=De(ke(t.zone)),o.setpoint.blur()):i.key==="ArrowUp"?(i.preventDefault(),t.incSetpoint(),o.setpoint.value=De(ke(t.zone))):i.key==="ArrowDown"&&(i.preventDefault(),t.decSetpoint(),o.setpoint.value=De(ke(t.zone)))}),o.setpoint.addEventListener("blur",r),o.setpoint.addEventListener("focus",()=>{requestAnimationFrame(()=>o.setpoint.select())});let n=()=>t.update(e,o),a=i=>{let l=E("selectedZone");(i===p.temp(l)||i===p.setpoint(l)||i===p.baseSetpoint(l)||i===p.effectiveSetpoint(l)||i===p.coordinatorOffset(l)||i===p.valve(l)||i===p.state(l)||i===p.enabled(l))&&n()};for(let i=1;i<=6;i++)w(p.temp(i),a),w(p.setpoint(i),a),w(p.baseSetpoint(i),a),w(p.effectiveSetpoint(i),a),w(p.coordinatorOffset(i),a),w(p.valve(i),a),w(p.state(i),a),w(p.enabled(i),a),w(p.motorOpenRipples(i),n),w(p.motorCloseRipples(i),n),w(p.motorOpenFactor(i),n),w(p.motorCloseFactor(i),n),w(p.preheatAdvance(i),n),w(p.motorLastFault(i),n);w("sensor-manifold_return_temperature",n),B("selectedZone",n),_(e),n()}});var Ln=`
...
`;R("zone-sensor-card",Ln);var An=()=>{let t='<option value="None" data-i18n="common.none">None</option>';for(let e=1;e<=8;e++)t+='<option value="Probe '+e+'">Probe '+e+"</option>";return`
  ...
  `};function er(t,e){let o=t.value,r='<option value="None" data-i18n="common.none">'+f("common.none")+"</option>";for(let n=1;n<=6;n++)n!==e&&(r+='<option value="Zone '+n+'">'+f("common.zone")+" "+n+"</option>");t.innerHTML=r,t.value=o||"None"}function En(t){return t==="BLE"||t==="BLE Sensor"?"BLE Sensor":"Local Probe"}function Mn(t){return t==="BLE Sensor"?"BLE":"Local Probe"}function tr(t,e){let o='<option value="Local Probe" data-i18n="zone.sensor.localProbe">'+f("zone.sensor.localProbe")+'</option><option value="BLE Sensor" data-i18n="zone.sensor.bleSource">'+f("zone.sensor.bleSource")+"</option>";t.innerHTML!==o&&(t.innerHTML=o),t.value=e}var ii=N({tag:"zone-sensor-card",render:An,onMount(t,e){let o=e.querySelector(".zs-probe"),r=e.querySelector(".zs-source"),n=e.querySelector(".zs-ble"),a=e.querySelector(".zs-sync"),i=e.querySelector(".zs-row-ble"),l=e.querySelector(".zs-scan"),b=e.querySelector(".zs-scan-list"),u=0;function m(){return E("selectedZone")}function x(){i.style.display=r.value==="BLE Sensor"?"":"none"}let d=ie(e);tr(r,"Local Probe"),d.select(o,{read:()=>S(p.probe(m()))||void 0,commit:c=>et(m(),"zone_probe",c)}),d.select(r,{read:()=>En(String(S(p.tempSource(m()))||"")),commit:c=>et(m(),"zone_temp_source",Mn(c))}),d.select(a,{read:()=>S(p.syncTo(m()))||"None",commit:c=>et(m(),"zone_sync_to",c)});let h=d.text(n,{read:()=>S(p.ble(m()))||"",commit:c=>_t(m(),"zone_ble_mac",c)});r.addEventListener("change",x);function g(){let c=m();u!==c?(er(a,c),u=c,b.style.display="none",d.discard()):d.refresh(),x()}function y(c){let z=m();(c===p.probe(z)||c===p.tempSource(z)||c===p.syncTo(z)||c===p.ble(z)||/^select-zone_\d+_sync_to$/.test(c))&&(d.refresh(),x())}l.addEventListener("click",()=>{if(l.disabled)return;l.disabled=!0,l.textContent="\u2026",b.style.display="",b.innerHTML='<div class="scan-msg">'+f("zone.sensor.scanning")+"</div>";let c=new AbortController,z=setTimeout(()=>c.abort(),8e3);fetch("/api/hv6/v1/ble-scan",{cache:"no-store",signal:c.signal}).then(C=>{if(!C.ok)throw new Error("HTTP "+C.status);return C.json()}).then(C=>{if(clearTimeout(z),l.disabled=!1,l.textContent=f("zone.sensor.scan"),!C.ok||!C.sensors||C.sensors.length===0){b.innerHTML='<div class="scan-msg">'+f("zone.sensor.noSensors")+"</div>";return}let k=m(),T=(S(p.ble(k))||"").toUpperCase(),A=P=>String(P).replace(/[&<>"']/g,q=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[q]),M="";for(let P of C.sensors){let q=P.mac.toUpperCase(),D=P.name?A(P.name):"",U=P.temp_c!=null?P.temp_c.toFixed(1)+"\xB0C":"\u2014",G=P.rssi!=null?P.rssi+" dBm":"",J=P.age_s<60?f("common.secondsAgo",{value:P.age_s}):f("common.minutesAgo",{value:Math.round(P.age_s/60)}),F="";q===T?F='<span class="ble-badge">'+f("zone.sensor.assignedThisZone")+"</span>":P.zone>0&&(F='<span class="ble-badge">'+f("zone.sensor.zoneBadge",{zone:P.zone})+"</span>");let Z=D?`<div class="ble-mac">${D}</div><div class="ble-meta">${q}</div>`:`<div class="ble-mac">${q}</div>`;M+=`<div class="ble-scan-item">
              <div>
                ${Z}
            ...
            </div>`}b.innerHTML=M,b.querySelectorAll(".btn-assign").forEach(P=>{P.addEventListener("click",()=>{n.value=P.dataset.mac,h.markDirty(),b.style.display="none"})})}).catch(C=>{clearTimeout(z),l.disabled=!1,l.textContent=f("zone.sensor.scan");let k=C&&C.name==="AbortError"?f("zone.sensor.scanTimeout"):f("zone.sensor.scanFailed");b.innerHTML='<div class="scan-msg">'+k+"</div>"})}),B("selectedZone",g);for(let c=1;c<=6;c++)w(p.probe(c),y),w(p.tempSource(c),y),w(p.syncTo(c),y),w(p.ble(c),y);_(e),g()}});var Fn=".zone-room-card { height: 100%; }";R("zone-room-card",Fn);var Tn=()=>`
...
`,fi=N({tag:"zone-room-card",render:Tn,onMount(t,e){let o=e.querySelector(".zr-friendly");function r(){return E("selectedZone")}let n=ie(e);n.text(o,{read:()=>ve(r())||"",commit:a=>lo(r(),a)}),B("selectedZone",n.discard),B("zoneNames",n.refresh),_(e),n.refresh()}});var pe=6,Nn="var(--flow-disabled)",rr="var(--flow-unknown)",nr="var(--accent)",Ve="var(--flow-return)",Ht="var(--text-strong)",Dn="var(--flow-disabled)",Ce="var(--flow-label)",mt="var(--flow-disabled)",qt="var(--flow-label)",ar="var(--flow-label)",or="var(--flow-return)",Rn="#66BB6A",On="#FF6361",V={w:1160,h:310,boxX:452,boxY:34,boxW:256,boxH:68,srcY:102,fanY:158,zoneY:232,zoneXs:[92,286,480,674,868,1062],srcSpread:15,bgDstHW:28,srcHW:7},I={w:760,h:340,boxX:38,boxY:132,boxW:142,boxH:72,srcX:180,endX:386,nameX:446,midY:168,zoneYs:[58,104,150,196,242,288],spread:8,bgDstHW:15,srcHW:4},In=`
.flow-wrap {
  width: 100%;
  border-radius: 8px;
  overflow: hidden;
...
`;R("flow-diagram",In);function Pn(t,e){let o=String(ve(t)||"").trim();if(!o)return"";let r=o.toUpperCase();return r.length>e?r.slice(0,Math.max(1,e-1))+"\u2026":r}function qn(t){if(!t)return null;let e=String(t).match(/(\d+)/);if(!e)return null;let o=Number(e[1]);return Number.isFinite(o)&&o>=1&&o<=8?o:null}function Hn(t,e){return e?t==null||Number.isNaN(t)?rr:t>0?nr:Ce:Nn}function sr(t){let e=t==="desktop"?"0 1":"1 0",o=[];o.push("<defs>");for(let r=1;r<=pe;r++)o.push('<linearGradient id="'+t+"-rg"+r+'" x1="0" y1="0" x2="'+e.split(" ")[0]+'" y2="'+e.split(" ")[1]+'">'),o.push('<stop id="'+t+"-rgs"+r+'" offset="0%" stop-color="var(--accent)" stop-opacity=".96"/>'),o.push('<stop id="'+t+"-rga"+r+'" offset="100%" stop-color="var(--accent)" stop-opacity=".7"/>'),o.push("</linearGradient>");return o.push("</defs>"),o.join("")}function Bn(t){let e=V.boxX+V.boxW/2+(t-2.5)*V.srcSpread,o=V.zoneXs[t];return"M"+e.toFixed(1)+" "+V.srcY+" C"+e.toFixed(1)+" "+V.fanY+" "+o.toFixed(1)+" "+(V.fanY+34)+" "+o.toFixed(1)+" "+(V.zoneY-20)}function $n(t){let e=I.midY+(t-2.5)*I.spread,o=I.zoneYs[t],r=I.endX-I.srcX;return"M"+I.srcX+" "+e.toFixed(1)+" C"+(I.srcX+r*.34)+" "+e.toFixed(1)+" "+(I.srcX+r*.7)+" "+o.toFixed(1)+" "+I.endX+" "+o.toFixed(1)}function ir(t,e,o){let r=V.boxX+V.boxW/2+(t-2.5)*V.srcSpread,n=V.srcY,a=V.zoneXs[t],i=V.zoneY-20,l=V.fanY,b=V.fanY+34;return"M"+(r-e).toFixed(1)+" "+n+" C"+(r-e).toFixed(1)+" "+l+" "+(a-o).toFixed(1)+" "+b+" "+(a-o).toFixed(1)+" "+i+" L"+(a+o).toFixed(1)+" "+i+" C"+(a+o).toFixed(1)+" "+b+" "+(r+e).toFixed(1)+" "+l+" "+(r+e).toFixed(1)+" "+n+"Z"}function lr(t,e,o){let r=I.midY+(t-2.5)*I.spread,n=I.zoneYs[t],a=I.endX-I.srcX,i=I.srcX+a*.34,l=I.srcX+a*.7;return"M"+I.srcX+" "+(r-e).toFixed(1)+" C"+i+" "+(r-e).toFixed(1)+" "+l+" "+(n-o).toFixed(1)+" "+I.endX+" "+(n-o).toFixed(1)+" L"+I.endX+" "+(n+o).toFixed(1)+" C"+l+" "+(n+o).toFixed(1)+" "+i+" "+(r+e).toFixed(1)+" "+I.srcX+" "+(r+e).toFixed(1)+"Z"}function cr(t,e,o){return'<rect width="'+t+'" height="'+e+'" rx="10" fill="var(--surface-raised)"/>'}function dr(t){let e=t==="desktop"?V:I,o=t==="desktop"?e.boxY+27:e.boxY+29,r=t==="desktop"?e.boxY+56:e.boxY+58;return'<rect x="'+e.boxX+'" y="'+e.boxY+'" width="'+e.boxW+'" height="'+e.boxH+'" rx="7" fill="var(--flow-source-bg)" stroke="var(--accent)" stroke-width="2"/><text id="'+t+'-fd-flow-label" x="'+(e.boxX+e.boxW/2)+'" y="'+o+'" text-anchor="middle" font-size="'+(t==="desktop"?20:19)+'" font-weight="800" fill="var(--accent)" letter-spacing="2">'+f("overview.flowDiagram.flow")+'</text><text id="'+t+'-fd-flow-temp" class="flow-metric" x="'+(e.boxX+e.boxW/2)+'" y="'+r+'" text-anchor="middle" font-size="'+(t==="desktop"?29:27)+'" fill="var(--text-strong)">---</text>'}function Vn(){let t=[],e=V.w,o=V.h,r=V.zoneY-20;t.push('<svg class="flow-svg flow-svg-desktop" viewBox="0 5 '+e+" "+(o-5)+'" preserveAspectRatio="xMidYMid meet" aria-hidden="true" focusable="false">'),t.push(sr("desktop")),t.push(cr(e,o,"desktop")),t.push(dr("desktop")),t.push('<text id="desktop-fd-ret-temp" x="'+(V.boxX+V.boxW+24)+'" y="'+(V.boxY+20)+'" font-size="17" font-weight="800" fill="'+Ve+'" font-family="var(--mono)">'+f("overview.flowDiagram.returnShort")+" ---</text>"),t.push('<text id="desktop-fd-dt-label" x="'+(V.boxX+V.boxW+24)+'" y="'+(V.boxY+42)+'" font-size="13" font-weight="800" fill="'+ar+'" letter-spacing="2">'+f("overview.flowDiagram.dt")+"</text>"),t.push('<text id="desktop-fd-dt" x="'+(V.boxX+V.boxW+24)+'" y="'+(V.boxY+66)+'" class="flow-metric" font-size="24" fill="var(--accent)">---</text>');for(let n=1;n<=pe;n++)t.push('<path id="desktop-fd-track-'+n+'" class="flow-track" d="'+Bn(n-1)+'" opacity=".7"/>');for(let n=1;n<=pe;n++)t.push('<path id="desktop-fd-path-'+n+'" class="flow-ribbon" d="'+ir(n-1,V.srcHW,V.bgDstHW)+'" fill="url(#desktop-rg'+n+')" opacity="1"/>');t.push('<line x1="54" y1="'+r+'" x2="'+(e-54)+'" y2="'+r+'" stroke="var(--flow-track)" stroke-width="2" opacity=".72"/>');for(let n=1;n<=pe;n++){let a=V.zoneXs[n-1];t.push('<g class="flow-zone-hit">'),t.push('<line id="desktop-fd-tick-'+n+'" x1="'+a+'" y1="'+(r-8)+'" x2="'+a+'" y2="'+(r+8)+'" stroke="var(--flow-track)" stroke-width="2"/>'),t.push('<text id="desktop-fd-zn'+n+'" x="'+a+'" y="'+(r-13)+'" text-anchor="middle" font-size="15" fill="'+Ht+'" font-weight="800" letter-spacing="1.5">Z'+n+"</text>"),t.push('<text id="desktop-fd-zf'+n+'" x="'+a+'" y="'+(r+21)+'" text-anchor="middle" font-size="11.5" fill="'+Ce+'" font-weight="700" letter-spacing=".55">---</text>'),t.push('<text id="desktop-fd-zsp'+n+'" x="'+a+'" y="'+(r+21)+'" text-anchor="middle" font-size="10.5" fill="'+mt+'" font-weight="600" font-family="var(--mono)"></text>'),t.push('<text id="desktop-fd-zt'+n+'" x="'+a+'" y="'+(r+44)+'" text-anchor="middle" class="flow-metric" font-size="17" fill="var(--text-strong)">---\xB0C</text>'),t.push('<text id="desktop-fd-zv'+n+'" x="'+(a-30)+'" y="'+(r+64)+'" text-anchor="middle" class="flow-metric" font-size="14" fill="'+Ce+'">---%</text>'),t.push('<text id="desktop-fd-zr'+n+'" x="'+(a+30)+'" y="'+(r+64)+'" text-anchor="middle" class="flow-metric" font-size="14" fill="'+Ve+'">---</text>'),t.push("</g>")}return t.push("</svg>"),t.join("")}function jn(){let t=[],e=I.w,o=I.h;t.push('<svg class="flow-svg flow-svg-mobile" viewBox="0 0 '+e+" "+o+'" preserveAspectRatio="xMidYMid meet" aria-hidden="true" focusable="false">'),t.push(sr("mobile")),t.push(cr(e,o,"mobile")),t.push(dr("mobile"));for(let r=1;r<=pe;r++)t.push('<path id="mobile-fd-track-'+r+'" class="flow-track" d="'+$n(r-1)+'" opacity=".7"/>');for(let r=1;r<=pe;r++)t.push('<path id="mobile-fd-path-'+r+'" class="flow-ribbon" d="'+lr(r-1,I.srcHW,I.bgDstHW)+'" fill="url(#mobile-rg'+r+')" opacity="1"/>');t.push('<rect x="'+(I.boxX+9)+'" y="'+(I.boxY+I.boxH+9)+'" width="'+(I.boxW-18)+'" height="60" rx="8" fill="var(--flow-source-bg)" stroke="var(--flow-return)" stroke-opacity=".7"/>'),t.push('<text id="mobile-fd-ret-temp" x="'+(I.boxX+I.boxW/2)+'" y="'+(I.boxY+I.boxH+27)+'" text-anchor="middle" font-size="14" font-weight="800" fill="'+Ve+'" font-family="var(--mono)">'+f("overview.flowDiagram.returnShort")+" ---</text>"),t.push('<text id="mobile-fd-dt-label" x="'+(I.boxX+I.boxW/2)+'" y="'+(I.boxY+I.boxH+43)+'" text-anchor="middle" font-size="11.5" font-weight="800" fill="'+ar+'" letter-spacing="1.1">'+f("overview.flowDiagram.dt")+"</text>"),t.push('<text id="mobile-fd-dt" x="'+(I.boxX+I.boxW/2)+'" y="'+(I.boxY+I.boxH+63)+'" text-anchor="middle" class="flow-metric" font-size="19" fill="var(--accent)">---</text>'),t.push('<line x1="'+I.endX+'" y1="34" x2="'+I.endX+'" y2="'+(o-34)+'" stroke="var(--flow-track)" stroke-width="2" opacity=".72"/>'),t.push('<text id="mobile-fd-temp-head" x="506" y="30" font-size="12" fill="'+qt+'" font-weight="700" letter-spacing="1.2">'+f("overview.graph.layers.temp").toUpperCase()+"</text>"),t.push('<text id="mobile-fd-flow-head" x="592" y="30" font-size="12" fill="'+qt+'" font-weight="700" letter-spacing="1.2">'+f("overview.flowDiagram.flow")+"</text>"),t.push('<text id="mobile-fd-ret-head" x="678" y="30" font-size="12" fill="'+qt+'" font-weight="700" letter-spacing="1.2">'+f("overview.flowDiagram.returnShort")+"</text>");for(let r=1;r<=pe;r++){let n=I.zoneYs[r-1];t.push('<line id="mobile-fd-tick-'+r+'" x1="'+(I.endX-8)+'" y1="'+n+'" x2="'+(I.endX+8)+'" y2="'+n+'" stroke="var(--flow-track)" stroke-width="2"/>'),t.push('<text id="mobile-fd-zn'+r+'" x="'+(I.endX-14)+'" y="'+(n+5)+'" text-anchor="end" font-size="14" fill="'+Ht+'" font-weight="800" letter-spacing="1.2">Z'+r+"</text>"),t.push('<text id="mobile-fd-zf'+r+'" x="'+I.nameX+'" y="'+(n-8)+'" text-anchor="middle" font-size="10.5" fill="'+Ce+'" font-weight="700" letter-spacing=".5">---</text>'),t.push('<text id="mobile-fd-zsp'+r+'" x="'+I.nameX+'" y="'+(n+8)+'" text-anchor="middle" font-size="10" fill="'+mt+'" font-weight="600" font-family="var(--mono)"></text>'),t.push('<text id="mobile-fd-zt'+r+'" x="506" y="'+(n+5)+'" class="flow-metric" font-size="15" fill="var(--text-strong)">---\xB0C</text>'),t.push('<text id="mobile-fd-zv'+r+'" x="592" y="'+(n+5)+'" class="flow-metric" font-size="15" fill="'+Ce+'">---%</text>'),t.push('<text id="mobile-fd-zr'+r+'" x="678" y="'+(n+5)+'" class="flow-metric" font-size="15" fill="'+Ve+'">---</text>')}return t.push("</svg>"),t.join("")}var Zn=()=>'<div class="flow-wrap" role="img" aria-label="'+f("overview.flowDiagram.flow")+'">'+Vn()+jn()+"</div>";N({tag:"flow-diagram",render:Zn,onMount(t,e){let o=["desktop","mobile"],r={};o.forEach(u=>{r[u]={flowEl:e.querySelector("#"+u+"-fd-flow-temp"),flowLabelEl:e.querySelector("#"+u+"-fd-flow-label"),retEl:e.querySelector("#"+u+"-fd-ret-temp"),dtLabelEl:e.querySelector("#"+u+"-fd-dt-label"),dtEl:e.querySelector("#"+u+"-fd-dt"),zones:new Array(pe+1)};for(let m=1;m<=pe;m++)r[u].zones[m]={textTemp:e.querySelector("#"+u+"-fd-zt"+m),textSetpoint:e.querySelector("#"+u+"-fd-zsp"+m),textFlow:e.querySelector("#"+u+"-fd-zv"+m),textRet:e.querySelector("#"+u+"-fd-zr"+m),label:e.querySelector("#"+u+"-fd-zn"+m),friendly:e.querySelector("#"+u+"-fd-zf"+m),track:e.querySelector("#"+u+"-fd-track-"+m),tick:e.querySelector("#"+u+"-fd-tick-"+m),path:e.querySelector("#"+u+"-fd-path-"+m)}});function n(u,m){u&&(u.textContent=m)}function a(u,m,x,d,h){let g=r[u];n(g.flowLabelEl,f("overview.flowDiagram.flow")),n(g.flowEl,Y(m)),n(g.retEl,f("overview.flowDiagram.returnShort")+" "+Y(x)),n(g.dtLabelEl,f("overview.flowDiagram.dt")),n(g.dtEl,d==null?"---":d.toFixed(1)+"\xB0C"),g.dtEl&&g.dtEl.setAttribute("fill",h)}function i(){n(e.querySelector("#mobile-fd-temp-head"),f("overview.graph.layers.temp").toUpperCase()),n(e.querySelector("#mobile-fd-flow-head"),f("overview.flowDiagram.flow")),n(e.querySelector("#mobile-fd-ret-head"),f("overview.flowDiagram.returnShort"))}function l(u,m,x){let d=r[u].zones[m];if(!d)return;let{enabled:h,pct:g,temp:y,setpoint:c,valve:z,returnTemp:C,hasReturn:k}=x,T=Pn(m,u==="desktop"?11:12),A=Y(y),M=c!=null?Y(c):"";n(d.label,"Z"+m),n(d.friendly,u==="desktop"?(T||"---")+(M?" ("+M+")":""):T||"---"),n(d.textTemp,A),n(d.textSetpoint,u==="desktop"?"":M?"("+M+")":""),n(d.textFlow,Te(z)),n(d.textRet,k?Y(C):"---"),d.label.setAttribute("fill",h?Ht:Dn),d.friendly.setAttribute("fill",h?Ce:mt),d.textSetpoint.setAttribute("fill",h?Ce:mt),d.textFlow.setAttribute("fill",Hn(g,h)),d.textRet.setAttribute("fill",k&&h?Ve:rr);let P=h&&g!=null&&g>0;d.track.setAttribute("opacity",h?".78":".38"),d.track.setAttribute("stroke-dasharray",h?"none":"5 7"),d.tick.setAttribute("stroke",P?nr:"var(--flow-track)"),d.tick.setAttribute("stroke-width",P?"3":"2");let q=d.path;if(!P)q.setAttribute("opacity","0");else{let D=u==="desktop"?V:I,U=Math.max(2.5,g*D.bgDstHW),G=Math.max(1.3,g*D.srcHW);q.setAttribute("d",u==="desktop"?ir(m-1,G,U):lr(m-1,G,U)),q.setAttribute("fill","url(#"+u+"-rg"+m+")"),q.setAttribute("opacity",".96")}}function b(){let u=L(s.flow),m=L(s.ret),x=u!=null&&m!=null?Number(u)-Number(m):null,d=x==null||x<3?or:x>8?On:Rn;o.forEach(h=>a(h,u,m,x,d));for(let h=1;h<=pe;h++){let g=L(p.temp(h)),y=L(p.setpoint(h)),c=L(p.valve(h)),z=X(p.enabled(h)),C=String(S(p.tempSource(h))||"Local Probe"),k=qn(S(p.probe(h))||""),T=k?L(p.probeTemp(k)):null,A=C!=="Local Probe"&&T!=null&&!Number.isNaN(Number(T)),M=c!=null?Math.max(0,Math.min(100,Number(c)))/100:null,P={enabled:z,pct:M,temp:g,setpoint:y,valve:c,returnTemp:T,hasReturn:A};o.forEach(q=>l(q,h,P))}}w(s.flow,b),w(s.ret,b),B("zoneNames",b);for(let u=1;u<=pe;u++)w(p.temp(u),b),w(p.setpoint(u),b),w(p.valve(u),b),w(p.enabled(u),b),w(p.probe(u),b),w(p.tempSource(u),b);for(let u=1;u<=8;u++)w(p.probeTemp(u),b);i(),b()}});var Un={1:{label:"E",color:"#ff6361"},2:{label:"W",color:"#ffd380"},3:{label:"I",color:"#79d17e"},4:{label:"C",color:"#7aa7ce"},5:{label:"D",color:"rgba(214,228,255,.7)"},6:{label:"V",color:"rgba(214,228,255,.5)"},7:{label:"VV",color:"rgba(214,228,255,.4)"}},Wn=`
.logs-view {
  background: var(--panel-bg-vibrant);
  border: 1px solid var(--panel-border);
  border-radius: 8px;
...
`;function Kn(t){let e=Un[t.level]||{label:"?",color:"var(--text-secondary)"},o=pr(t.tag||""),r=pr(t.msg||"");return'<div class="log-line"><span class="lv" style="color:'+e.color+'">'+e.label+'</span><span class="tag">'+o+'</span><span class="msg">'+r+"</span></div>"}function pr(t){return String(t).replace(/[&<>]/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;"})[e])}var Ai=N({tag:"logs-view",render:Gn,onMount(t,e){let o=e.querySelector(".logs-stream"),r=e.querySelector(".pause-btn"),n=e.querySelector(".clear-btn"),a=e.querySelector(".download-btn"),i=!1;function l(){if(i)return;let b=Ke();if(!b||!b.length){o.innerHTML='<div class="logs-empty">'+f("logs.waiting")+"</div>";return}let u=o.scrollHeight-o.scrollTop-o.clientHeight<40;o.innerHTML=b.map(Kn).join(""),u&&(o.scrollTop=o.scrollHeight)}r.addEventListener("click",()=>{i=!i,r.textContent=i?f("logs.resume"):f("logs.pause"),r.classList.toggle("on",i),i||l()}),n.addEventListener("click",()=>{Ut()}),a.addEventListener("click",()=>{a.disabled=!0,_o().catch(b=>{console.error("[Logs] download failed:",b),window.alert(f("logs.downloadFailed"))}).finally(()=>{a.disabled=!1})}),B("deviceLog",l),_(e),l()}});var Xn=`
.diag-i2c {
  background: var(--panel-bg-vibrant);
  border: 1px solid var(--panel-border);
  border-radius: 8px;
...
`,Ri=N({tag:"diag-i2c",render:Yn,onMount(t,e){let o=e.querySelector("#i2c-result");function r(){o.textContent=E("i2cResult")||f("diagnostics.i2c.empty")}e.querySelector("#btn-i2c-scan").addEventListener("click",()=>{ao()}),B("i2cResult",r),_(e),r()}});var Jn=`
.diag-manual-badge {
  display: none;
  align-items: center;
  gap: 10px;
...
`,Bi=N({tag:"diag-manual-badge",render:Qn,onMount(t,e){let o=e.classList.contains("diag-manual-badge")?e:e.querySelector(".diag-manual-badge");function r(){let n=!!E("manualMode");o&&o.classList.toggle("on",n)}B("manualMode",r),_(e),r()}});var ea=`
.diag-zone-motor {
  background: var(--panel-bg-vibrant);
  border: 1px solid var(--panel-border);
  border-radius: 8px;
...
`;R("diag-zone-motor",ea);var ta=t=>{let e=t.zone||E("selectedZone")||1,o="";for(let r=1;r<=6;r++)o+='<option value="'+r+'"'+(r===e?" selected":"")+">"+f("common.zone")+" "+r+"</option>";return`
  ...
  `},Ki=N({tag:"diag-zone-motor-card",render:ta,onMount(t,e){let o=Number(t.zone||E("selectedZone")||1),r=!!E("manualMode"),n=e.querySelector(".manual-mode-toggle"),a=e.querySelector(".motor-gated"),i=e.querySelector(".motor-zone-select"),l=e.querySelector(".motor-target-input"),b=e.querySelector(".motor-open-btn"),u=e.querySelector(".motor-close-btn"),m=e.querySelector(".motor-stop-btn"),x=()=>{let y=i.value||String(o),c="";for(let z=1;z<=6;z++)c+='<option value="'+z+'">'+f("common.zone")+" "+z+"</option>";i.innerHTML=c,i.value=y};function d(y){r=!!y,n&&(n.classList.toggle("on",r),n.setAttribute("aria-checked",r?"true":"false")),a&&a.classList.toggle("locked",!r),[i,l,b,u,m].forEach(c=>{c&&(c.disabled=!r)})}function h(){let y=!r;if(d(y),y){Lt(!0);for(let c=1;c<=6;c++)Ct(c)}else Lt(!1)}function g(){let y=L(p.motorTarget(o));l&&y!=null?l.value=Number(y).toFixed(0):l&&(l.value="0")}i==null||i.addEventListener("change",()=>{o=Number(i.value||1),g()}),n==null||n.addEventListener("click",h),n==null||n.addEventListener("keydown",y=>{y.key!==" "&&y.key!=="Enter"||(y.preventDefault(),h())});for(let y=1;y<=6;y++)w(p.motorTarget(y),g);g(),d(r),B("manualMode",()=>{d(!!E("manualMode"))}),_(e),l==null||l.addEventListener("change",y=>{if(!r)return;let c=y.target.value;co(o,c)}),b==null||b.addEventListener("click",()=>{r&&po(o,1e4)}),u==null||u.addEventListener("click",()=>{r&&uo(o,1e4)}),m==null||m.addEventListener("click",()=>{r&&Ct(o)})}});var oa=`
.diag-zone-recovery {
  background: var(--panel-bg-vibrant);
  border: 1px solid var(--panel-border);
  border-radius: 8px;
...
`;R("diag-zone-recovery",oa);var ra=()=>`
  ...
  `,ol=N({tag:"diag-zone-recovery-card",render:ra,onMount(t,e){let o=Number(E("selectedZone")||1),r=e.querySelector(".recovery-fault-btn"),n=e.querySelector(".recovery-factors-btn"),a=e.querySelector(".recovery-relearn-btn"),i=e.querySelector(".recovery-status");B("selectedZone",()=>{o=Number(E("selectedZone")||1)});let l=null;function b(m,x){i.textContent=m,i.className="recovery-status show "+(x?"ok":"err"),clearTimeout(l),l=setTimeout(()=>{i.classList.remove("show")},4e3)}function u(m,x){let d=m(o);b(x,!0),d&&typeof d.then=="function"&&d.then(h=>{h&&h.ok===!1&&b(f("diagnostics.recovery.rejected"),!1)}).catch(()=>b(f("diagnostics.recovery.unreachable"),!1))}r==null||r.addEventListener("click",()=>{u(mo,"\u2713 "+f("diagnostics.recovery.faultSent",{zone:ae(o)}))}),n==null||n.addEventListener("click",()=>{confirm(f("diagnostics.recovery.confirmFactors",{zone:ae(o)}))&&u(go,"\u2713 "+f("diagnostics.recovery.factorsReset",{zone:ae(o)}))}),a==null||a.addEventListener("click",()=>{confirm(f("diagnostics.recovery.confirmRelearn",{zone:ae(o)}))&&u(fo,"\u2713 "+f("diagnostics.recovery.relearnStarted",{zone:ae(o)}))}),_(e)}});var na=`
...
`,dl=N({tag:"diag-system-card",render:aa,onMount(t,e){let o=e.querySelector('[data-k="cpu0"]'),r=e.querySelector('[data-k="cpu1"]'),n=e.querySelector('[data-k="heap"]'),a=e.querySelector('[data-k="psram"]'),i=e.querySelector('[data-bar="cpu0"]'),l=e.querySelector('[data-bar="cpu1"]'),b=e.querySelector('[data-k="reset"]'),u=(d,h,g)=>{if(g==null||!Number.isFinite(Number(g))){d.textContent="\u2014",d.classList.remove("warn"),h.style.width="0%";return}let y=Math.max(0,Math.min(100,Number(g)));d.textContent=y.toFixed(0)+"%",d.classList.toggle("warn",y>=90),h.style.width=y+"%"},m=(d,h,g)=>{if(h==null||!Number.isFinite(Number(h))){d.textContent="\u2014";return}let y=Number(h);d.textContent=y+" KB",d.classList.toggle("warn",g!=null&&y<g)},x=()=>{u(o,i,L(s.cpuLoadCore0)),u(r,l,L(s.cpuLoadCore1)),m(n,L(s.freeInternalKb),48),m(a,L(s.freePsramKb),null);let d=String(S(s.resetReason)||E("resetReason")||"").trim();b.textContent=d||"\u2014"};e.querySelector(".sys-dump").addEventListener("click",()=>{bo().catch(d=>console.error("[System] dump failed:",d))}),w(s.cpuLoadCore0,x),w(s.cpuLoadCore1,x),w(s.freeInternalKb,x),w(s.freePsramKb,x),w(s.resetReason,x),B("resetReason",x),_(e),x()}});var sa=`
...
`;R("settings-manifold-card",sa);var ia=()=>{let t="";for(let o=1;o<=8;o++)t+="<option>Probe "+o+"</option>";let e="";for(let o=1;o<=8;o++)e+='<div class="probe-cell"><div class="probe-name">Probe '+o+'</div><div class="probe-temp" data-probe="'+o+'">---</div></div>';return`
  ...
  `},yl=N({tag:"settings-manifold-card",render:ia,onMount(t,e){let o=e.querySelector(".sm-type"),r=e.querySelector(".sm-flow"),n=e.querySelector(".sm-ret"),a=ie(e);a.select(o,{read:()=>S(s.manifoldType)||"NO (Normally Open)",commit:l=>le("manifold_type",l)}),a.select(r,{read:()=>S(s.manifoldFlowProbe)||"Probe 7",commit:l=>le("manifold_flow_probe",l)}),a.select(n,{read:()=>S(s.manifoldReturnProbe)||"Probe 8",commit:l=>le("manifold_return_probe",l)});function i(){for(let l=1;l<=8;l++){let b=e.querySelector('[data-probe="'+l+'"]');b&&(b.textContent=Y(L(p.probeTemp(l))))}}w(s.manifoldType,a.refresh),w(s.manifoldFlowProbe,a.refresh),w(s.manifoldReturnProbe,a.refresh);for(let l=1;l<=8;l++)w(p.probeTemp(l),i);_(e),a.refresh(),i()}});var la=`
...
`;R("settings-touch-card",la);var ca=()=>`
  ...
  </div>`,Ll=N({tag:"settings-touch-card",render:ca,onMount(t,e){let o=e.querySelector(".touch-status"),r=e.querySelector(".touch-status-copy"),n=e.querySelector(".touch-identity"),a=e.querySelector(".touch-note"),i=e.querySelector(".touch-error"),l=e.querySelector(".touch-approve"),b=e.querySelector(".touch-disconnect");function u(){let m=X(s.authorityConfigured),x=X(s.authorityProposalPending),d=S(s.authorityState)||"unconfigured",h=x?S(s.authorityProposalInstallationId):S(s.authorityInstallationId),g=x?S(s.authorityProposalCoordinatorId):S(s.authorityCoordinatorId),y=S(s.authorityProposalName)||"Lune Touch",c=S(s.authorityProposalSite)||"House";o.classList.toggle("connected",m&&!x),o.classList.toggle("pending",x),r.innerHTML=x?`<strong>${y} is ready to connect</strong>${m?"Approve it to replace the current Touch connection.":"Review the discovered coordinator, then approve it on this V6."}`:m?`<strong>Control approved</strong>${d.replace(/_/g," ")}${Number(L(s.authorityLeaseRemainingS))>0?` \xB7 ${Math.round(Number(L(s.authorityLeaseRemainingS)))} s lease`:""}`:"<strong>Waiting for Lune Touch</strong>Add this manifold in Lune Touch. Its identity will appear here automatically.",n.hidden=!m&&!x,e.querySelector(".touch-name").textContent=x?y:"Lune Touch",e.querySelector(".touch-site").textContent=x?c:"Approved coordinator",e.querySelector(".touch-installation-value").textContent=h||"\u2014",e.querySelector(".touch-coordinator-value").textContent=g||"\u2014",a.textContent=x?"Approval is local to this manifold. Discovery alone never grants control.":m?"V6 accepts authenticated commands from this Touch while retaining local safety, clamp, and expiry.":"Installation identity and authentication are generated and transferred automatically. There are no connection fields to complete.",l.hidden=!x,b.hidden=!m||x}l.addEventListener("click",async()=>{i.textContent="",l.disabled=!0,l.textContent="Approving\u2026";try{await so()}catch(m){i.textContent=(m==null?void 0:m.message)||"Unable to approve Lune Touch."}finally{l.disabled=!1,l.textContent="Approve Lune Touch"}}),b.addEventListener("click",async()=>{if(i.textContent="",!!window.confirm("Disconnect Lune Touch? Touch commands will be rejected until it is approved again.")){b.disabled=!0;try{await io()}catch(m){i.textContent=(m==null?void 0:m.message)||"Unable to disconnect Lune Touch."}finally{b.disabled=!1}}}),[s.authorityConfigured,s.authorityInstallationId,s.authorityCoordinatorId,s.authorityState,s.authorityLeaseRemainingS,s.authorityProposalPending,s.authorityProposalInstallationId,s.authorityProposalCoordinatorId,s.authorityProposalName,s.authorityProposalSite].forEach(m=>w(m,u)),u()}});var da=()=>`
...
`,Rl=N({tag:"settings-minimum-flow-card",render:da,onMount(t,e){let o=e.querySelector(".smf-always"),r=e.querySelector(".smf-pct"),n=e.querySelector(".smf-pct-row"),a=ie(e),i=l=>{n.hidden=!l,n.setAttribute("aria-hidden",l?"false":"true"),r.disabled=!l};a.toggle(o,{read:()=>X(s.minimumFlowAlways),onChange:i,commit:l=>{let b=l?"on":"off";v(s.minimumFlowAlways,{state:b}),le("minimum_flow_always",b).catch(()=>v(s.minimumFlowAlways,{state:l?"off":"on"}))}}),a.num(r,{read:()=>L(s.minZoneFlowPct),commit:l=>{v(s.minZoneFlowPct,{value:l}),ce("min_zone_flow_pct",l)}}),w(s.minimumFlowAlways,a.refresh),w(s.minZoneFlowPct,a.refresh),_(e),a.refresh()}});var pa=[{value:"15",labelKey:"settings.bleClock.interval15"},{value:"60",labelKey:"settings.bleClock.interval60"},{value:"360",labelKey:"settings.bleClock.interval360"},{value:"1440",labelKey:"settings.bleClock.interval1440"}];function ua(){if(String(S(s.bleClockSyncAdvertising)||"").toLowerCase()==="on")return f("common.clockSyncing");let t=String(S(s.bleClockSyncLastError)||"").trim();if(t==="clock_invalid")return f("settings.bleClock.waitingClock");if(t==="ble_busy")return f("settings.bleClock.busy");if(t)return t;let e=Number(L(s.bleClockSyncLastOkS)||0);if(!e)return f("settings.bleClock.never");let o=Math.max(0,Math.round(Date.now()/1e3)-e);if(o<60)return f("common.secondsAgo",{value:o});if(o<3600)return f("common.minutesAgo",{value:Math.round(o/60)});let r=Math.round(o/3600);return f("settings.bleClock.hoursAgo",{value:r})}var ma=()=>`
...
`,Vl=N({tag:"settings-ble-clock-card",render:ma,onMount(t,e){let o=e.querySelector(".sbc-enabled"),r=e.querySelector(".sbc-interval"),n=e.querySelector(".sbc-status"),a=e.querySelector(".sbc-now"),i=ie(e),l=()=>{let u=r.value;r.innerHTML=pa.map(m=>`<option value="${m.value}">${f(m.labelKey)}</option>`).join(""),u&&(r.value=u)},b=()=>{n.textContent=ua()};l(),i.toggle(o,{read:()=>X(s.bleClockSyncEnabled),commit:u=>{let m=u?"on":"off";v(s.bleClockSyncEnabled,{state:m}),le("ble_clock_sync_enabled",m).catch(()=>v(s.bleClockSyncEnabled,{state:u?"off":"on"}))}}),i.select(r,{read:()=>String(Math.round(Number(L(s.bleClockSyncIntervalMin))||60)),commit:u=>{let m=Number(u);v(s.bleClockSyncIntervalMin,{value:m}),ce("ble_clock_sync_interval_min",m)}}),a.addEventListener("click",()=>{v(s.bleClockSyncAdvertising,{state:"on"}),b(),se("ble_clock_sync_now")}),w(s.bleClockSyncEnabled,i.refresh),w(s.bleClockSyncIntervalMin,i.refresh),w(s.bleClockSyncLastOkS,b),w(s.bleClockSyncLastError,b),w(s.bleClockSyncAdvertising,b),_(e),i.refresh(),b()}});var ga=`
...
`;R("settings-control-card",ga);var fa=()=>`
...
`,Kl=N({tag:"settings-control-card",render:fa,onMount(t,e){_(e),e.querySelector(".sc-reset-probe-map").addEventListener("click",()=>{window.confirm("Reset the 1-Wire probe map and restart V6? Probe assignments must be discovered again.")&&se("reset_1wire_probe_map_reboot")}),e.querySelector(".sc-dump-1wire").addEventListener("click",()=>{se("dump_1wire_probe_diagnostics")}),e.querySelector(".sc-restart").addEventListener("click",()=>{window.confirm("Restart Lune V6 now? Heating continues after the controller has started again.")&&se("restart")})}});var ba=`
...
`;R("settings-motor-calibration-card",ba);var gt=[{cls:"safe-runtime",key:"generic_runtime_limit_seconds",id:s.genericRuntimeLimitSeconds,labelKey:"settings.motor.maxSafeRuntime",unit:"s"},{cls:"close-threshold",key:"close_threshold_multiplier",id:s.closeThresholdMultiplier,labelKey:"settings.motor.closeThreshold",unit:"x"},{cls:"close-slope-threshold",key:"close_slope_threshold",id:s.closeSlopeThreshold,labelKey:"settings.motor.closeSlope",unit:"mA/s"},{cls:"close-slope-floor",key:"close_slope_current_factor",id:s.closeSlopeCurrentFactor,labelKey:"settings.motor.closeSlopeFloor",unit:"x"},{cls:"open-threshold",key:"open_threshold_multiplier",id:s.openThresholdMultiplier,labelKey:"settings.motor.openThreshold",unit:"x"},{cls:"open-slope-threshold",key:"open_slope_threshold",id:s.openSlopeThreshold,labelKey:"settings.motor.openSlope",unit:"mA/s"},{cls:"open-slope-floor",key:"open_slope_current_factor",id:s.openSlopeCurrentFactor,labelKey:"settings.motor.openSlopeFloor",unit:"x"},{cls:"open-ripple-limit",key:"open_ripple_limit_factor",id:s.openRippleLimitFactor,labelKey:"settings.motor.openRippleLimit",unit:"x"},{cls:"relearn-movements",key:"relearn_after_movements",id:s.relearnAfterMovements,labelKey:"settings.motor.relearnMovements",unit:"count"},{cls:"relearn-hours",key:"relearn_after_hours",id:s.relearnAfterHours,labelKey:"settings.motor.relearnHours",unit:"h"},{cls:"learn-min-samples",key:"learned_factor_min_samples",id:s.learnedFactorMinSamples,labelKey:"settings.motor.learnMinSamples",unit:"count"},{cls:"learn-max-deviation",key:"learned_factor_max_deviation_pct",id:s.learnedFactorMaxDeviationPct,labelKey:"settings.motor.learnMaxDeviation",unit:"%"}],va=()=>{let t="";for(let e=0;e<gt.length;e++){let o=gt[e];if(o.key==="generic_runtime_limit_seconds")continue;let r=ha(o.key)?"1":"0.1";t+='<div class="ui-row"><span class="ui-label"><span data-i18n="'+o.labelKey+'">'+f(o.labelKey)+"</span> ("+o.unit+')</span><span class="ui-field"><input type="number" class="ui-input smc-'+o.cls+'" value="0" step="'+r+'"></span></div>'}return`
  ...
  `};function ha(t){return t==="learned_factor_min_samples"||t==="generic_runtime_limit_seconds"||t==="relearn_after_movements"||t==="relearn_after_hours"}var nc=N({tag:"settings-motor-calibration-card",render:va,onMount(t,e){let o=e.querySelector(".smc-profile"),r=e.querySelector(".smc-safe-runtime"),n=e.querySelector(".mc-drivers-toggle"),a=ie(e);function i(b){if(b==="HmIP VdMot"&&ce("hmip_runtime_limit_seconds",40),b==="Generic"){let u=Number(L(s.genericRuntimeLimitSeconds));(!Number.isFinite(u)||u<=0)&&ce("generic_runtime_limit_seconds",45)}}a.toggle(n,{read:()=>X(s.drivers),commit:b=>no(b)}),a.select(o,{read:()=>S(s.motorProfileDefault)||"HmIP VdMot",commit:b=>{le("motor_profile_default",b),i(b)}});function l(){let b=S(s.motorProfileDefault)||"HmIP VdMot";r.disabled=b==="HmIP VdMot"}a.num(r,{read:()=>(S(s.motorProfileDefault)||"HmIP VdMot")==="HmIP VdMot"?40:L(s.genericRuntimeLimitSeconds),commit:b=>{o.value==="Generic"&&ce("generic_runtime_limit_seconds",b)}});for(let b=0;b<gt.length;b++){let u=gt[b];if(u.key==="generic_runtime_limit_seconds")continue;let m=e.querySelector(".smc-"+u.cls);m&&(a.num(m,{read:()=>L(u.id),commit:x=>ce(u.key,x)}),w(u.id,a.refresh))}w(s.drivers,a.refresh),w(s.motorProfileDefault,()=>{a.refresh(),l()}),w(s.genericRuntimeLimitSeconds,a.refresh),w(s.hmipRuntimeLimitSeconds,a.refresh),_(e),i(S(s.motorProfileDefault)||"HmIP VdMot"),a.refresh(),l()}});var xa=600*1e3,ur=600,ya=`
...
`;R("settings-firmware-card",ya);var wa=()=>`
...
`;function mr(t){let e=String(t||"").trim().replace(/^v/i,"").match(/^(\d+)\.(\d+)\.(\d+)/);return e?[Number(e[1]),Number(e[2]),Number(e[3])]:null}function je(t,e){let o=mr(t);if(!o)return!1;let r=mr(e);if(!r)return!0;for(let n=0;n<3;n++)if(o[n]!==r[n])return o[n]>r[n];return!1}function gr(t){return String(t).replace(/[&<>]/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;"})[e])}function za(t){let e=String(t||"").trim();return e.length<=ur?e:e.slice(0,ur).replace(/\s+\S*$/,"")+"\u2026"}function ka(){let t=document.querySelector(".settings-backup-card");if(!t)return;let e=t.closest("details");e&&(e.open=!0),t.scrollIntoView({behavior:"smooth",block:"center"});let o=t.querySelector(".sbk-save");o&&o.focus({preventScroll:!0})}var mc=N({tag:"settings-firmware-card",render:wa,onMount(t,e){let o=e.querySelector(".sfw-version"),r=e.querySelector(".sfw-status"),n=e.querySelector(".sfw-check"),a=e.querySelector(".sfw-banner"),i=e.querySelector(".sfw-hop"),l=e.querySelector(".sfw-notes"),b=e.querySelector(".sfw-install"),u=e.querySelector(".sfw-asset"),m=e.querySelector(".sfw-jump"),x=e.querySelector(".sfw-file"),d=e.querySelector(".sfw-choose"),h=e.querySelector(".sfw-upload"),g=e.querySelector(".sfw-filename"),y=e.querySelector(".sfw-progress"),c=y.querySelector("i"),z=e.querySelector(".sfw-upload-status"),C=null,k=!1,T=0,A=!1,M=()=>S(s.firmware)||E("firmwareVersion")||"",P=(F,Z)=>{r.textContent=F||"",r.className="ui-sublabel sfw-status"+(Z?" "+Z:"")},q=()=>{let F=Ae.firmware_update;if(!F||F.available!==!0)return null;let Z=String(F.latest||"").trim();return Z?{tag:Z,notes:f("settings.firmware.deviceReported"),asset:Et(Z)}:null},D=()=>{let F=q();return C?F&&je(F.tag,C.tag)?F:C:F},U=()=>{o.textContent=M()||f("settings.firmware.unknownVersion")},G=()=>{let F=D(),Z=!!F&&je(F.tag,M());if(a.hidden=!Z,!Z){ge("firmwareUpdateAvailable",null);return}i.innerHTML=gr(M()||f("settings.firmware.unknownVersion"))+" <span>\u2192</span> "+gr(F.tag),l.textContent=za(F.notes)||f("common.noData"),u.href=F.asset.url,u.setAttribute("download",F.asset.name),u.title=F.asset.name,ge("firmwareUpdateAvailable",{current:M(),latest:F.tag,url:F.asset.url})},J=F=>{k||!F&&T&&Date.now()-T<xa||(k=!0,T=Date.now(),n.disabled=!0,P(f("settings.firmware.checking")),Promise.resolve(vo()).catch(()=>{}),yo().then(Z=>{C=Z,G();let ue=je(Z.tag,M());P(ue?f("settings.firmware.availableStatus",{version:Z.tag}):f("settings.firmware.upToDate"),ue?null:"ok")}).catch(Z=>{C=null,G();let ue=q();if(ue){P(je(ue.tag,M())?f("settings.firmware.availableStatus",{version:ue.tag}):f("settings.firmware.upToDate"),je(ue.tag,M())?null:"ok");return}if((Z instanceof he?Z.code:"network")==="no_releases"){P(f("settings.firmware.noReleases"),"ok");return}P(f("settings.firmware.checkFailed"),"err")}).finally(()=>{k=!1,n.disabled=!1}))};n.addEventListener("click",()=>J(!0)),m.addEventListener("click",ka),b.addEventListener("click",()=>{let F=D();F&&window.confirm(f("settings.firmware.confirmInstall",{version:F.tag}))&&(b.disabled=!0,b.textContent=f("settings.firmware.installing"),Promise.resolve(ho()).then(()=>P(f("settings.firmware.installStarted"))).catch(()=>{P(f("settings.firmware.installFailed"),"err"),b.disabled=!1,b.textContent=f("settings.firmware.install")}))}),d.addEventListener("click",()=>x.click()),x.addEventListener("change",()=>{let F=x.files&&x.files[0];g.textContent=F?F.name:f("settings.firmware.noFile"),h.disabled=!F||A,z.textContent="",z.className="ui-note sfw-upload-status"}),h.addEventListener("click",()=>{let F=x.files&&x.files[0];!F||A||window.confirm(f("settings.firmware.confirmUpload",{file:F.name}))&&(A=!0,h.disabled=!0,d.disabled=!0,y.hidden=!1,c.style.width="0%",z.className="ui-note sfw-upload-status",z.textContent=f("settings.firmware.uploading",{value:0}),Promise.resolve(xo()).catch(Z=>console.warn("[Firmware] prepare rejected, continuing with upload:",Z)).then(()=>wo(F,Z=>{c.style.width=Z+"%",z.textContent=f("settings.firmware.uploading",{value:Z})})).then(()=>{c.style.width="100%",z.className="ui-note sfw-upload-status",z.textContent=f("settings.firmware.uploadDone")}).catch(Z=>{console.error("[Firmware] upload failed:",Z),y.hidden=!0,z.textContent=f("settings.firmware.uploadFailed")}).finally(()=>{A=!1,d.disabled=!1,h.disabled=!1}))}),B("section",()=>{E("section")==="settings"&&J(!1)}),w(s.firmware,()=>{U(),G()}),w("firmware_update",G),_(e),U(),E("section")==="settings"&&J(!1)}});var Sa=`
...
`;R("settings-backup-card",Sa);var _a=()=>`
...
`,yc=N({tag:"settings-backup-card",render:_a,onMount(t,e){let o=e.querySelector(".sbk-save"),r=e.querySelector(".sbk-learned"),n=e.querySelector(".sbk-file"),a=e.querySelector(".sbk-choose"),i=e.querySelector(".sbk-restore"),l=e.querySelector(".sbk-filename"),b=e.querySelector(".sbk-status"),u=e.querySelector(".sbk-result"),m=!0,x=!1,d=(h,g)=>{b.textContent=h||"",b.className="sbk-status"+(g?" "+g:"")};r.addEventListener("click",()=>{m=!m,r.classList.toggle("on",m),r.setAttribute("aria-checked",m?"true":"false")}),o.addEventListener("click",()=>{x||(x=!0,o.disabled=!0,u.textContent="",d(f("settings.backup.saving")),zo(!0).then(h=>{if(!tt(h))throw new Error("unexpected_export_payload");d(f("settings.backup.saved",{file:So(h)}),"ok")}).catch(h=>{console.error("[Backup] export failed:",h),d(f("settings.backup.saveFailed"),"err")}).finally(()=>{x=!1,o.disabled=!1}))}),a.addEventListener("click",()=>n.click()),n.addEventListener("change",()=>{let h=n.files&&n.files[0];l.textContent=h?h.name:f("settings.backup.noFile"),i.disabled=!h||x,u.textContent="",d("")}),i.addEventListener("click",async()=>{let h=n.files&&n.files[0];if(!h||x)return;let g="";try{g=await h.text()}catch(c){d(f("settings.backup.readFailed"),"err");return}let y=null;try{y=JSON.parse(g)}catch(c){d(f("settings.backup.invalidFile"),"err");return}if(!tt(y)){d(f("settings.backup.invalidFile"),"err");return}window.confirm(f("settings.backup.confirmRestore",{file:h.name}))&&(x=!0,i.disabled=!0,u.textContent="",d(f("settings.backup.restoring")),ko(y,m).then(c=>{d(f("settings.backup.restored"),"ok"),u.textContent=f("settings.backup.result",{applied:c.applied,skipped:c.skipped,ignored:c.ignored})}).catch(c=>{console.error("[Backup] restore failed:",c),d(f("settings.backup.restoreFailed"),"err")}).finally(()=>{x=!1,i.disabled=!1}))}),_(e)}});var Le=Object.freeze({refinedEmber:"refined-ember",deepForest:"deep-forest"}),br="lune-dashboard-theme",vr="(prefers-color-scheme: dark)",Re=null,fr=!1;function hr(t){return Object.values(Le).includes(t)?t:Le.refinedEmber}function ft(){try{return hr(localStorage.getItem(br))}catch(t){return Le.refinedEmber}}function Ca(){return typeof window=="undefined"||typeof window.matchMedia!="function"||window.matchMedia(vr).matches?"dark":"light"}function La(){let t=Ca();if(typeof document=="undefined")return t;let e=document.documentElement;if(e.dataset.colorScheme=t,e.style.colorScheme=t,!fr&&typeof window!="undefined"&&typeof window.matchMedia=="function"){Re=window.matchMedia(vr);let o=()=>{let r=Re.matches?"dark":"light";e.dataset.colorScheme=r,e.style.colorScheme=r,window.dispatchEvent(new CustomEvent("lune-color-scheme-change",{detail:r}))};typeof Re.addEventListener=="function"?Re.addEventListener("change",o):typeof Re.addListener=="function"&&Re.addListener(o),fr=!0}return t}function Bt(t=ft()){let e=hr(t);if(typeof document=="undefined")return e;La();let o=document.documentElement;return Object.values(Le).forEach(r=>o.classList.remove(`theme-${r}`)),o.classList.add(`theme-${e}`),o.dataset.theme=e,e}function xr(t){let e=Bt(t);try{localStorage.setItem(br,e)}catch(o){}return typeof window!="undefined"&&window.dispatchEvent(new CustomEvent("lune-theme-change",{detail:e})),e}var Aa=[{value:Le.refinedEmber,labelKey:"settings.appearance.refinedEmber"},{value:Le.deepForest,labelKey:"settings.appearance.deepForest"}],Ea=()=>`
...
`,Lc=N({tag:"settings-appearance-card",render:Ea,onMount(t,e){let o=e.querySelector(".sap-theme"),r=()=>{let n=o.value||ft();o.innerHTML=Aa.map(a=>`<option value="${a.value}">${f(a.labelKey)}</option>`).join(""),o.value=n};r(),o.value=ft(),o.addEventListener("change",()=>xr(o.value)),window.addEventListener("lune-theme-change",n=>{n.detail&&(o.value=n.detail)}),_(e)}});var Ma=`
...
`;R("smart-preheat-card",Ma);var Fa=()=>`
...
`,Oc=N({tag:"smart-preheat-card",render:Fa,onMount(t,e){let o=e.querySelector(".absorb-toggle"),r=e.querySelector(".absorb-badge"),n=e.querySelector(".absorb-band"),a=e.querySelector(".absorb-delta"),i=e.querySelector(".absorb-body"),l=ie(e),b=m=>{i&&i.classList.toggle("is-disabled",!m)};l.toggle(o,{read:()=>X(s.preheatAbsorbEnabled),onChange:b,commit:m=>{let x=m?"on":"off";v(s.preheatAbsorbEnabled,{state:x}),le("preheat_absorb_enabled",x)}}),l.num(n,{read:()=>L(s.preheatAbsorbBandC),commit:m=>{v(s.preheatAbsorbBandC,{value:m}),ce("preheat_absorb_band_c",m)}}),l.num(a,{read:()=>L(s.preheatDetectDeltaC),commit:m=>{v(s.preheatDetectDeltaC,{value:m}),ce("preheat_detect_delta_c",m)}});function u(){let m=String(S(s.preheatAbsorbing)||"").toLowerCase()==="active";r.textContent=m?f("common.active"):f("common.idle"),r.classList.toggle("active",m)}w(s.preheatAbsorbEnabled,l.refresh),w(s.preheatAbsorbing,u),w(s.preheatAbsorbBandC,l.refresh),w(s.preheatDetectDeltaC,l.refresh),_(e),l.refresh(),u()}});Bt();var Ta=`
...
<div class="ftr">Lune V6 \xB7 Local manifold controller</div></main></div></div></div>`;N({tag:"app-root",render:Na,onMount(t,e){e.querySelector(".hdr").appendChild(j("hv6-header")),e.querySelector(".side-nav-slot").appendChild(j("hv6-sidebar")),e.querySelector(".zones-list").appendChild(j("zone-grid",{selection:!0,navigate:!0})),e.querySelector(".flow-diagram-slot").appendChild(j("flow-diagram")),e.querySelector(".hydraulic-history-slot").appendChild(j("graph-widgets",{variant:"flow-return"})),e.querySelector(".timeline-slot").appendChild(j("zone-state-timeline")),e.querySelector(".connectivity-slot").appendChild(j("connectivity-card")),e.querySelector(".zone-detail-slot").appendChild(j("zone-detail",{zone:E("selectedZone")})),e.querySelector(".zone-sensor-slot").appendChild(j("zone-sensor-card")),e.querySelector(".zone-recovery-slot").appendChild(j("diag-zone-recovery-card")),e.querySelector(".zone-room-slot").appendChild(j("zone-room-card")),e.querySelector(".touch-slot").appendChild(j("settings-touch-card")),e.querySelector(".manifold-slot").appendChild(j("settings-manifold-card")),e.querySelector(".minimum-flow-slot").appendChild(j("settings-minimum-flow-card")),e.querySelector(".ble-clock-slot").appendChild(j("settings-ble-clock-card")),e.querySelector(".preheat-slot").appendChild(j("smart-preheat-card")),e.querySelector(".motor-slot").appendChild(j("settings-motor-calibration-card")),e.querySelector(".firmware-slot").appendChild(j("settings-firmware-card")),e.querySelector(".backup-slot").appendChild(j("settings-backup-card")),e.querySelector(".appearance-slot").appendChild(j("settings-appearance-card")),e.querySelector(".diag-actions-slot").appendChild(j("settings-control-card")),e.querySelector(".manual-control-col").appendChild(j("diag-manual-badge")),e.querySelector(".manual-control-col").appendChild(j("diag-zone-motor-card",{zone:E("selectedZone")||1})),e.querySelector(".logs-main-col").appendChild(j("logs-view")),e.querySelector(".system-health-slot").appendChild(j("diag-system-card")),e.querySelector(".diag-health-slot").appendChild(j("connectivity-card")),e.querySelector(".diag-health-slot").appendChild(j("diag-i2c"));let o=e.querySelectorAll(".sec"),r=e.querySelector(".zones-index"),n=e.querySelector(".zone-detail-view"),a=e.querySelector(".selected-zone-title"),i=e.querySelector(".zone-tabstrip"),l=!1;function b(){let d=E("selectedZone")||1;i.innerHTML=Array.from({length:6},(h,g)=>{let y=g+1,c=y===d;return`<button type="button" class="zone-tab" id="${c?"selected-zone-tab":"zone-tab-"+y}" role="tab" aria-controls="selected-zone-panel" aria-selected="${c}" tabindex="${c?"0":"-1"}" data-zone-select="${y}">${ae(y)}</button>`}).join("")}function u(){let d=E("section")||"overview";o.forEach(h=>h.classList.toggle("active",h.dataset.section===d)),x()}function m(){let d=[],h=0,g=0;for(let M=1;M<=6;M++){let P=String(S(p.enabled(M))).toLowerCase()==="on",q=String(S(p.state(M))).toLowerCase(),D=String(S(p.motorLastFault(M))).toLowerCase();P&&d.push(M),P&&["heating","calling"].includes(q)&&h++,(q==="fault"||D!==""&&D!=="none"&&D!=="ok")&&g++}let y=L(s.flow),c=L(s.ret),z=String(S(s.authorityState)||"").replace(/_/g," "),C=g===0&&E("live"),k=`<div class="status-summary-main"><span class="eyebrow">System status</span><h2 class="${C?"status-ok":E("live")?"status-warn":"status-danger"}">${C?"Operating normally":E("live")?"Needs attention":"Device offline"}</h2><p>${g?g+" zone fault"+(g===1?"":"s")+" require attention.":E("live")?"V6 is running local control safely.":"Unable to read current manifold state."}</p></div><div class="status-fact"><span class="eyebrow">Heating</span><strong>${h} zones</strong><small>${d.length} enabled</small></div><div class="status-fact"><span class="eyebrow">Flow</span><strong>${Y(y)}</strong><small>Return ${Y(c)}</small></div><div class="status-fact"><span class="eyebrow">Touch</span><strong>${z||"not connected"}</strong><small>${L(s.authorityLeaseRemainingS)?Math.round(L(s.authorityLeaseRemainingS))+" s lease":"local control"}</small></div>`,T=X(s.authorityConfigured),A=String(S(s.drivers)||"off");e.querySelector(".overview-status").innerHTML=k,e.querySelector(".hydraulic-summary").textContent=`${h} heating \xB7 Flow ${Y(y)} \xB7 Return ${Y(c)}`,e.querySelector(".settings-readiness").innerHTML=`<div class="status-summary-main"><span class="eyebrow">Configuration</span><h2 class="${E("live")?"status-ok":"status-danger"}">${E("live")?"Ready":"Waiting for device"}</h2><p>V6 validates and saves changes locally.</p></div><div class="status-fact"><span class="eyebrow">Device</span><strong>${E("live")?"Live":"Offline"}</strong><small>local controller</small></div><div class="status-fact"><span class="eyebrow">Touch</span><strong>${T?"Approved":"Not approved"}</strong><small>${T?"authenticated control":"local control only"}</small></div><div class="status-fact"><span class="eyebrow">Drivers</span><strong>${A}</strong><small>motor outputs</small></div>`,e.querySelector(".diagnostics-readiness").innerHTML=`<div class="status-summary-main"><span class="eyebrow">Overall health</span><h2 class="${g?"status-danger":C?"status-ok":"status-warn"}">${g?g+" issue"+(g===1?"":"s"):C?"Healthy":"Awaiting data"}</h2><p>${g?"Resolve current exceptions before using service controls.":"No active motor faults reported."}</p></div><div class="status-fact"><span class="eyebrow">Zone faults</span><strong>${g}</strong><small>${g?"requires review":"none reported"}</small></div><div class="status-fact"><span class="eyebrow">Drivers</span><strong>${A}</strong><small>motor outputs</small></div><div class="status-fact"><span class="eyebrow">Touch</span><strong>${z||"not connected"}</strong><small>${T?"approved":"local control"}</small></div>`,e.querySelector(".zones-count").textContent=`6 physical loops \xB7 ${d.length} enabled \xB7 ${h} heating`,e.querySelector(".zones-summary").innerHTML=`<span class="eyebrow">Zone status</span><h2>${g?g+" zone"+(g===1?"":"s")+" need attention":d.length?h?h+" zone"+(h===1?" is":"s are")+" heating":"All enabled zones are idle":"No zones enabled"}</h2><p>${g?"Open an affected zone to review its valve, sensor and recovery state.":d.length?"Select a zone to review its applied target, sensor coverage and local fallback.":"Enable zones after their valve and temperature source are configured."}</p>`,[e.querySelector(".overview-attention"),e.querySelector(".diagnostics-attention")].forEach(M=>{M.hidden=!g,M.innerHTML=g?`<strong>Review ${g} zone fault${g===1?"":"s"}</strong><span>Open Zones to inspect the affected valve and sensor state.</span>`:""})}function x(){let d=E("selectedZone")||1,h=E("section")==="zones";a.textContent=ae(d),b(),r.hidden=!h||l,n.hidden=!h||!l}e.addEventListener("zone-open",()=>{l=!0,x()}),e.querySelector("[data-zone-back]").addEventListener("click",()=>{l=!1,x();let d=e.querySelector(`.zones-list .zone-card[data-zone="${E("selectedZone")||1}"]`);d&&d.focus()}),i.addEventListener("click",d=>{let h=d.target.closest("[data-zone-select]");h&&Oe(Number(h.dataset.zoneSelect))}),i.addEventListener("keydown",d=>{if(!["ArrowLeft","ArrowRight","Home","End"].includes(d.key))return;d.preventDefault();let h=E("selectedZone")||1,g=d.key==="Home"?1:d.key==="End"?6:d.key==="ArrowLeft"?h===1?6:h-1:h===6?1:h+1;Oe(g),requestAnimationFrame(()=>{var y;return(y=i.querySelector(`[data-zone-select="${g}"]`))==null?void 0:y.focus()})}),e.querySelectorAll("[data-open-zones]").forEach(d=>d.addEventListener("click",()=>{l=!1,fe("zones")})),e.querySelectorAll("[data-help-section]").forEach(d=>d.addEventListener("click",h=>{h.preventDefault(),fe(d.dataset.helpSection)})),B("section",u),B("selectedZone",x),B("live",m),B("zoneNames",()=>{b(),m()});for(let d=1;d<=6;d++)[p.temp(d),p.setpoint(d),p.valve(d),p.state(d),p.enabled(d),p.motorLastFault(d)].forEach(h=>w(h,m));[s.flow,s.ret,s.authorityConfigured,s.authorityState,s.authorityLeaseRemainingS,s.drivers].forEach(d=>w(d,m)),_(e),u(),x(),m()}});function Da(){let t=document.getElementById("app");if(!t)throw new Error("Dashboard root #app not found");t.innerHTML="",t.appendChild(j("app-root")),Fo()}Da();})();
