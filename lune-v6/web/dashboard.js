...
`;O("ui-kit",Fn);function ve(t){let e=d(t);return`<span class="help-badge" tabindex="0" role="img" aria-label="${String(e).replace(/"/g,"&quot;")}" data-i18n-label="${t}">?<span class="help-tip" data-i18n="${t}">${e}</span></span>`}function Tn(t,e){let o=Math.abs(Number(t));return!Number.isFinite(o)||o<1e3?e:Math.pow(10,Math.floor(Math.log10(o))-1)}function Nn(t){let e=String(t),o=e.indexOf(".");return o<0?0:e.length-o-1}function xe(t,e={}){let o=t.querySelector(e.title||".ui-card-title"),a=document.createElement("div");a.className="ui-form-banner",a.innerHTML='<span class="ui-form-banner-msg" data-i18n="form.unsaved">Unsaved changes</span><span class="ui-form-banner-btns"><button type="button" class="ui-form-discard" data-i18n="form.discard">Discard</button><button type="button" class="ui-form-apply" data-i18n="form.apply">Apply</button></span>',o?o.insertAdjacentElement("afterend",a):t.insertAdjacentElement("afterbegin",a);let n=[],r=()=>a.classList.toggle("show",n.some(h=>h.dirty)),s=(h,w)=>{h.dirty=w,r()};function c(h){return h.markDirty=()=>s(h,!0),n.push(h),h}function p(h,w){let E={dirty:!1,input:h},S=w.baseStep!=null?w.baseStep:parseFloat(h.step)||1,D=Nn(S),A=w.min!=null?w.min:h.min!==""?parseFloat(h.min):-1/0,I=w.max!=null?w.max:h.max!==""?parseFloat(h.max):1/0,K=j=>D>0?Number(j).toFixed(D):String(Math.round(Number(j)));if(!w.nostep){let j=document.createElement("div");j.className="ui-stepper",h.parentNode.insertBefore(j,h);let R=document.createElement("button");R.type="button",R.className="ui-step-btn",R.textContent="\u2212",R.setAttribute("aria-label",d("common.decrease"));let X=document.createElement("button");X.type="button",X.className="ui-step-btn",X.textContent="+",X.setAttribute("aria-label",d("common.increase")),j.appendChild(R),j.appendChild(h),j.appendChild(X);let te=H=>{if(h.disabled)return;let L=parseFloat(h.value);Number.isFinite(L)||(L=parseFloat(h.placeholder)),Number.isFinite(L)||(L=0);let k=Math.min(I,Math.max(A,L+H*Tn(L,S)));h.value=K(k),s(E,!0)};R.addEventListener("click",()=>te(-1)),X.addEventListener("click",()=>te(1)),h.addEventListener("keydown",H=>{H.key==="Enter"&&h.blur()})}return h.addEventListener("input",()=>s(E,!0)),E.sync=()=>{let j=w.read();h.value=j!=null&&Number.isFinite(Number(j))?K(j):""},E.commit=()=>{let j=parseFloat(h.value);Number.isFinite(j)&&w.commit(Math.min(I,Math.max(A,j)))},c(E)}function u(h,w){let E={dirty:!1,input:h};return h.addEventListener("input",()=>s(E,!0)),E.sync=()=>{let S=w.read();h.value=S!=null?S:""},E.commit=()=>w.commit(h.value.trim()),c(E)}function l(h,w){let E={dirty:!1,input:h};return h.addEventListener("change",()=>s(E,!0)),E.sync=()=>{let S=w.read();S!=null&&(h.value=S)},E.commit=()=>w.commit(h.value),c(E)}function m(h,w){let E={dirty:!1,input:h,staged:!1},S=h.closest(".ui-row"),D=()=>{h.classList.toggle("on",E.staged),S&&S.classList.toggle("is-on",E.staged),h.setAttribute("aria-checked",E.staged?"true":"false"),w.onChange&&w.onChange(E.staged)};return h.addEventListener("click",()=>{E.staged=!E.staged,s(E,!0),D()}),E.sync=()=>{E.staged=!!w.read(),D()},E.commit=()=>w.commit(E.staged),c(E)}function g(h){let w={dirty:!1,sync:h.sync,commit:h.commit};return c(w)}let f=()=>n.forEach(h=>{!h.dirty&&h.sync&&h.sync()}),b=()=>{n.forEach(h=>{h.dirty&&(h.commit&&h.commit(),h.dirty=!1)}),r(),e.onApply&&e.onApply()},y=()=>{n.forEach(h=>{h.dirty=!1,h.sync&&h.sync()}),r(),e.onDiscard&&e.onDiscard()};return a.querySelector(".ui-form-apply").addEventListener("click",b),a.querySelector(".ui-form-discard").addEventListener("click",y),M(a),{num:p,text:u,select:l,toggle:m,custom:g,refresh:f,apply:b,discard:y,isDirty:()=>n.some(h=>h.dirty)}}var Dn=`
...
`,Ll=P({tag:"connectivity-card",render:In,onMount(t,e){let o=e.querySelector(".cc-ip"),a=e.querySelector(".cc-ssid"),n=e.querySelector(".cc-mac"),r=e.querySelector(".cc-up"),s=e.querySelector(".cc-ver"),c=0,p=Date.now(),u=!1;function l(){if(!u){r.textContent="---";return}let f=Math.max(0,Math.floor((Date.now()-p)/1e3)),b=rr(c+f);r.textContent!==b&&(r.textContent=b)}function m(){o.textContent=C(i.ip)||"---",a.textContent=C(i.ssid)||"---",n.textContent=C(i.mac)||"---",s.textContent=C(i.firmware)||"---";let f=_(i.uptime);if(f!=null&&!isNaN(f)&&f>=0){let b=f|0;(!u||b!==c)&&(c=b,p=Date.now(),u=!0)}l()}z(i.ip,m),z(i.ssid,m),z(i.mac,m),z(i.firmware,m),z(i.uptime,m);let g=setInterval(l,1e3);e.addEventListener("hv6-unmount",()=>clearInterval(g),{once:!0}),M(e),m()}});var qn="http://www.w3.org/2000/svg",Hn=`
.chart-card {
  border: 1px solid var(--panel-border);
  border-radius: 8px;
  background: var(--panel-bg-vibrant);
...
`;function es(t,e){if(!t||!t.entries||t.entries.length===0)return null;let o=t.entries,a=t.uptime_s||e||0,n=Number(Date.now()/1e3)|0,r=1e3,s=r-at;function c(S){let D=(S+Mt)/Yn;return at+Math.max(0,Math.min(1,D))*s}function p(S){return S-a}let u="http://www.w3.org/2000/svg",l=document.createElementNS(u,"svg");l.setAttribute("viewBox","0 0 "+r+" "+po),l.classList.add("timeline-svg");let m=document.createElementNS(u,"rect");m.setAttribute("x",at),m.setAttribute("y",ft),m.setAttribute("width",s),m.setAttribute("height",po-ft-uo),m.setAttribute("fill","rgba(0,32,46,0.55)"),m.setAttribute("rx","4"),l.appendChild(m);let g=c(0),f=[-24,-18,-12,-6,0].map(S=>S*3600);for(let S of f){let D=c(S),A=document.createElementNS(u,"line");A.setAttribute("x1",D),A.setAttribute("y1",ft),A.setAttribute("x2",D),A.setAttribute("y2",po-uo),A.setAttribute("stroke",S===0?"var(--series-solar)":"rgba(120,146,200,.16)"),A.setAttribute("stroke-width","1"),S===0&&(A.setAttribute("stroke-dasharray","2 3"),A.setAttribute("opacity",".55"),A.setAttribute("vector-effect","non-scaling-stroke")),l.appendChild(A)}l.appendChild(ts(u,"text",{x:g+4,y:ft+11,"text-anchor":"start",fill:"rgba(255,211,128,.92)","font-size":"9","font-family":"Montserrat, sans-serif","font-weight":"600"},"now"));for(let S=0;S<ye;S++){let D=ft+S*(At+Wo),A=document.createElementNS(u,"rect");A.setAttribute("x",at),A.setAttribute("y",D),A.setAttribute("width",s),A.setAttribute("height",At),A.setAttribute("fill",S%2===0?"rgba(124,155,208,0.05)":"rgba(124,155,208,0.00)"),l.appendChild(A);let I=document.createElementNS(u,"text");I.setAttribute("x",at-4),I.setAttribute("y",D+At/2+1),I.setAttribute("text-anchor","end"),I.setAttribute("dominant-baseline","middle"),I.setAttribute("fill","rgba(233,222,210,.62)"),I.setAttribute("font-size","9.5"),I.setAttribute("font-family","Montserrat, sans-serif"),I.setAttribute("font-weight","600"),I.textContent="Z"+(S+1),l.appendChild(I);let K=o.map(R=>({rel:p(R[0]),state:R[S+1]})).filter(R=>R.rel>=-Mt&&R.rel<=0),j=(R,X,te)=>{if(te===255)return;let H=We[te]||We[255];if(H.color==="transparent")return;let L=c(R),k=c(X),q=Math.max(1,k-L),T=document.createElementNS(u,"rect");T.setAttribute("x",L),T.setAttribute("y",D+(At-Zo)/2),T.setAttribute("width",q),T.setAttribute("height",Zo),T.setAttribute("fill",H.color),T.setAttribute("rx",String(Zo/2)),T.setAttribute("opacity","0.9"),l.appendChild(T)};if(K.length){let R=K[0].rel,X=K[0].state;for(let te=1;te<K.length;te++){let H=K[te];H.state!==X&&(j(R,H.rel,X),R=H.rel,X=H.state)}j(R,0,X)}}{let S=document.createElementNS(u,"rect");S.setAttribute("x",at),S.setAttribute("y",Uo),S.setAttribute("width",s),S.setAttribute("height",mo),S.setAttribute("fill","rgba(188,80,144,0.10)"),S.setAttribute("rx","2"),l.appendChild(S);let D=document.createElementNS(u,"text");D.setAttribute("x",at-4),D.setAttribute("y",Uo+mo/2+1),D.setAttribute("text-anchor","end"),D.setAttribute("dominant-baseline","middle"),D.setAttribute("fill","rgba(233,222,210,.62)"),D.setAttribute("font-size","8.5"),D.setAttribute("font-family","Montserrat, sans-serif"),D.setAttribute("font-weight","600"),D.textContent=d("overview.timeline.absorb"),l.appendChild(D);let A=o.map(I=>({rel:p(I[0]),on:I.length>ur?I[ur]:0})).filter(I=>I.rel>=-Mt&&I.rel<=0);if(A.length){let I=(R,X)=>{let te=c(R),H=Math.max(1,c(X)-te),L=document.createElementNS(u,"rect");L.setAttribute("x",te),L.setAttribute("y",Uo),L.setAttribute("width",H),L.setAttribute("height",mo),L.setAttribute("fill",br),L.setAttribute("rx","2"),L.setAttribute("opacity","0.9"),l.appendChild(L)},K=A[0].rel,j=A[0].on;for(let R=1;R<A.length;R++)A[R].on!==j&&(j&&I(K,A[R].rel),K=A[R].rel,j=A[R].on);j&&I(K,0)}}let b=po-uo+15,y=3600,h=Math.ceil((n-Mt)/y)*y,w=Math.floor(n/y)*y,E=Math.floor(n/y)*y;for(let S=h;S<=w;S+=y){let D=S-n,A=c(D),I=new Date(S*1e3),K=String(I.getHours()).padStart(2,"0"),j=S===E,R=document.createElementNS(u,"text");R.setAttribute("x",A),R.setAttribute("y",b),R.setAttribute("text-anchor","end"),R.setAttribute("fill",j?"rgba(255,211,128,.95)":"rgba(202,219,248,.72)"),R.setAttribute("font-size","9"),R.setAttribute("font-family",'"Montserrat", sans-serif'),R.setAttribute("font-weight","500"),R.setAttribute("font-variant-numeric","tabular-nums lining-nums"),R.setAttribute("font-feature-settings",'"tnum" 1, "lnum" 1'),R.setAttribute("letter-spacing","0"),R.setAttribute("transform",`rotate(-45 ${A.toFixed(1)} ${b})`),R.textContent=K,l.appendChild(R)}return l}function ts(t,e,o,a){let n=document.createElementNS(t,e);for(let r in o)n.setAttribute(r,o[r]);return a!=null&&(n.textContent=a),n}function mr(t){t.innerHTML="";let e=[{code:5,...We[5]},{code:6,...We[6]},{code:0,...We[0]},{code:1,...We[1]},{code:7,...We[7]},{code:2,...We[2]}];for(let a of e){let n=document.createElement("div");n.className="tl-legend-item",n.innerHTML='<span class="tl-legend-dot" style="background:'+a.color+'"></span>'+(a.labelKey?d(a.labelKey):""),t.appendChild(n)}let o=document.createElement("div");o.className="tl-legend-item",o.innerHTML='<span class="tl-legend-dot" style="background:'+br+'"></span>'+d("overview.timeline.preheatAbsorption"),t.appendChild(o)}var jl=P({tag:"zone-state-timeline",render:Qn,onMount(t,e){let o=e.querySelector(".tl-body"),a=e.querySelector(".timeline-legend");mr(a);function n(){let r=N("zoneStateHistory"),s=(()=>{let p=N&&N("zoneStateHistory");return p&&p.uptime_s||Number(Date.now()/1e3)|0})();if(o.innerHTML="",!r||!r.entries||r.entries.length===0){let p=document.createElement("div");p.className="timeline-empty",p.textContent=d("overview.timeline.noHistory"),o.appendChild(p);return}let c=es(r,s);c&&o.appendChild(c)}Z("zoneStateHistory",n),Z("zoneNames",n),z(i.drivers,n);for(let r=1;r<=ye;r++)z(v.enabled(r),n),z(v.state(r),n),z(v.temp(r),n),z(v.setpoint(r),n),z(v.preheatAdvance(r),n);M(e),n()}});var os=`
.zone-grid {
    display: grid;
...
`,tc=P({tag:"zone-card",state:t=>({zone:t.zone,selection:t.selection!==!1,navigate:t.navigate!==!1}),render:ns,onMount(t,e){let o=t.zone,a=v.temp(o),n=v.state(o),r=v.enabled(o),s=e.querySelector(".zc-state-label"),c=e.querySelector(".zc-zone-name"),p=e.querySelector(".zc-friendly"),u=e.querySelector(".zc-temp"),l=e.querySelector(".zc-target"),m=e.querySelector(".zc-valve-value");function g(){var I;let b=ce(r),y=String(C(n)||"").toUpperCase()||"OFF",h=String(C(v.motorLastFault(o))||"").toUpperCase(),w=h&&h!=="NONE"&&h!=="OK",E=b&&(y==="FAULT"||w)?"FAULT":y,S=t.selection&&N("selectedZone")===o,D=pt(o);c.innerHTML=Oe(o),p.textContent=D?"":"---",p.hidden=!!D,e.setAttribute("aria-label",Se(o)),u.textContent=ue(_(a)),l.textContent=d("zone.card.setpoint",{value:ue((I=_(v.effectiveSetpoint(o)))!=null?I:_(v.setpoint(o)))}),m.textContent=mt(_(v.valve(o)));let A=b?E:"OFF";s.textContent=A==="HEATING"?d("state.heating"):A==="IDLE"?d("state.idle"):A==="FAULT"?d("common.fault"):A==="MANUAL"?d("state.manual"):A==="OVERHEATED"?d("state.overheated"):A==="CALIBRATING"?d("state.calibrating"):d("state.off"),e.title=w?d("zone.card.fault",{fault:h}):"",e.classList.toggle("active",S),S?e.setAttribute("aria-current","location"):e.removeAttribute("aria-current"),e.setAttribute("aria-label",`${c.textContent}, ${u.textContent}, ${l.textContent}, ${s.textContent}. Open details.`),e.classList.toggle("disabled",!b),e.classList.toggle("zs-heating",b&&A==="HEATING"),e.classList.toggle("zs-fault",b&&A==="FAULT"),e.classList.toggle("zs-idle",b&&A==="IDLE"),e.classList.toggle("zs-off",!b||A==="OFF")}function f(){Vt(o),t.navigate&&Pe("zones"),e.dispatchEvent(new CustomEvent("zone-open",{bubbles:!0,detail:{zone:o}}))}e.addEventListener("click",f),z(a,g),z(v.setpoint(o),g),z(v.effectiveSetpoint(o),g),z(v.valve(o),g),z(n,g),z(r,g),z(v.motorLastFault(o),g),Z("selectedZone",g),Z("zoneNames",g),g()}});var ss=`
...
`,hr=5,vr=35,bo=.5;function ht(t){return t==null||Number.isNaN(Number(t))?"":(Math.round(Number(t)*10)/10).toFixed(1)}function ls(t){if(t==null)return null;let e=String(t).trim().replace(",",".").replace(/[^\d.+-]/g,"");if(!e)return null;let o=Number(e);if(!Number.isFinite(o))return null;let a=Math.round(o/bo)*bo;return Math.min(vr,Math.max(hr,Number(a.toFixed(1))))}function cs(t){let e=Number(_(v.setpoint(t)));return Number.isFinite(e)?e:null}function go(t){let e=Number(_(v.coordinatorOffset(t)));return Number.isFinite(e)?e:0}function Ke(t){let e=Number(_(v.effectiveSetpoint(t)));if(Number.isFinite(e))return e;let o=cs(t);return o==null?null:Number((o+go(t)).toFixed(1))}function Et(t){return Math.min(vr,Math.max(hr,Number(Number(t).toFixed(1))))}function ds(t,e){if(!e)return d("common.disabled");let o=String(t||"IDLE").toUpperCase();return o==="HEATING"?d("state.heating"):o==="IDLE"?d("state.idle"):o==="OFF"?d("state.off"):o==="FAULT"?d("common.fault"):o==="MANUAL"?d("state.manual"):o==="OVERHEATED"?d("state.overheated"):o==="CALIBRATING"?d("state.calibrating"):o}var dc=P({tag:"zone-detail",state:t=>({zone:t.zone,temp:"---",setpoint:"---",valve:"---",state:"---"}),render:is,methods:{update(t,e){var p;let o=N("selectedZone"),a=String(C(v.state(o))||"").toUpperCase(),n=ce(v.enabled(o));this.zone=o,t.dataset.zone=String(o),document.activeElement!==e.setpoint&&(e.setpoint.value=ht(Ke(o))),e.base.textContent=ue((p=_(v.baseSetpoint(o)))!=null?p:_(v.setpoint(o)));let r=_(v.coordinatorOffset(o));e.offset.textContent=r==null?"---":(r>0?"+":"")+Number(r).toFixed(1)+"\xB0C",e.temp.textContent=ue(_(v.temp(o))),e.ret.textContent=ue(_("sensor-manifold_return_temperature")),e.valve.textContent=mt(_(v.valve(o)));let s=e.badge;s.textContent=ds(a,n);let c=n?a==="HEATING"?"badge-heating":a==="IDLE"?"badge-idle":a==="FAULT"?"badge-fault":"":"badge-disabled";s.className="zd-badge"+(c?" "+c:""),e.toggle.classList.toggle("on",n)},commitSetpoint(t){let e=this.zone,o=ls(t);if(o==null)return null;let a=Et(o-go(e));return Jt(e,a),Ke(e)},incSetpoint(){let t=this.zone,e=Ke(t),o=Et((e==null?20:e)+bo);Jt(t,Et(o-go(t)))},decSetpoint(){let t=this.zone,e=Ke(t),o=Et((e==null?20:e)-bo);Jt(t,Et(o-go(t)))},toggleEnabled(){let t=this.zone,e=ce(v.enabled(t));_a(t,!e)}},onMount(t,e){let o={setpoint:e.querySelector(".zd-setpoint"),temp:e.querySelector(".zd-temp"),base:e.querySelector(".zd-base"),offset:e.querySelector(".zd-offset"),ret:e.querySelector(".zd-ret"),valve:e.querySelector(".zd-valve"),badge:e.querySelector(".zd-badge"),toggle:e.querySelector(".btn-toggle"),inc:e.querySelector(".btn-inc"),dec:e.querySelector(".btn-dec")};o.inc.onclick=()=>t.incSetpoint(),o.dec.onclick=()=>t.decSetpoint(),o.toggle.onclick=()=>t.toggleEnabled();let a=()=>{let s=t.commitSetpoint(o.setpoint.value);o.setpoint.value=s!=null?ht(s):ht(Ke(t.zone))};o.setpoint.addEventListener("keydown",s=>{s.key==="Enter"?(s.preventDefault(),o.setpoint.blur()):s.key==="Escape"?(s.preventDefault(),o.setpoint.value=ht(Ke(t.zone)),o.setpoint.blur()):s.key==="ArrowUp"?(s.preventDefault(),t.incSetpoint(),o.setpoint.value=ht(Ke(t.zone))):s.key==="ArrowDown"&&(s.preventDefault(),t.decSetpoint(),o.setpoint.value=ht(Ke(t.zone)))}),o.setpoint.addEventListener("blur",a),o.setpoint.addEventListener("focus",()=>{requestAnimationFrame(()=>o.setpoint.select())});let n=()=>t.update(e,o),r=s=>{let c=N("selectedZone");(s===v.temp(c)||s===v.setpoint(c)||s===v.baseSetpoint(c)||s===v.effectiveSetpoint(c)||s===v.coordinatorOffset(c)||s===v.valve(c)||s===v.state(c)||s===v.enabled(c))&&n()};for(let s=1;s<=6;s++)z(v.temp(s),r),z(v.setpoint(s),r),z(v.baseSetpoint(s),r),z(v.effectiveSetpoint(s),r),z(v.coordinatorOffset(s),r),z(v.valve(s),r),z(v.state(s),r),z(v.enabled(s),r);z("sensor-manifold_return_temperature",n),Z("selectedZone",n),M(e),n()}});var ps=`
...
`;O("zone-sensor-card",ps);var us=()=>`
  ...
  `;function ms(t){return t==="BLE"||t==="BLE Sensor"?"BLE Sensor":"Local Probe"}function gs(t){return t==="BLE Sensor"?"BLE":"Local Probe"}function xr(t,e){let o='<option value="Local Probe" data-i18n="zone.sensor.localProbe">'+d("zone.sensor.localProbe")+'</option><option value="BLE Sensor" data-i18n="zone.sensor.bleSource">'+d("zone.sensor.bleSource")+"</option>";t.innerHTML!==o&&(t.innerHTML=o),t.value=e}var xc=P({tag:"zone-sensor-card",render:us,onMount(t,e){let o=e.querySelector(".zs-source"),a=e.querySelector(".zs-ble"),n=e.querySelector(".zs-row-ble"),r=e.querySelector(".zs-scan"),s=e.querySelector(".zs-scan-list"),c=0;function p(){return N("selectedZone")}function u(){n.style.display=o.value==="BLE Sensor"?"":"none"}let l=xe(e);xr(o,"Local Probe"),l.select(o,{read:()=>ms(String(C(v.tempSource(p()))||"")),commit:b=>Ve(p(),"zone_temp_source",gs(b))});let m=l.text(a,{read:()=>C(v.ble(p()))||"",commit:b=>Ro(p(),"zone_ble_mac",b)});o.addEventListener("change",u);function g(){let b=p();c!==b?(c=b,s.style.display="none",l.discard()):l.refresh(),u()}function f(b){let y=p();(b===v.tempSource(y)||b===v.ble(y))&&(l.refresh(),u())}r.addEventListener("click",()=>{if(r.disabled)return;r.disabled=!0,r.textContent="\u2026",s.style.display="",s.innerHTML='<div class="scan-msg">'+d("zone.sensor.scanning")+"</div>";let b=new AbortController,y=setTimeout(()=>b.abort(),8e3);fetch("/api/hv6/v1/ble-scan",{cache:"no-store",signal:b.signal}).then(h=>{if(!h.ok)throw new Error("HTTP "+h.status);return h.json()}).then(h=>{if(clearTimeout(y),r.disabled=!1,r.textContent=d("zone.sensor.scan"),!h.ok||!h.sensors||h.sensors.length===0){s.innerHTML='<div class="scan-msg">'+d("zone.sensor.noSensors")+"</div>";return}let w=p(),E=(C(v.ble(w))||"").toUpperCase(),S=A=>String(A).replace(/[&<>"']/g,I=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[I]),D="";for(let A of h.sensors){let I=A.mac.toUpperCase(),K=A.name?S(A.name):"",j=A.temp_c!=null?A.temp_c.toFixed(1)+"\xB0C":"\u2014",R=A.rssi!=null?A.rssi+" dBm":"",X=A.age_s<60?d("common.secondsAgo",{value:A.age_s}):d("common.minutesAgo",{value:Math.round(A.age_s/60)}),te="";I===E?te='<span class="ble-badge">'+d("zone.sensor.assignedThisZone")+"</span>":A.zone>0&&(te='<span class="ble-badge">'+d("zone.sensor.zoneBadge",{zone:A.zone})+"</span>");let H=K?`<div class="ble-mac">${K}</div><div class="ble-meta">${I}</div>`:`<div class="ble-mac">${I}</div>`;D+=`<div class="ble-scan-item">
              <div>
                ${H}
            ...
            </div>`}s.innerHTML=D,s.querySelectorAll(".btn-assign").forEach(A=>{A.addEventListener("click",()=>{a.value=A.dataset.mac,m.markDirty(),s.style.display="none"})})}).catch(h=>{clearTimeout(y),r.disabled=!1,r.textContent=d("zone.sensor.scan");let w=h&&h.name==="AbortError"?d("zone.sensor.scanTimeout"):d("zone.sensor.scanFailed");s.innerHTML='<div class="scan-msg">'+w+"</div>"})}),Z("selectedZone",g);for(let b=1;b<=6;b++)z(v.tempSource(b),f),z(v.ble(b),f);M(e),g()}});var bs=".zone-coordination-card { height: 100%; }";O("zone-coordination-card",bs);var fs=()=>`
...
`;function yr(t,e){let o=t.value,a='<option value="None" data-i18n="common.none">'+d("common.none")+"</option>";for(let n=1;n<=6;n++)n!==e&&(a+='<option value="Zone '+n+'">'+d("common.zone")+" "+n+"</option>");t.innerHTML=a,t.value=o||"None"}var Mc=P({tag:"zone-coordination-card",render:fs,onMount(t,e){let o=e.querySelector(".zc-sync"),a=0;function n(){return N("selectedZone")}let r=xe(e);r.select(o,{read:()=>C(v.syncTo(n()))||"None",commit:p=>Ve(n(),"zone_sync_to",p)});function s(){let p=n();a!==p?(yr(o,p),a=p,r.discard()):r.refresh()}function c(p){let u=n();(p===v.syncTo(u)||/^select-zone_\d+_sync_to$/.test(p))&&r.refresh()}Z("selectedZone",s);for(let p=1;p<=6;p++)z(v.syncTo(p),c);M(e),s()}});var hs=".zone-room-card { height: 100%; }";O("zone-room-card",hs);var vs=()=>`
...
`,Pc=P({tag:"zone-room-card",render:vs,onMount(t,e){let o=e.querySelector(".zr-friendly");function a(){return N("selectedZone")}let n=xe(e);n.text(o,{read:()=>kt(a())||"",commit:r=>Aa(a(),r)}),Z("selectedZone",n.discard),Z("zoneNames",n.refresh),M(e),n.refresh()}});var xs=`
...
`,Vc=P({tag:"zone-actuator-card",render:ws,onMount(t,e){var p,u,l;let o=Number(N("selectedZone")||1),a={orip:e.querySelector(".za-orip"),crip:e.querySelector(".za-crip"),ofac:e.querySelector(".za-ofac"),cfac:e.querySelector(".za-cfac"),ph:e.querySelector(".za-ph"),fault:e.querySelector(".za-fault"),faultVal:e.querySelector(".za-fault-val"),faultBtn:e.querySelector(".recovery-fault-btn"),factorsBtn:e.querySelector(".recovery-factors-btn"),relearnBtn:e.querySelector(".recovery-relearn-btn"),status:e.querySelector(".za-status")};function n(){o=Number(N("selectedZone")||1),a.orip.textContent=kr(_(v.motorOpenRipples(o))),a.crip.textContent=kr(_(v.motorCloseRipples(o))),a.ofac.textContent=wr(_(v.motorOpenFactor(o))),a.cfac.textContent=wr(_(v.motorCloseFactor(o))),a.ph.textContent=ys(_(v.preheatAdvance(o)));let m=String(C(v.motorLastFault(o))||"").toUpperCase(),g=m&&m!=="NONE"&&m!=="OK";a.fault.hidden=!g,g&&(a.faultVal.textContent=m)}let r=null;function s(m,g){a.status.textContent=m,a.status.className="za-status show "+(g?"ok":"err"),clearTimeout(r),r=setTimeout(()=>{a.status.classList.remove("show")},4e3)}function c(m,g){let f=m(o);s(g,!0),f&&typeof f.then=="function"&&f.then(b=>{b&&b.ok===!1&&s(d("diagnostics.recovery.rejected"),!1)}).catch(()=>s(d("diagnostics.recovery.unreachable"),!1))}(p=a.faultBtn)==null||p.addEventListener("click",()=>{c(Da,"\u2713 "+d("diagnostics.recovery.faultSent",{zone:Se(o)}))}),(u=a.factorsBtn)==null||u.addEventListener("click",()=>{confirm(d("diagnostics.recovery.confirmFactors",{zone:Se(o)}))&&c(Ra,"\u2713 "+d("diagnostics.recovery.factorsReset",{zone:Se(o)}))}),(l=a.relearnBtn)==null||l.addEventListener("click",()=>{confirm(d("diagnostics.recovery.confirmRelearn",{zone:Se(o)}))&&c(Pa,"\u2713 "+d("diagnostics.recovery.relearnStarted",{zone:Se(o)}))}),Z("selectedZone",n);for(let m=1;m<=6;m++)z(v.motorOpenRipples(m),n),z(v.motorCloseRipples(m),n),z(v.motorOpenFactor(m),n),z(v.motorCloseFactor(m),n),z(v.preheatAdvance(m),n),z(v.motorLastFault(m),n);M(e),n()}});var De=6,ks="var(--flow-disabled)",Sr="var(--flow-unknown)",_r="var(--accent)",Ft="var(--flow-return)",Go="var(--text-strong)",zs="var(--flow-disabled)",rt="var(--flow-label)",fo="var(--flow-disabled)",Ko="var(--flow-label)",Cr="var(--flow-label)",zr="var(--flow-return)",Ss="#66BB6A",_s="#FF6361",ee={w:1160,h:372,boxX:440,boxY:26,boxW:280,boxH:90,srcY:116,fanY:168,zoneY:262,zoneXs:[92,286,480,674,868,1062],srcSpread:15,bgDstHW:28,srcHW:7},V={w:760,h:424,boxX:26,boxY:148,boxW:168,boxH:92,srcX:196,endX:386,nameX:446,midY:190,zoneYs:[56,120,184,248,312,376],spread:10,bgDstHW:15,srcHW:4},Cs=`
.flow-wrap {
  width: 100%;
  border-radius: 8px;
  overflow: hidden;
...
`,dd=P({tag:"diag-i2c",render:Hs,onMount(t,e){let o=e.querySelector("#i2c-result");function a(){o.textContent=N("i2cResult")||d("diagnostics.i2c.empty")}e.querySelector("#btn-i2c-scan").addEventListener("click",()=>{Ca()}),Z("i2cResult",a),M(e),a()}});var Bs=`
.diag-manual-badge {
  display: none;
  align-items: center;
  gap: 10px;
...
`,fd=P({tag:"diag-manual-badge",render:$s,onMount(t,e){let o=e.classList.contains("diag-manual-badge")?e:e.querySelector(".diag-manual-badge");function a(){let n=!!N("manualMode");o&&o.classList.toggle("on",n)}Z("manualMode",a),M(e),a()}});var js=`
.diag-zone-motor {
  background: var(--panel-bg-vibrant);
  border: 1px solid var(--panel-border);
  border-radius: 8px;
...
`;function jr(t){return t==="open"?{factor:_(i.openThresholdMultiplier),slope:_(i.openSlopeThreshold),floor:_(i.openSlopeCurrentFactor),ripple:_(i.openRippleLimitFactor)}:{factor:_(i.closeThresholdMultiplier),slope:_(i.closeSlopeThreshold),floor:_(i.closeSlopeCurrentFactor)}}function qt(t){let e=jr(t.direction),o=t.direction==="open"?"open":"close",a=[{key:o+"_threshold_multiplier",labelKey:t.direction==="open"?"settings.motor.openThreshold":"settings.motor.closeThreshold",current:e.factor,suggested:t.suggested_factor,unit:"x"},{key:o+"_slope_threshold",labelKey:t.direction==="open"?"settings.motor.openSlope":"settings.motor.closeSlope",current:e.slope,suggested:t.suggested_slope,unit:"mA/s"},{key:o+"_slope_current_factor",labelKey:t.direction==="open"?"settings.motor.openSlopeFloor":"settings.motor.closeSlopeFloor",current:e.floor,suggested:t.suggested_slope_floor,unit:"x"}];return t.direction==="open"&&t.suggested_ripple_limit!=null&&a.push({key:"open_ripple_limit_factor",labelKey:"settings.motor.openRippleLimit",current:e.ripple,suggested:t.suggested_ripple_limit,unit:"x"}),a}function aa(t){return d(t?"common.on":"common.off")}function ra(){return{current:null,mean:null,peak:null,slope:null,runtime:0,motion:0,busy:!1,direction:"\u2014",stroke:0,pinSeen:!1,pinAt:null,pinMa:null,tachoPeriodUs:null,tachoCadenceUs:null,cadenceHz:null,faultCode:0,armed:!1,backend:"\u2014",invalidSamples:0,tachoRejected:0}}var Hd=P({tag:"diag-motor-lab",render:li,onMount(t,e){let o=Number(N("selectedZone")||1),a="setup",n="idle",r={active:!1,aborted:!1,direction:null,timer:null,live:[],started:0},s={open:null,close:null,seat:null},c={samples:[],analysis:null,live:!1},p={current:!0,overlays:!0,phase:!0,cadence:!0,slope:!0},u=[],l=ra(),m=e.querySelector(".lab-step-chip"),g=e.querySelector(".lab-zone-chip"),f=e.querySelector(".lab-banner"),b=e.querySelector(".lab-guide"),y=e.querySelector(".lab-kicker"),h=e.querySelector(".lab-stage h3"),w=e.querySelector(".lab-stage p"),E=e.querySelector(".lab-setup"),S=e.querySelector(".lab-zone"),D=e.querySelector(".lab-phase-label"),A=e.querySelector(".lab-log"),I=e.querySelector(".lab-chart"),K=e.querySelector(".lab-metrics"),j=e.querySelector(".lab-suggest"),R=e.querySelector(".lab-primary"),X=e.querySelector(".lab-secondary"),te=e.querySelector(".lab-estop"),H={current:e.querySelector('[data-k="current"]'),mean:e.querySelector('[data-k="mean"]'),peak:e.querySelector('[data-k="peak"]'),slope:e.querySelector('[data-k="slope"]'),runtime:e.querySelector('[data-k="runtime"]'),motion:e.querySelector('[data-k="motion"]'),cadence:e.querySelector('[data-k="cadence"]'),direction:e.querySelector('[data-k="direction"]'),drivers:e.querySelector('[data-k="drivers"]'),busy:e.querySelector('[data-k="busy"]'),stroke:e.querySelector('[data-k="stroke"]'),pin:e.querySelector('[data-k="pin"]'),armed:e.querySelector('[data-k="armed"]'),backend:e.querySelector('[data-k="backend"]'),fault:e.querySelector('[data-k="fault"]'),invalid:e.querySelector('[data-k="invalid"]'),tachoRejected:e.querySelector('[data-k="tachoRejected"]')};function L(F){return It.indexOf(F)}function k(){return Se(o)}function q(){let F=String(o);S.innerHTML=Array.from({length:6},(Y,oe)=>'<option value="'+(oe+1)+'">'+Se(oe+1).replace(/</g,"&lt;")+"</option>").join(""),S.value=F,S.setAttribute("aria-label",d("diagnostics.lab.motor")),g.textContent=k(),g.setAttribute("aria-label",d("diagnostics.lab.motor"))}function T(F,Y){let oe=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"});u.push(oe+"  "+d(F,Y)),u.length>8&&u.shift(),A.innerHTML=u.map(se=>"<div>"+se+"</div>").join(""),A.scrollTop=A.scrollHeight}function ie(F){f.textContent=F||"",f.classList.toggle("show",!!F)}function ae(F,Y){n=F,D.dataset.kind=Y||"",D.textContent=d("diagnostics.lab.phase."+F)}function B(){let F=Nt(l.stroke);H.current.textContent=l.current==null?"\u2014":l.current.toFixed(1)+" mA",H.mean.textContent=l.mean==null?"\u2014":l.mean.toFixed(1)+" mA",H.peak.textContent=l.peak==null?"\u2014":l.peak.toFixed(1)+" mA",H.slope.textContent=l.slope==null?"\u2014":l.slope.toFixed(1)+" mA/s",H.runtime.textContent=l.runtime?(l.runtime/1e3).toFixed(1)+" s":"\u2014",H.motion.textContent=l.motion?String(l.motion):"\u2014",H.cadence.textContent=l.cadenceHz==null?"\u2014":l.cadenceHz.toFixed(1)+" /s",H.direction.textContent=l.direction,H.drivers.textContent=aa(ce(i.drivers)),H.busy.textContent=aa(l.busy),H.armed.textContent=aa(l.armed),H.backend.textContent=l.backend||"\u2014",H.fault.textContent=l.faultCode?String(l.faultCode):d("common.ok"),H.invalid.textContent=String(l.invalidSamples||0),H.tachoRejected.textContent=String(l.tachoRejected||0),H.stroke.textContent=d("diagnostics.lab.stroke."+F),H.stroke.dataset.phase=F,l.pinSeen?(H.pin.textContent=d("diagnostics.lab.pinSeen",{count:l.pinAt}),H.pin.dataset.seen="true"):(H.pin.textContent=d("diagnostics.lab.pinWaiting"),H.pin.dataset.seen="false")}function me(){let F=c.analysis?Pr(c.analysis,jr(c.analysis.direction)):[],Y=Br(I,{samples:c.samples,analysis:c.analysis,live:c.live,overlays:F,visible:p});Y&&Y.addEventListener("click",oe=>{let se=oe.target.closest(".gw-toggle");if(!se)return;let W=se.dataset.layer;p[W]=!p[W],Object.keys(p).some(fe=>p[fe])||(p[W]=!0),me()})}function be(F,Y,oe){c={analysis:F&&F.ok?F:null,samples:Y||[],live:!!oe},c.analysis&&(l.mean=c.analysis.mean_ma,l.peak=c.analysis.peak_ma,l.slope=c.analysis.max_stall_slope_ma_s),me();let se=[];if(a==="review"?(s.open&&se.push(...qt(s.open)),s.close&&se.push(...qt(s.close))):c.analysis&&se.push(...qt(c.analysis)),!c.analysis&&a!=="review"){K.innerHTML="",j.hidden=!0;return}let W=a==="review"?s.close||s.open:c.analysis;if(!W){K.innerHTML="",j.hidden=!0;return}let pe=W.pin_seen?d("diagnostics.lab.pinMetric",{ms:W.pin_t_ms,count:W.pin_motion_count}):d("diagnostics.lab.pinWaiting"),fe=[[d("diagnostics.lab.mean"),W.mean_ma.toFixed(1)+" mA"],[d("diagnostics.lab.peak"),W.peak_ma.toFixed(1)+" mA"],[d("diagnostics.lab.runtime"),(W.runtime_ms/1e3).toFixed(1)+" s"],[d("diagnostics.lab.ripples"),String(W.ripples)],[d("diagnostics.lab.pin"),pe]];if(a==="review"&&s.open&&s.close){fe[0]=[d("diagnostics.lab.mean"),s.open.mean_ma.toFixed(1)+" / "+s.close.mean_ma.toFixed(1)+" mA"],fe[1]=[d("diagnostics.lab.peak"),s.open.peak_ma.toFixed(1)+" / "+s.close.peak_ma.toFixed(1)+" mA"];let de=s.close.pin_seen?d("diagnostics.lab.pinMetric",{ms:s.close.pin_t_ms,count:s.close.pin_motion_count}):d("diagnostics.lab.pinWaiting");fe[4]=[d("diagnostics.lab.pin"),de]}K.innerHTML=fe.map(de=>'<div class="lab-metric"><span>'+de[0]+"</span><strong>"+de[1]+"</strong></div>").join(""),j.querySelector("tbody").innerHTML=se.map(de=>"<tr><td>"+d(de.labelKey)+"</td><td>"+Number(de.current).toFixed(1)+" "+de.unit+'</td><td class="better">'+Number(de.suggested).toFixed(1)+" "+de.unit+"</td></tr>").join(""),j.hidden=!se.length}function re(){let F=a==="halted",Y=F?"setup":a,oe=L(Y),se=F?"halted":"active";m.dataset.state=se,m.textContent=F?d("diagnostics.lab.halted"):d("diagnostics.lab.stepChip",{step:oe+1,total:It.length,name:d("diagnostics.lab.steps."+Y)}),y.textContent=F?d("diagnostics.lab.halted"):d("diagnostics.lab.stepOf",{step:oe+1,total:It.length}),h.textContent=d("diagnostics.lab."+(F?"halt":Y)+".title");let W=a==="seat"&&s.seat||a==="open"&&s.open||a==="close"&&s.close,pe=F?"diagnostics.lab.halt.copy":W?"diagnostics.lab."+Y+".done":"diagnostics.lab."+Y+".copy";w.textContent=d(pe);let fe=a==="setup";b.dataset.setup=fe?"true":"false",E.hidden=!fe,S.disabled=!fe||r.active,g.hidden=fe,g.textContent=k(),te.dataset.armed=r.active?"true":"false",B();let de=r.active,_e={key:"diagnostics.lab.next",disabled:de,action:"next"},Be=null;a==="setup"?_e={key:"diagnostics.lab.setup.action",disabled:!1,action:"start"}:a==="arm"?_e={key:"diagnostics.lab.arm.action",disabled:de,action:"arm"}:a==="seat"?_e={key:s.seat?"diagnostics.lab.next":"diagnostics.lab.seat.action",disabled:de,action:s.seat?"next":"seat"}:a==="open"?_e={key:s.open?"diagnostics.lab.next":"diagnostics.lab.open.action",disabled:de,action:s.open?"next":"open"}:a==="close"?_e={key:s.close?"diagnostics.lab.next":"diagnostics.lab.close.action",disabled:de,action:s.close?"next":"close"}:a==="review"?(_e={key:"diagnostics.lab.apply",disabled:!(s.open||s.close),action:"apply"},Be={key:"diagnostics.lab.restart",action:"restart"}):F&&(_e={key:"diagnostics.lab.restart",disabled:!1,action:"restart"}),de&&(_e={key:"diagnostics.lab.runningAction",disabled:!0,action:"none"}),!de&&(a==="seat"||a==="open"||a==="close")&&!s[a==="seat"?"seat":a]&&n==="failed"&&(_e={key:"diagnostics.lab.retry",disabled:!1,action:a}),!de&&W&&(a==="seat"||a==="open"||a==="close")&&(Be={key:"diagnostics.lab.restart",action:"restart"}),R.dataset.action=_e.action,R.disabled=!!_e.disabled,R.textContent=d(_e.key),Be?(X.hidden=!1,X.dataset.action=Be.action,X.textContent=d(Be.key)):(X.hidden=!0,X.dataset.action="")}function Fe(){r.timer&&clearInterval(r.timer),r.timer=null}function he(F){if(a=F,(F==="arm"||F==="setup")&&ie(""),F==="review"){let Y=s.close||s.open;be(Y,Y?Y.samples:[],!1),ae("done","ok")}else F==="setup"&&be(null,[],!1);re()}async function Xe(){if(!r.active){r.active=!0,ae("arming","run"),re(),T("diagnostics.lab.log.arming",{zone:o});try{if(N("manualMode")||(Te("manualMode",!0),await Ct(!0),T("diagnostics.lab.log.manual")),r.aborted||(ce(i.drivers)||(await _t(!0),T("diagnostics.lab.log.drivers")),r.aborted))return;l.busy=!1,l.armed=!0,ae("armed","ok"),T("diagnostics.lab.log.armed"),r.active=!1,he("seat")}catch(F){r.active=!1,ae("failed","halt"),T("diagnostics.lab.log.armFailed"),re()}}}async function it(F,Y,oe){ae("analyzing","run"),re();let se=Nr(F),W=Rr(se,Y),pe=W.ok?W.samples:se;if(oe&&(s[oe]=W.ok?W:null),be(W,pe,!1),!W.ok)ae("failed","halt"),T(oe==="seat"?"diagnostics.lab.log.seatShort":"diagnostics.lab.log.weak"),oe==="seat"&&(s.seat={short:!0},T("diagnostics.lab.log.seatContinue"));else if(ae("done","ok"),T("diagnostics.lab.log.captured",{direction:d("diagnostics.lab.dir."+W.direction),peak:W.peak_ma.toFixed(1)}),W.pin_seen){let fe=l.pinSeen;l.pinSeen=!0,l.pinAt=W.pin_motion_count,l.pinMa=W.pin_current_ma,l.stroke=1,fe||T("diagnostics.lab.log.pinTrace",{count:W.pin_motion_count,ma:W.pin_current_ma.toFixed(1),ms:W.pin_t_ms})}else(oe==="close"||oe==="seat")&&T("diagnostics.lab.log.pinMissing")}async function He(F,Y){for(let oe=0;oe<6;oe++){if(r.aborted)return;try{ae("fetching","run"),re();let se=await Na();T("diagnostics.lab.log.trace"),await it(se,F,Y);return}catch(se){if(se&&se.code==="motor_busy"){await new Promise(W=>setTimeout(W,250));continue}ae("failed","halt"),T("diagnostics.lab.log.traceFailed"),be(null,r.live,!0);return}}ae("failed","halt")}function lt(F){let Y=F.map(se=>se.current_ma).filter(Number.isFinite);if(!Y.length)return;let oe=Y.reduce((se,W)=>se+W,0);if(l.mean=Math.round(oe/Y.length*10)/10,l.peak=Math.round(Math.max(...Y)*10)/10,F.length>=2){let se=F[Math.max(0,F.length-3)],W=F[F.length-1],pe=(W.t_ms-se.t_ms)/1e3;pe>.05&&(l.slope=Math.round((W.current_ma-se.current_ma)/pe*10)/10)}}async function qe(F,Y){if(!r.active){r={active:!0,aborted:!1,direction:F,timer:null,live:[],started:Date.now()},l=ra(),l.direction=d("diagnostics.lab.dir."+F),ae("starting","run"),re(),be(null,[],!0),T("diagnostics.lab.log.starting",{direction:d("diagnostics.lab.dir."+F),zone:o});try{if(F==="open"?await Qt(o,1e4):await eo(o,1e4),r.aborted)return;ae("waiting","run"),re();let oe=!1,se=async()=>{if(!r.aborted)try{let W=await Ta(),pe=W&&W.data&&W.data.motor_safety?W.data.motor_safety:{},fe=Number(pe.current_ma),de=!!pe.motor_busy,_e=pe.drive_on!=null?!!pe.drive_on:de;de&&!oe&&(oe=!0,ae("running","run"),T("diagnostics.lab.log.busy")),l.busy=de,l.runtime=Date.now()-r.started,l.motion=Number(pe.motion_evidence_count)||l.motion,l.stroke=Number(pe.stroke_phase)||0,l.tachoPeriodUs=Number(pe.tacho_period_us)||l.tachoPeriodUs,l.tachoCadenceUs=Number(pe.tacho_cadence_us)||l.tachoCadenceUs;let Be=l.tachoPeriodUs||l.tachoCadenceUs;l.cadenceHz=Be>0?1e6/Be:null,l.faultCode=Number(pe.fault_code)||0,l.armed=!!pe.armed,l.backend=pe.backend||l.backend,l.invalidSamples=Number(pe.invalid_samples)||0,l.tachoRejected=Number(pe.tacho_rejected)||0,!l.pinSeen&&(l.stroke===1||l.stroke===2)&&(l.pinSeen=!0,l.pinAt=l.motion,l.pinMa=Number.isFinite(fe)?fe:l.current,T("diagnostics.lab.log.pin",{count:l.pinAt,ma:Number(l.pinMa||0).toFixed(1)})),Number.isFinite(fe)&&(l.current=fe,r.live.push({t_ms:Date.now()-r.started,current_ma:fe,motion_count:l.motion,drive_on:_e,direction_open:F==="open",stroke_phase:l.stroke,tacho_period_us:l.tachoPeriodUs,tacho_cadence_us:l.tachoCadenceUs,armed:l.armed,fault_code:l.faultCode,backend:l.backend}),lt(r.live),be(null,r.live,!0)),B(),(oe&&!de||Date.now()-r.started>$r)&&(Fe(),l.busy=!1,oe&&T("diagnostics.lab.log.stopped"),r.active=!1,await He(F,Y),re())}catch(W){Date.now()-r.started>$r&&(Fe(),r.active=!1,ae("failed","halt"),T("diagnostics.lab.log.traceFailed"),re())}};r.timer=setInterval(se,si),se()}catch(oe){r.active=!1,ae("failed","halt"),T("diagnostics.lab.log.startFailed"),re()}}}function wt(){Fe(),r={active:!1,aborted:!1,direction:null,timer:null,live:[],started:0},s={open:null,close:null,seat:null},l=ra(),u.length=0,A.innerHTML="",ie(""),ae("idle"),he("setup")}async function Bt(){r.aborted=!0,r.active=!1,Fe(),l.busy=!1,ie(d("diagnostics.lab.estopDone")),ae("halted","halt"),T("diagnostics.lab.log.estop"),a="halted",re();try{await Fa()}catch(F){}B()}function ko(){let F=L(a);F<0||F>=It.length-1||he(It[F+1])}function Ye(F){if(F==="start"){T("diagnostics.lab.log.selected",{zone:o}),he("arm");return}if(F==="arm")return Xe();if(F==="seat")return qe("close","seat");if(F==="open")return qe("open","open");if(F==="close")return qe("close","close");if(F==="next")return ko();if(F==="restart")return wt();if(F==="apply"){let Y=[];s.open&&Y.push(...qt(s.open)),s.close&&Y.push(...qt(s.close)),Y.forEach(oe=>Me(oe.key,oe.suggested)),ae("applied","ok"),T("diagnostics.lab.log.applied"),re()}}R.addEventListener("click",()=>Ye(R.dataset.action)),X.addEventListener("click",()=>Ye(X.dataset.action)),te.addEventListener("click",Bt),S.addEventListener("change",()=>{o=Number(S.value||1),g.textContent=k()});function ct(F){F.key==="Escape"&&N("section")==="motorlab"&&(F.preventDefault(),Bt())}window.addEventListener("keydown",ct),q(),ae("idle"),be(null,[],!1),re(),z(i.drivers,B),Z("manualMode",B),Z("selectedZone",()=>{a==="setup"&&(o=Number(N("selectedZone")||o),S.value=String(o))}),M(e)}});var ci=`
...
`,Kd=P({tag:"diag-system-card",render:di,onMount(t,e){let o=e.querySelector('[data-k="cpu0"]'),a=e.querySelector('[data-k="cpu1"]'),n=e.querySelector('[data-k="heap"]'),r=e.querySelector('[data-k="dma"]'),s=e.querySelector('[data-k="largestInternal"]'),c=e.querySelector('[data-k="minInternal"]'),p=e.querySelector('[data-k="psram"]'),u=e.querySelector('[data-k="largestPsram"]'),l=e.querySelector('[data-bar="cpu0"]'),m=e.querySelector('[data-bar="cpu1"]'),g=e.querySelector('[data-k="reset"]'),f=(h,w,E)=>{if(E==null||!Number.isFinite(Number(E))){h.textContent="\u2014",h.classList.remove("warn"),w.style.width="0%";return}let S=Math.max(0,Math.min(100,Number(E)));h.textContent=S.toFixed(0)+"%",h.classList.toggle("warn",S>=90),w.style.width=S+"%"},b=(h,w,E)=>{if(w==null||!Number.isFinite(Number(w))){h.textContent="\u2014";return}let S=Number(w);h.textContent=S+" KB",h.classList.toggle("warn",E!=null&&S<E)},y=()=>{f(o,l,_(i.cpuLoadCore0)),f(a,m,_(i.cpuLoadCore1)),b(n,_(i.freeInternalKb),48),b(r,_(i.freeDmaKb),32),b(s,_(i.largestInternalKb),24),b(c,_(i.minInternalKb),48),b(p,_(i.freePsramKb),null),b(u,_(i.largestPsramKb),null);let h=String(C(i.resetReason)||N("resetReason")||"").trim();g.textContent=h||"\u2014"};e.querySelector(".sys-dump").addEventListener("click",()=>{Oa().catch(h=>console.error("[System] dump failed:",h))}),z(i.cpuLoadCore0,y),z(i.cpuLoadCore1,y),z(i.freeInternalKb,y),z(i.freeDmaKb,y),z(i.largestInternalKb,y),z(i.minInternalKb,y),z(i.freePsramKb,y),z(i.largestPsramKb,y),z(i.resetReason,y),Z("resetReason",y),M(e),y()}});var pi=`
...
`;O("settings-manifold-card",pi);var ui=()=>{let t="";for(let o=1;o<=8;o++)t+="<option>Probe "+o+"</option>";let e="";for(let o=1;o<=8;o++)e+='<div class="probe-cell"><div class="probe-name">Probe '+o+'</div><div class="probe-temp" data-probe="'+o+'">---</div></div>';return`
  ...
  `},rp=P({tag:"settings-manifold-card",render:ui,onMount(t,e){let o=e.querySelector(".sm-type"),a=e.querySelector(".sm-flow"),n=e.querySelector(".sm-ret"),r=xe(e);r.select(o,{read:()=>C(i.manifoldType)||"NO (Normally Open)",commit:c=>Ae("manifold_type",c)}),r.select(a,{read:()=>C(i.manifoldFlowProbe)||"Probe 7",commit:c=>Ae("manifold_flow_probe",c)}),r.select(n,{read:()=>C(i.manifoldReturnProbe)||"Probe 8",commit:c=>Ae("manifold_return_probe",c)});function s(){for(let c=1;c<=8;c++){let p=e.querySelector('[data-probe="'+c+'"]');p&&(p.textContent=ue(_(v.probeTemp(c))))}}z(i.manifoldType,r.refresh),z(i.manifoldFlowProbe,r.refresh),z(i.manifoldReturnProbe,r.refresh);for(let c=1;c<=8;c++)z(v.probeTemp(c),s);M(e),r.refresh(),s()}});var mi=`
...
`;O("settings-return-temp-card",fi);function nt(t){return!!(t&&t!=="None")}function hi(t){return"Probe "+t}function vi(){for(let t=1;t<=6;t++)if(nt(C(v.probe(t))))return!0;return!1}function xi(){let t="";for(let e=1;e<=8;e++)t+='<option value="Probe '+e+'">Probe '+e+"</option>";return t}var yi=()=>{let t=xi(),e="";for(let o=1;o<=6;o++)e+=`
  ...
  `},Mp=P({tag:"settings-return-temp-card",render:yi,onMount(t,e){let o=e.querySelector(".srt-enabled"),a=e.querySelector(".srt-zones"),n=Array.from(e.querySelectorAll(".srt-probe")),r=Object.create(null);function s(m){return r[m]||hi(m)}function c(m){a.hidden=!m,a.setAttribute("aria-hidden",m?"false":"true");for(let g of n)g.disabled=!m}function p(){for(let m=1;m<=6;m++){let g=e.querySelector('[data-zone-label="'+m+'"]');g&&(g.innerHTML=Oe(m))}}let u=xe(e),l;for(let m of n){let g=Number(m.dataset.zone);u.select(m,{read:()=>{let f=C(v.probe(g));return nt(f)?(r[g]=f,f):s(g)},commit:f=>{!l||!l.staged||(r[g]=f,Ve(g,"zone_probe",f))}})}l=u.toggle(o,{read:()=>vi(),onChange:m=>{if(m)for(let g of n){let f=Number(g.dataset.zone);nt(g.value)||(g.value=s(f)),nt(g.value)&&(r[f]=g.value)}else for(let g of n){let f=Number(g.dataset.zone);nt(g.value)&&(r[f]=g.value)}c(m)},commit:m=>{if(!m){for(let g of n){let f=Number(g.dataset.zone);nt(g.value)&&(r[f]=g.value),Ve(f,"zone_probe","None")}return}for(let g of n){let f=Number(g.dataset.zone),b=nt(g.value)?g.value:s(f);r[f]=b,Ve(f,"zone_probe",b)}}});for(let m=1;m<=6;m++)z(v.probe(m),u.refresh),z(v.name(m),p);M(e),p(),u.refresh()}});var wi=[{value:"15",labelKey:"settings.bleClock.interval15"},{value:"60",labelKey:"settings.bleClock.interval60"},{value:"360",labelKey:"settings.bleClock.interval360"},{value:"1440",labelKey:"settings.bleClock.interval1440"}];function ki(){if(String(C(i.bleClockSyncAdvertising)||"").toLowerCase()==="on")return d("common.clockSyncing");let t=String(C(i.bleClockSyncLastError)||"").trim();if(t==="clock_invalid")return d("settings.bleClock.waitingClock");if(t==="ble_busy")return d("settings.bleClock.busy");if(t)return t;let e=Number(_(i.bleClockSyncLastOkS)||0);if(!e)return d("settings.bleClock.never");let o=Math.max(0,Math.round(Date.now()/1e3)-e);if(o<60)return d("common.secondsAgo",{value:o});if(o<3600)return d("common.minutesAgo",{value:Math.round(o/60)});let a=Math.round(o/3600);return d("settings.bleClock.hoursAgo",{value:a})}var zi=()=>`
...
`,$p=P({tag:"settings-control-card",render:_i,onMount(t,e){M(e),e.querySelector(".sc-reset-probe-map").addEventListener("click",()=>{window.confirm("Reset the 1-Wire probe map and restart V6? Probe assignments must be discovered again.")&&Le("reset_1wire_probe_map_reboot")}),e.querySelector(".sc-dump-1wire").addEventListener("click",()=>{Le("dump_1wire_probe_diagnostics")}),e.querySelector(".sc-restart").addEventListener("click",()=>{window.confirm("Restart Lune V6 now? Heating continues after the controller has started again.")&&Le("restart")})}});var Ci=`
...
`;O("settings-motor-calibration-card",Ci);var yo=[{cls:"safe-runtime",key:"generic_runtime_limit_seconds",id:i.genericRuntimeLimitSeconds,labelKey:"settings.motor.maxSafeRuntime",unit:"s"},{cls:"close-threshold",key:"close_threshold_multiplier",id:i.closeThresholdMultiplier,labelKey:"settings.motor.closeThreshold",unit:"x"},{cls:"close-slope-threshold",key:"close_slope_threshold",id:i.closeSlopeThreshold,labelKey:"settings.motor.closeSlope",unit:"mA/s"},{cls:"close-slope-floor",key:"close_slope_current_factor",id:i.closeSlopeCurrentFactor,labelKey:"settings.motor.closeSlopeFloor",unit:"x"},{cls:"open-threshold",key:"open_threshold_multiplier",id:i.openThresholdMultiplier,labelKey:"settings.motor.openThreshold",unit:"x"},{cls:"open-slope-threshold",key:"open_slope_threshold",id:i.openSlopeThreshold,labelKey:"settings.motor.openSlope",unit:"mA/s"},{cls:"open-slope-floor",key:"open_slope_current_factor",id:i.openSlopeCurrentFactor,labelKey:"settings.motor.openSlopeFloor",unit:"x"},{cls:"open-ripple-limit",key:"open_ripple_limit_factor",id:i.openRippleLimitFactor,labelKey:"settings.motor.openRippleLimit",unit:"x"},{cls:"relearn-movements",key:"relearn_after_movements",id:i.relearnAfterMovements,labelKey:"settings.motor.relearnMovements",unit:"count"},{cls:"relearn-hours",key:"relearn_after_hours",id:i.relearnAfterHours,labelKey:"settings.motor.relearnHours",unit:"h"},{cls:"learn-min-samples",key:"learned_factor_min_samples",id:i.learnedFactorMinSamples,labelKey:"settings.motor.learnMinSamples",unit:"count"},{cls:"learn-max-deviation",key:"learned_factor_max_deviation_pct",id:i.learnedFactorMaxDeviationPct,labelKey:"settings.motor.learnMaxDeviation",unit:"%"}],Li=()=>{let t="";for(let e=0;e<yo.length;e++){let o=yo[e];if(o.key==="generic_runtime_limit_seconds")continue;let a=Mi(o.key)?"1":"0.1";t+='<div class="ui-row"><span class="ui-label"><span data-i18n="'+o.labelKey+'">'+d(o.labelKey)+"</span> ("+o.unit+')</span><span class="ui-field"><input type="number" class="ui-input smc-'+o.cls+'" value="0" step="'+a+'"></span></div>'}return`
  ...
  `};function Mi(t){return t==="learned_factor_min_samples"||t==="generic_runtime_limit_seconds"||t==="relearn_after_movements"||t==="relearn_after_hours"}var Yp=P({tag:"settings-motor-calibration-card",render:Li,onMount(t,e){let o=e.querySelector(".smc-profile"),a=e.querySelector(".smc-safe-runtime"),n=e.querySelector(".mc-drivers-toggle"),r=xe(e);function s(p){if(p==="HmIP VdMot"&&Me("hmip_runtime_limit_seconds",40),p==="Generic"){let u=Number(_(i.genericRuntimeLimitSeconds));(!Number.isFinite(u)||u<=0)&&Me("generic_runtime_limit_seconds",45)}}r.toggle(n,{read:()=>ce(i.drivers),commit:p=>_t(p)}),r.select(o,{read:()=>C(i.motorProfileDefault)||"HmIP VdMot",commit:p=>{Ae("motor_profile_default",p),s(p)}});function c(){let p=C(i.motorProfileDefault)||"HmIP VdMot";a.disabled=p==="HmIP VdMot"}r.num(a,{read:()=>(C(i.motorProfileDefault)||"HmIP VdMot")==="HmIP VdMot"?40:_(i.genericRuntimeLimitSeconds),commit:p=>{o.value==="Generic"&&Me("generic_runtime_limit_seconds",p)}});for(let p=0;p<yo.length;p++){let u=yo[p];if(u.key==="generic_runtime_limit_seconds")continue;let l=e.querySelector(".smc-"+u.cls);l&&(r.num(l,{read:()=>_(u.id),commit:m=>Me(u.key,m)}),z(u.id,r.refresh))}z(i.drivers,r.refresh),z(i.motorProfileDefault,()=>{r.refresh(),c()}),z(i.genericRuntimeLimitSeconds,r.refresh),z(i.hmipRuntimeLimitSeconds,r.refresh),M(e),s(C(i.motorProfileDefault)||"HmIP VdMot"),r.refresh(),c()}});var Ai=600*1e3,Vr=600,Ei=`
...
`;function Zr(t){let e=String(t||"").trim().replace(/^v/i,"").match(/^(\d+)\.(\d+)\.(\d+)/);return e?[Number(e[1]),Number(e[2]),Number(e[3])]:null}function Ht(t,e){let o=Zr(t);if(!o)return!1;let a=Zr(e);if(!a)return!0;for(let n=0;n<3;n++)if(o[n]!==a[n])return o[n]>a[n];return!1}function Ur(t){return String(t).replace(/[&<>]/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;"})[e])}function Ti(t){let e=String(t||"").trim();return e.length<=Vr?e:e.slice(0,Vr).replace(/\s+\S*$/,"")+"\u2026"}function Ni(){let t=document.querySelector(".settings-backup-card");if(!t)return;let e=t.closest("details");e&&(e.open=!0),t.scrollIntoView({behavior:"smooth",block:"center"});let o=t.querySelector(".sbk-save");o&&o.focus({preventScroll:!0})}var su=P({tag:"settings-firmware-card",render:Fi,onMount(t,e){let o=e.querySelector(".sfw-version"),a=e.querySelector(".sfw-status"),n=e.querySelector(".sfw-check"),r=e.querySelector(".sfw-banner"),s=e.querySelector(".sfw-hop"),c=e.querySelector(".sfw-notes"),p=e.querySelector(".sfw-install"),u=e.querySelector(".sfw-asset"),l=e.querySelector(".sfw-jump"),m=e.querySelector(".sfw-file"),g=e.querySelector(".sfw-choose"),f=e.querySelector(".sfw-upload"),b=e.querySelector(".sfw-filename"),y=e.querySelector(".sfw-progress"),h=y.querySelector("i"),w=e.querySelector(".sfw-upload-status"),E=null,S=!1,D=0,A=!1,I=()=>C(i.firmware)||N("firmwareVersion")||"",K=(L,k)=>{a.textContent=L||"",a.className="ui-sublabel sfw-status"+(k?" "+k:"")},j=()=>{let L=dt.firmware_update;if(!L||L.available!==!0)return null;let k=String(L.latest||"").trim();return k?{tag:k,notes:d("settings.firmware.deviceReported"),asset:Io(k)}:null},R=()=>{let L=j();return E?L&&Ht(L.tag,E.tag)?L:E:L},X=()=>{o.textContent=I()||d("settings.firmware.unknownVersion")},te=()=>{let L=R(),k=!!L&&Ht(L.tag,I());if(r.hidden=!k,!k){Te("firmwareUpdateAvailable",null);return}s.innerHTML=Ur(I()||d("settings.firmware.unknownVersion"))+" <span>\u2192</span> "+Ur(L.tag),c.textContent=Ti(L.notes)||d("common.noData"),u.href=L.asset.url,u.setAttribute("download",L.asset.name),u.title=L.asset.name,Te("firmwareUpdateAvailable",{current:I(),latest:L.tag,url:L.asset.url})},H=L=>{S||!L&&D&&Date.now()-D<Ai||(S=!0,D=Date.now(),n.disabled=!0,K(d("settings.firmware.checking")),Promise.resolve(Ia()).catch(()=>{}),Ba().then(k=>{E=k,te();let q=Ht(k.tag,I());K(q?d("settings.firmware.availableStatus",{version:k.tag}):d("settings.firmware.upToDate"),q?null:"ok")}).catch(k=>{E=null,te();let q=j();if(q){K(Ht(q.tag,I())?d("settings.firmware.availableStatus",{version:q.tag}):d("settings.firmware.upToDate"),Ht(q.tag,I())?null:"ok");return}if((k instanceof je?k.code:"network")==="no_releases"){K(d("settings.firmware.noReleases"),"ok");return}K(d("settings.firmware.checkFailed"),"err")}).finally(()=>{S=!1,n.disabled=!1}))};n.addEventListener("click",()=>H(!0)),l.addEventListener("click",Ni),p.addEventListener("click",()=>{let L=R();L&&window.confirm(d("settings.firmware.confirmInstall",{version:L.tag}))&&(p.disabled=!0,p.textContent=d("settings.firmware.installing"),Promise.resolve(qa()).then(()=>K(d("settings.firmware.installStarted"))).catch(()=>{K(d("settings.firmware.installFailed"),"err"),p.disabled=!1,p.textContent=d("settings.firmware.install")}))}),g.addEventListener("click",()=>m.click()),m.addEventListener("change",()=>{let L=m.files&&m.files[0];b.textContent=L?L.name:d("settings.firmware.noFile"),f.disabled=!L||A,w.textContent="",w.className="ui-note sfw-upload-status"}),f.addEventListener("click",()=>{let L=m.files&&m.files[0];!L||A||window.confirm(d("settings.firmware.confirmUpload",{file:L.name}))&&(A=!0,f.disabled=!0,g.disabled=!0,y.hidden=!1,h.style.width="0%",w.className="ui-note sfw-upload-status",w.textContent=d("settings.firmware.uploading",{value:0}),Promise.resolve(Ha()).catch(k=>console.warn("[Firmware] prepare rejected, continuing with upload:",k)).then(()=>$a(L,k=>{h.style.width=k+"%",w.textContent=d("settings.firmware.uploading",{value:k})})).then(()=>{h.style.width="100%",w.className="ui-note sfw-upload-status",w.textContent=d("settings.firmware.uploadDone")}).catch(k=>{console.error("[Firmware] upload failed:",k),y.hidden=!0,w.textContent=d("settings.firmware.uploadFailed")}).finally(()=>{A=!1,g.disabled=!1,f.disabled=!1}))}),Z("section",()=>{N("section")==="settings"&&H(!1)}),z(i.firmware,()=>{X(),te()}),z("firmware_update",te),M(e),X(),N("section")==="settings"&&H(!1)}});var Di=`
...
`,Au=P({tag:"smart-preheat-card",render:Bi,onMount(t,e){let o=e.querySelector(".absorb-toggle"),a=e.querySelector(".absorb-badge"),n=e.querySelector(".absorb-band"),r=e.querySelector(".absorb-delta"),s=e.querySelector(".absorb-body"),c=xe(e),p=l=>{s&&s.classList.toggle("is-disabled",!l)};c.toggle(o,{read:()=>ce(i.preheatAbsorbEnabled),onChange:p,commit:l=>{let m=l?"on":"off";x(i.preheatAbsorbEnabled,{state:m}),Ae("preheat_absorb_enabled",m)}}),c.num(n,{read:()=>_(i.preheatAbsorbBandC),commit:l=>{x(i.preheatAbsorbBandC,{value:l}),Me("preheat_absorb_band_c",l)}}),c.num(r,{read:()=>_(i.preheatDetectDeltaC),commit:l=>{x(i.preheatDetectDeltaC,{value:l}),Me("preheat_detect_delta_c",l)}});function u(){let l=String(C(i.preheatAbsorbing)||"").toLowerCase()==="active";a.textContent=l?d("common.active"):d("common.idle"),a.classList.toggle("active",l)}z(i.preheatAbsorbEnabled,c.refresh),z(i.preheatAbsorbing,u),z(i.preheatAbsorbBandC,c.refresh),z(i.preheatDetectDeltaC,c.refresh),M(e),c.refresh(),u()}});function Jr(t){let e=String(t||"").trim();return/^v?\d+\.\d+\.\d+-.+/.test(e)}na();var $i=`
...
<div class="ftr">Lune V6 \xB7 Local manifold controller</div></main></div></div></div>`;P({tag:"app-root",render:ji,onMount(t,e){e.querySelector(".hdr").appendChild(Q("hv6-header")),e.querySelector(".side-nav-slot").appendChild(Q("hv6-sidebar")),e.querySelector(".flow-diagram-slot").appendChild(Q("flow-diagram")),e.querySelector(".hydraulic-history-slot").appendChild(Q("graph-widgets",{variant:"flow-return"})),e.querySelector(".timeline-slot").appendChild(Q("zone-state-timeline")),e.querySelector(".connectivity-slot").appendChild(Q("connectivity-card")),e.querySelector(".zone-detail-slot").appendChild(Q("zone-detail",{zone:N("selectedZone")})),e.querySelector(".zone-sensor-slot").appendChild(Q("zone-sensor-card")),e.querySelector(".zone-coordination-slot").appendChild(Q("zone-coordination-card")),e.querySelector(".zone-actuator-slot").appendChild(Q("zone-actuator-card")),e.querySelector(".zone-room-slot").appendChild(Q("zone-room-card")),e.querySelector(".touch-slot").appendChild(Q("settings-touch-card")),e.querySelector(".manifold-slot").appendChild(Q("settings-manifold-card")),e.querySelector(".return-temp-slot").appendChild(Q("settings-return-temp-card")),e.querySelector(".minimum-flow-slot").appendChild(Q("settings-minimum-flow-card")),e.querySelector(".ble-clock-slot").appendChild(Q("settings-ble-clock-card")),e.querySelector(".preheat-slot").appendChild(Q("smart-preheat-card")),e.querySelector(".motor-slot").appendChild(Q("settings-motor-calibration-card")),e.querySelector(".firmware-slot").appendChild(Q("settings-firmware-card")),e.querySelector(".backup-slot").appendChild(Q("settings-backup-card")),e.querySelector(".appearance-slot").appendChild(Q("settings-appearance-card")),e.querySelector(".diag-actions-slot").appendChild(Q("settings-control-card")),e.querySelector(".manual-control-col").appendChild(Q("diag-manual-badge")),e.querySelector(".manual-control-col").appendChild(Q("diag-zone-motor-card",{zone:N("selectedZone")||1}));let o=e.querySelector(".motor-lab-slot"),a=e.querySelector('.sec[data-section="motorlab"]');function n(){let k=Jr(C(i.firmware)||N("firmwareVersion")),q=e.querySelector('.v6-side-link[data-section="motorlab"]');q&&(q.hidden=!k),a&&(a.hidden=!k),k&&o&&!o.firstChild&&o.appendChild(Q("diag-motor-lab")),!k&&N("section")==="motorlab"&&Pe("diagnostics")}z(i.firmware,n),Z("firmwareVersion",n),Z("section",n),n(),e.querySelector(".logs-main-col").appendChild(Q("logs-view")),e.querySelector(".system-health-slot").appendChild(Q("diag-system-card")),e.querySelector(".diag-health-slot").appendChild(Q("connectivity-card")),e.querySelector(".diag-health-slot").appendChild(Q("diag-i2c"));let r=e.querySelectorAll(".sec"),s=e.querySelector(".shell"),c=e.querySelector(".zone-detail-view"),p=e.querySelector(".selected-zone-title"),u=e.querySelector(".zone-overview-strip"),l=e.querySelector(".mobile-zone-dock"),m=e.querySelector(".mobile-zone-dock .zone-chipstrip");function g(k){let q=ce(v.enabled(k)),T=String(C(v.state(k))||"").toUpperCase()||"OFF",ie=String(C(v.motorLastFault(k))||"").toUpperCase();return q?q&&(T==="FAULT"||ie&&ie!=="NONE"&&ie!=="OK")?"FAULT":T:"OFF"}function f(k){return k==="HEATING"?d("state.heating"):k==="IDLE"?d("state.idle"):k==="FAULT"?d("common.fault"):k==="MANUAL"?d("state.manual"):k==="OVERHEATED"?d("state.overheated"):k==="CALIBRATING"?d("state.calibrating"):d("state.off")}function b(k){return k==="HEATING"||k==="CALLING"?"zs-heating":k==="FAULT"?"zs-fault":k==="IDLE"?"zs-idle":"zs-off"}function y(k){let q=String(k||"").trim();if(!q||/^none$/i.test(q)||q==="0"||q==="-1")return 0;let T=q.match(/(\d+)/),ie=T?Number(T[1]):0;return ie>=1&&ie<=6?ie:0}function h(){var ae;let k=[0,0,0,0,0,0,0];for(let B=1;B<=6;B++)k[B]=y(C(v.syncTo(B)));let q=[0,0,0,0,0,0,0];for(let B=1;B<=6;B++){let me=B;for(let be=0;be<6;be++){let re=k[me];if(!re||re<1||re>6)break;if(re===B){me=B;break}me=re}q[B]=me}let T={};for(let B=1;B<=6;B++)(T[ae=q[B]]||(T[ae]=[])).push(B);let ie=[[],[],[],[],[],[],[]];for(let B=1;B<=6;B++){let me=T[q[B]]||[B],be=me.length>1&&me.some(re=>k[re]>0);ie[B]=be?me.filter(re=>re!==B):[]}return{roots:q,partners:ie}}function w(){return window.matchMedia("(min-width: 901px)").matches}function E(){let k=h(),q=N("selectedZone")||1,T=w();u.setAttribute("role",T?"group":"list"),u.setAttribute("aria-label",T?"Select zone":"Zone status overview"),u.innerHTML=Array.from({length:6},(ie,ae)=>{var Y;let B=ae+1,me=B===q,be=Se(B),re=Oe(B),Fe=ue(_(v.temp(B))),he=ue((Y=_(v.effectiveSetpoint(B)))!=null?Y:_(v.setpoint(B))),Xe=g(B),it=f(Xe),He=b(Xe),lt=k.partners[B],qe=lt.length>0,wt=qe?d("overview.zone.mergedWith",{zones:lt.map(Mo).join(", ")}):"",Bt=qe&&lt.includes(B+1)&&k.roots[B]===k.roots[B+1],ko=qe&&lt.includes(B-1)&&k.roots[B]===k.roots[B-1],Ye=[qe?"is-merged":"",Bt?"zo-pair-start":"",ko?"zo-pair-cont":""].filter(Boolean).join(" "),ct=`${be}, ${Fe} / ${he}, ${it}${wt?", "+wt:""}`.replace(/"/g,"&quot;"),F=qe?`<span class="zo-merge">${wt}</span>`:"";return T?`<button type="button" class="zone-overview-card ${He}${Ye?" "+Ye:""}" data-zone-select="${B}" aria-current="${me?"true":"false"}" aria-label="${ct}" title="${ct}" tabindex="${me?"0":"-1"}"><span class="zo-status" aria-hidden="true"></span><span class="zo-title zone-label-compact">${re}</span><span class="zo-temps">${Fe} / ${he}</span>${F}</button>`:`<div class="zone-overview-card ${He}${Ye?" "+Ye:""}" role="listitem" aria-label="${ct}" title="${ct}"><span class="zo-status" aria-hidden="true"></span><span class="zo-title zone-label-compact">${re}</span><span class="zo-temps">${Fe} / ${he}</span>${F}</div>`}).join("")}function S(){let k=N("selectedZone")||1;m.innerHTML=Array.from({length:6},(q,T)=>{let ie=T+1,ae=ie===k,B=Se(ie),me=Oe(ie),be=B.replace(/"/g,"&quot;");return`<button type="button" class="zone-chip zone-label-compact" role="tab" aria-selected="${ae}" aria-label="${be}" title="${be}" tabindex="${ae?"0":"-1"}" data-zone-select="${ie}">${me}</button>`}).join("")}function D(){E(),S()}function A(){let k=N("section")==="zones";l.hidden=!k,l.setAttribute("aria-hidden",k?"false":"true"),s.classList.toggle("has-zone-dock",k)}function I(k){Vt(k)}function K(){let k=N("section")||"overview";r.forEach(q=>q.classList.toggle("active",q.dataset.section===k)),R()}function j(){let k=[],q=0,T=0;for(let he=1;he<=6;he++){let Xe=String(C(v.enabled(he))).toLowerCase()==="on",it=String(C(v.state(he))).toLowerCase(),He=String(C(v.motorLastFault(he))).toLowerCase();Xe&&k.push(he),Xe&&["heating","calling"].includes(it)&&q++,(it==="fault"||He!==""&&He!=="none"&&He!=="ok")&&T++}let ie=_(i.flow),ae=_(i.ret),B=String(C(i.authorityState)||"").replace(/_/g," "),me=T===0&&N("live"),be=`<div class="status-summary-main"><span class="eyebrow">System status</span><h2 class="${me?"status-ok":N("live")?"status-warn":"status-danger"}">${me?"Operating normally":N("live")?"Needs attention":"Device offline"}</h2><p>${T?T+" zone fault"+(T===1?"":"s")+" require attention.":N("live")?"V6 is running local control safely.":"Unable to read current manifold state."}</p></div><div class="status-fact"><span class="eyebrow">Heating</span><strong>${q} zones</strong><small>${k.length} enabled</small></div><div class="status-fact"><span class="eyebrow">Flow</span><strong>${ue(ie)}</strong><small>Return ${ue(ae)}</small></div><div class="status-fact"><span class="eyebrow">Touch</span><strong>${B||"not connected"}</strong><small>${_(i.authorityLeaseRemainingS)?Math.round(_(i.authorityLeaseRemainingS))+" s lease":"local control"}</small></div>`,re=ce(i.authorityConfigured),Fe=String(C(i.drivers)||"off");e.querySelector(".overview-status").innerHTML=be,e.querySelector(".hydraulic-summary").textContent=`${q} heating \xB7 Flow ${ue(ie)} \xB7 Return ${ue(ae)}`,e.querySelector(".settings-readiness").innerHTML=`<div class="status-summary-main"><span class="eyebrow">Configuration</span><h2 class="${N("live")?"status-ok":"status-danger"}">${N("live")?"Ready":"Waiting for device"}</h2><p>V6 validates and saves changes locally.</p></div><div class="status-fact"><span class="eyebrow">Device</span><strong>${N("live")?"Live":"Offline"}</strong><small>local controller</small></div><div class="status-fact"><span class="eyebrow">Touch</span><strong>${re?"Approved":"Not approved"}</strong><small>${re?"authenticated control":"local control only"}</small></div><div class="status-fact"><span class="eyebrow">Drivers</span><strong>${Fe}</strong><small>motor outputs</small></div>`,e.querySelector(".diagnostics-readiness").innerHTML=`<div class="status-summary-main"><span class="eyebrow">Overall health</span><h2 class="${T?"status-danger":me?"status-ok":"status-warn"}">${T?T+" issue"+(T===1?"":"s"):me?"Healthy":"Awaiting data"}</h2><p>${T?"Resolve current exceptions before using service controls.":"No active motor faults reported."}</p></div><div class="status-fact"><span class="eyebrow">Zone faults</span><strong>${T}</strong><small>${T?"requires review":"none reported"}</small></div><div class="status-fact"><span class="eyebrow">Drivers</span><strong>${Fe}</strong><small>motor outputs</small></div><div class="status-fact"><span class="eyebrow">Touch</span><strong>${B||"not connected"}</strong><small>${re?"approved":"local control"}</small></div>`,[e.querySelector(".overview-attention"),e.querySelector(".diagnostics-attention")].forEach(he=>{he.hidden=!T,he.innerHTML=T?`<strong>Review ${T} zone fault${T===1?"":"s"}</strong><span>Open Zones to inspect the affected valve and sensor state.</span>`:""})}function R(){let k=N("selectedZone")||1,q=N("section")==="zones";p.innerHTML=Oe(k),D(),A(),c.hidden=!q}function X(k){let q=k.target.closest("[data-zone-select]");q&&I(Number(q.dataset.zoneSelect))}function te(k){w()&&X(k)}function H(k){if(!w()||!["ArrowLeft","ArrowRight","Home","End"].includes(k.key))return;k.preventDefault();let q=N("selectedZone")||1,T=k.key==="Home"?1:k.key==="End"?6:k.key==="ArrowLeft"?q===1?6:q-1:q===6?1:q+1;I(T),requestAnimationFrame(()=>{var ie;return(ie=u.querySelector(`[data-zone-select="${T}"]`))==null?void 0:ie.focus()})}function L(k){if(!["ArrowLeft","ArrowRight","Home","End"].includes(k.key))return;k.preventDefault();let q=N("selectedZone")||1,T=k.key==="Home"?1:k.key==="End"?6:k.key==="ArrowLeft"?q===1?6:q-1:q===6?1:q+1;I(T),requestAnimationFrame(()=>{var ie;return(ie=m.querySelector(`[data-zone-select="${T}"]`))==null?void 0:ie.focus()})}u.addEventListener("click",te),u.addEventListener("keydown",H),window.matchMedia("(min-width: 901px)").addEventListener("change",E),m.addEventListener("click",X),m.addEventListener("keydown",L),e.querySelectorAll("[data-open-zones]").forEach(k=>k.addEventListener("click",()=>Pe("zones"))),e.querySelectorAll("[data-help-section]").forEach(k=>k.addEventListener("click",q=>{q.preventDefault(),Pe(k.dataset.helpSection)})),Z("section",K),Z("selectedZone",R),Z("live",j),Z("zoneNames",()=>{D(),j()});for(let k=1;k<=6;k++)[v.temp(k),v.setpoint(k),v.effectiveSetpoint(k),v.valve(k),v.state(k),v.enabled(k),v.motorLastFault(k),v.syncTo(k)].forEach(q=>z(q,()=>{j(),D()}));[i.flow,i.ret,i.authorityConfigured,i.authorityState,i.authorityLeaseRemainingS,i.drivers].forEach(k=>z(k,j)),M(e),K(),R(),j()}});function Vi(){let t=document.getElementById("app");if(!t)throw new Error("Dashboard root #app not found");t.innerHTML="",t.appendChild(Q("app-root")),Ja()}Vi();})();
