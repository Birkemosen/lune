...
- Poll `/api/v1/overview` and `/api/v1/zones`, while retaining `/state`
...
resource-shaped API on top of the current `/api/v1` namespace:
...
| 2. Pair and poll one Lune V6 over the current `/api/v1/state` endpoint. | Implemented | Touch stores nodes, scans/polls candidates, retains legacy `/state` ingestion as fallback, and prefers resource-shaped V6 endpoints when available. |
...
- API namespace: V6 host API is `/api/v1` (hard cut from `/api/lv6/v1`); Touch keeps
  its own `/api/lune-touch/v1` on the coordinator host. Do not merge those namespaces.
