...
void RpiDpiRgb::setup() {
  ...
  if (this->enable_pin_ != nullptr) {
    ESP_LOGI(TAG, "7B panel power sequence: DISP/backlight high after RGB init");
...
void RpiDpiRgb::reset_display_() const {
  if (this->reset_pin_ != nullptr) {
    ESP_LOGI(TAG, "7B panel power sequence: reset low");
    this->reset_pin_->setup();
    this->reset_pin_->digital_write(false);
    delay(20);
    ESP_LOGI(TAG, "7B panel power sequence: reset high");
    this->reset_pin_->digital_write(true);
    delay(120);
  }
  if (this->enable_pin_ != nullptr) {
    this->enable_pin_->setup();
    ESP_LOGI(TAG, "7B panel power sequence: DISP/backlight low before RGB init");
