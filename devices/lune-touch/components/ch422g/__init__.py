...
CONF_CH422G = "ch422g"
CONF_INITIAL_OUTPUT_BITS = "initial_output_bits"

CONFIG_SCHEMA = cv.Schema(
    {
        cv.GenerateID(CONF_ID): cv.declare_id(CH422GComponent),
        cv.GenerateID(CONF_I2C_ID): cv.use_id(I2CBus),
        cv.Optional(CONF_ADDRESS, default=0x24): cv.i2c_address,
        cv.Optional(CONF_INITIAL_OUTPUT_BITS, default=0x00): cv.int_range(
            min=0x00, max=0xFF
        ),
    }
...
async def to_code(config):
    ...
    cg.add(var.set_initial_output_bits(config[CONF_INITIAL_OUTPUT_BITS]))
