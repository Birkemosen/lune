(()=>{var h=new Set,n={section:"overview",loading:!1,error:"",overview:null,nodes:[],zones:[],forecast:null,commands:[],diagnostics:null};function C(e){return h.add(e),()=>h.delete(e)}function E(){for(let e of h)e(n)}function N(e){n.section=e,E()}function v(e){Object.assign(n,e),E()}var T=[["overview","Overview"],["zones","Zones"],["manifolds","Manifolds"],["forecast","Forecast"],["commands","Commands"],["settings","Settings"],["diagnostics","Diagnostics"]];function q(){var e,a,t,s,o,d;return`
    <header class="topbar">
      <nav class="top-menu">
        ${T.map(([l,r])=>`<button class="menu-link ${n.section===l?"active":""}" data-section="${l}">${r}</button>`).join("")}
      </nav>
      <div class="brand">
    ...
    </header>`}function L(e){e.querySelectorAll("[data-section]").forEach(a=>{a.addEventListener("click",()=>N(a.dataset.section))})}var x="/api/lune-touch/v1";function Z(e){if(e==="/overview")return{summary:{zones:18,nodes:3,calling:5,stale_nodes:1,comfort_avg_c:21.1,forecast_status:"stale",latest_command:"accepted"}};if(e==="/nodes")return{nodes:[{id:"v6-a",hostname:"lune-v6-a.local",ip:"192.168.1.51",firmware:"mock",reachable:!0,trust:2},{id:"v6-b",hostname:"lune-v6-b.local",ip:"192.168.1.52",firmware:"mock",reachable:!0,trust:2},{id:"v6-c",hostname:"lune-v6-c.local",ip:"192.168.1.53",firmware:"mock",reachable:!1,trust:1}]};if(e==="/zones"){let a=["Living","Kitchen","Bath","Hall","Office","Bedroom","Guest","Utility","Laundry","Workshop","Pantry","Landing","Kids west","Kids east","Ensuite","Basement","Garage","Spare"],t=["heat","idle","call","hold","idle","preheat","idle","heat","idle","call","idle","hold","idle","heat","call","stale","idle","unused"];return{count:18,zones:a.map((s,o)=>({room_id:`room-${String(o+1).padStart(2,"0")}`,name:s,node_index:Math.floor(o/6),zone_index:o%6,temperature_c:[21.3,20.9,22.2,20.1,20.8,19.4,19.8,18.9,18.7,17.6,18.1,20.3,20.5,20,21.8,null,12.4,null][o],setpoint_c:[21,21,22.5,20,21,19.5,20,19,18.5,18,18,20,20.5,20.5,22,18,12,null][o],status:t[o],fresh:t[o]!=="stale"}))}}return e==="/forecast"?{status:"stale",location:{mode:"manual",latitude:0,longitude:0},last_fetch_age_s:0,decisions:[{room_id:"room-01",offset_c:.4,peak_in_h:10,reason:"mock wind preload"}]}:e==="/commands"?{commands:[{request_id:"mock-forecast-1",source:"forecast",reason:"wind preload",node_index:0,zone_index:0,requested_offset_c:.4,accepted_offset_c:.4,result:"accepted",clamp_applied:!1}]}:e==="/diagnostics"?{heap:"watching",nodes:3,zones:18,ledger:1,screen:"overview-only",api:x}:{}}async function m(e){var s,o;if((s=window.LUNE_TOUCH_DASHBOARD_CONFIG)!=null&&s.mock)return Z(e);let a=await fetch(x+e,{cache:"no-store"});if(!a.ok)throw new Error(`${e} failed: ${a.status}`);let t=await a.json();if(t&&t.ok===!1)throw new Error(((o=t.error)==null?void 0:o.message)||"API error");return t.data||t}async function i(){v({loading:!0,error:""});try{let[e,a,t,s,o,d]=await Promise.all([m("/overview"),m("/nodes"),m("/zones"),m("/forecast"),m("/commands"),m("/diagnostics")]);v({overview:e,nodes:a.nodes||[],zones:t.zones||[],forecast:s,commands:o.commands||[],diagnostics:d,loading:!1})}catch(e){v({loading:!1,error:e.message||String(e)})}}async function p(e,a={}){var d;if((d=window.LUNE_TOUCH_DASHBOARD_CONFIG)!=null&&d.mock)return{ok:!0};let t=new URLSearchParams;Object.entries(a).forEach(([l,r])=>{r!=null&&t.set(l,String(r))});let s=`${x}${e}${t.toString()?`?${t}`:""}`,o=await fetch(s,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(a)});if(!o.ok)throw new Error(`${e} failed: ${o.status}`);return o.json()}var c={scanNodes:()=>p("/nodes/scan"),addNode:e=>p("/nodes",e),removeNode:e=>p(`/nodes/${encodeURIComponent(e)}/remove`),saveZone:(e,a)=>p(`/zones/${encodeURIComponent(e)}`,a),setpointCommand:(e,a)=>p(`/zones/${encodeURIComponent(e)}/setpoint-command`,a),saveForecast:e=>p("/forecast/settings",e),fetchForecast:()=>p("/forecast/fetch")};var u=e=>e==null||Number.isNaN(e)?"--.- C":`${Number(e).toFixed(1)} C`,b=e=>`V6-${String.fromCharCode(65+Number(e||0))}`;function w(e){return e==="heat"||e==="call"||e==="preheat"?"ok":e==="stale"?"warn":e==="unused"?"muted":""}function P(e){return`<article class="zone-card ${w(e.status)}">
  ...
  </article>`}function A(){var a;let e=((a=n.overview)==null?void 0:a.summary)||{};return`<section class="panel">
    <div class="stat-grid">
    ...
    <div class="zone-matrix">${n.zones.map(P).join("")}</div>
    ...
    <div class="inline-form">
      ...
      <select class="input mini-input" id="map-node">${n.nodes.map((a,t)=>`<option value="${t}">${a.id||b(t)}</option>`).join("")||'<option value="0">V6-0</option>'}</select>
    ...
    <div class="data-table">
      ...
      ${n.zones.map(a=>`<div class="tr">
        <span>${a.name}</span><span>${u(a.temperature_c)}</span><span>${u(a.setpoint_c)}</span><span class="${w(a.status)}">${a.status}</span><span>${b(a.node_index)} / Z${Number(a.zone_index)+1}</span>
        <span><button class="btn slim" data-command-room="${a.room_id}">+0.5 C / 45m</button></span>
      </div>`).join("")}
    </div>
  ...
  </section>`}function R(){var a,t,s;let e=n.forecast||{};return`<section class="panel two-col">
    <div>
  ...
  </section>`}function F(){return`<section class="panel">
    ...
    <div class="data-table">
      ...
      ${n.commands.map(e=>`<div class="tr"><span>${e.request_id}</span><span>${e.source}</span><span>${e.reason}</span><span>${b(e.node_index)} / Z${Number(e.zone_index)+1}</span><span>${e.requested_offset_c}</span><span>${e.accepted_offset_c}</span><span class="${w(e.result==="accepted"?"heat":e.result)}">${e.result}</span></div>`).join("")}
    </div>
  </section>`}function O(){return`<section class="panel two-col">
  ...
  </section>`}function H(){let e=n.diagnostics||{};return`<section class="panel">
  ...
  </section>`}function U(e){var a,t,s,o,d,l;(a=e.querySelector('[data-action="refresh"]'))==null||a.addEventListener("click",i),(t=e.querySelector('[data-action="scan"]'))==null||t.addEventListener("click",()=>c.scanNodes().then(i)),(s=e.querySelector('[data-action="forecast-fetch"]'))==null||s.addEventListener("click",()=>c.fetchForecast().then(i)),(o=e.querySelector('[data-action="geo"]'))==null||o.addEventListener("click",()=>{navigator.geolocation&&navigator.geolocation.getCurrentPosition(r=>{c.saveForecast({latitude:r.coords.latitude,longitude:r.coords.longitude,source:"browser"}).then(i)})}),(d=e.querySelector('[data-action="add-node"]'))==null||d.addEventListener("click",()=>{var f,g;let r=(g=(f=e.querySelector("#node-host"))==null?void 0:f.value)==null?void 0:g.trim();r&&c.addNode({hostname:r}).then(i)}),(l=e.querySelector('[data-action="save-room-map"]'))==null||l.addEventListener("click",()=>{var $,y,k,_,S,z;let r=(y=($=e.querySelector("#map-room-id"))==null?void 0:$.value)==null?void 0:y.trim(),f=((_=(k=e.querySelector("#map-room-name"))==null?void 0:k.value)==null?void 0:_.trim())||r,g=Number(((S=e.querySelector("#map-node"))==null?void 0:S.value)||0),B=Math.max(0,Number(((z=e.querySelector("#map-zone"))==null?void 0:z.value)||1)-1);r&&c.saveZone(r,{name:f,node_index:g,zone_index:B}).then(i)}),e.querySelectorAll("[data-remove-node]").forEach(r=>{r.addEventListener("click",()=>c.removeNode(r.dataset.removeNode).then(i))}),e.querySelectorAll("[data-command-room]").forEach(r=>{r.addEventListener("click",()=>c.setpointCommand(r.dataset.commandRoom,{offset_c:.5,ttl_s:2700,reason:"dashboard quick boost"}).then(i))})}var V=`
...
`;function G(){return n.section==="zones"?j():n.section==="manifolds"?I():n.section==="forecast"?R():n.section==="commands"?F():n.section==="settings"?O():n.section==="diagnostics"?H():A()}function D(e){if(!document.getElementById("lt-style")){let t=document.createElement("style");t.id="lt-style",t.textContent=V,document.head.appendChild(t)}function a(){e.innerHTML=`<div class="shell">${q()}${n.error?`<div class="error">${n.error}</div>`:""}${G()}</div>`,L(e),U(e)}C(a),a(),i(),setInterval(i,5e3)}var M=document.getElementById("app");if(!M)throw new Error("Dashboard root #app not found");D(M);})();
