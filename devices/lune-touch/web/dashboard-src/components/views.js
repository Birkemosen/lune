...
export function renderSettings() {
  ...
  const asgardMode = asgard.mode || (asgard.enabled === false ? 'disabled' : 'advisory');
  ...
  return `<section class="panel settings-layout">
    ...
    <div class="settings-panel wide"><h3>Coordinator</h3>
      ...
      <label class="check"><input id="settings-asgard-enabled" type="checkbox" ${asgard.enabled === false ? '' : 'checked'}> Asgard / Odin</label>
...
export function bindActions(root) {
  ...
  root.querySelector('[data-action="save-settings"]')?.addEventListener('click', () => {
    ...
    const asgard_enabled = root.querySelector('#settings-asgard-enabled')?.checked ? 1 : 0;
    const asgard_mode = root.querySelector('#settings-asgard-mode')?.value;
